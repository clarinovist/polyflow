...
async function main() {
    ...
    const variantPct =
        totalVariants > 0
            ? ((mappedVariants / totalVariants) * 100).toFixed(1)
            : '0';
    ...
    const productPct =
        totalProducts > 0
            ? ((mappedProducts / totalProducts) * 100).toFixed(1)
            : '0';
    ...
    const soLinePct =
        recentSOItems > 0
            ? (
                  ((recentSOItemsWithVariantRevenue +
                      recentSOItemsWithProductRevenue) /
                      recentSOItems) *
                  100
              ).toFixed(1)
            : '0';
    ...
    console.log(
        `Total mapped:     ${recentSOItemsWithVariantRevenue + recentSOItemsWithProductRevenue}`,
    );
    ...
    if (unmapped.length > 0) {
        ...
        for (const v of unmapped) {
            console.log(
                `  ${v.skuCode.padEnd(20)} ${v.name.padEnd(25)} (${v.product.name})`,
            );
        }
        if (totalVariants - mappedVariants > 10) {
            console.log(
                `  ... and ${totalVariants - mappedVariants - 10} more`,
            );
        }
    }
