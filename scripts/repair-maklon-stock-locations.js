...
const STAGE_SLUG_SET = new Set(
    MAKLON_STAGE_LOCATIONS.map((location) => location.slug),
);
...
async function transferStock(tx, params) {
    ...
    const currentQty = await getCurrentQuantity(
        tx,
        sourceLocationId,
        productVariantId,
    );
...
async function buildTasks(tx) {
    const targetLocations = await Promise.all(
        MAKLON_STAGE_LOCATIONS.map((location) => ensureLocation(tx, location)),
    );
    const destinationBySlug = new Map(
        targetLocations.map((location) => [location.slug, location]),
    );
    ...
    for (const receipt of maklonReceipts) {
        if (
            receipt.location?.slug &&
            STAGE_SLUG_SET.has(receipt.location.slug)
        ) {
            continue;
        }
        ...
        for (const item of receipt.items) {
            ...
            const existing = tasks.get(key) || {
                ...
                sourceLocationName:
                    receipt.location?.name || receipt.locationId,
    ...
    for (const order of maklonOrders) {
        ...
        const destinationSlug = resolveProductionDestinationSlug(
            order.bom?.category,
        );
        const destination = destinationBySlug.get(destinationSlug);

        const producedQtyFromExecutions = order.executions.reduce(
            (sum, execution) => sum + toNumber(execution.quantityProduced),
            0,
        );
        const producedQty =
            toNumber(order.actualQuantity) > 0
                ? toNumber(order.actualQuantity)
                : producedQtyFromExecutions;
        ...
        existing.reasons.push(
            `WO ${order.orderNumber} (${order.bom.category || 'UNKNOWN'})`,
        );
        tasks.set(key, existing);
    }
    ...
    return {
        tasks: Array.from(tasks.values()).sort((left, right) => {
            if (left.sourceLocationName === right.sourceLocationName) {
                return left.productVariantId.localeCompare(
                    right.productVariantId,
                );
            }

            return left.sourceLocationName.localeCompare(
                right.sourceLocationName,
            );
        }),
    };
}
...
async function main() {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (!args.execute || !args.yes) {
            console.log(
                'Dry-run complete. Re-run with --execute --yes to apply the transfer movements.',
            );
            return;
        }
        ...
        console.log(
            `Repair complete. Tasks moved: ${movedTasks}. Total qty moved: ${formatQty(movedQtyTotal)}.`,
        );
