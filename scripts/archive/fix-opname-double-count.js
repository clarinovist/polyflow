...
async function main() {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        // 2. Resolve accounts
        const acc11310 = await tx.account.findUnique({
            where: { code: '11310' },
        });
        const acc81100 = await tx.account.findUnique({
            where: { code: '81100' },
        });
        if (!acc11310 || !acc81100)
            throw new Error('Account 11310 or 81100 not found');

        // 3. Generate unique entry number
        const entryNumber = 'ADJ-' + Date.now();

        // 4. Find userId from existing entry
        const refEntry = await tx.journalEntry.findFirst({
        ...
        const entry = await tx.journalEntry.create({
            data: {
                ...
                reference: REFERENCE,
                description:
                    'Koreksi double-count: OB persediaan BB vs Stock Opname Feb 2',
                status: 'POSTED',
                    ...
                    create: [
                        {
                            ...
                            credit: 0,
                            description:
                                'Reverse inflated adjustment gain (OB overlap)',
                        },
                        {
                            ...
                            description:
                                'Reverse double-counted Raw Material from OB + Opname',
                        },
                    ],
                },
            },
        ...
        console.log(
            '  Debit  81100 (reduce gain): ' +
                CORRECTION_AMOUNT.toLocaleString(),
        );
        console.log(
            '  Credit 11310 (reduce inventory): ' +
                CORRECTION_AMOUNT.toLocaleString(),
        );
    });
    ...
    const finalBal = allLines.reduce(
