...
async function main() {
    ...
    const tradeReceivables = await prisma.account.findUnique({
        where: { code: '11210' },
    });
    const wrongAccount12400 = await prisma.account.findUnique({
        where: { code: '12400' },
    });
    ...
    if (tradeReceivables) {
        ...
        if (packagingWithWrongAccount.length > 0) {
            ...
            console.log(
                `  ✅ Fixed ${updateResult.count} PACKAGING products → 11340 (${pkgAcct.name})`,
            );
        }
    }
    ...
    for (const [type, accountId] of Object.entries(typeToAccount)) {
        ...
        if (missingCount > 0) {
            ...
            console.log(
                `  ✅ ${type}: Set inventoryAccountId for ${missingCount} products`,
            );
        } else {
            console.log(
                `  ✓ ${type}: All products have inventoryAccountId set`,
            );
        }
    }
    ...
    for (const row of allProducts) {
        ...
        console.log(
            `  ${row.productType}: ${withAccount}/${row._count.id} mapped`,
        );
    }
