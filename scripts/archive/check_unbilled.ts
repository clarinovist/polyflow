...
async function main() {
    ...
    console.log(
        `Found account: ${unbilledAccount.name} (${unbilledAccount.code}) - ID: ${unbilledAccount.id}`,
    );
    ...
    for (const line of lines) {
        ...
        console.log(
            `- ${line.journalEntry?.date?.toISOString().split('T')[0]} | Ref: ${line.journalEntry?.reference} | Debit: ${line.debit} | Credit: ${line.credit} | Net Impact on Liability: ${Number(line.credit) - Number(line.debit)} | Desc: ${line.description}`,
        );
    }
