...
async function main() {
    ...
    console.log(
        `Found SO. Reverting stock for ${so.movements.length} movements...`,
    );
    // 1. Revert stock
    for (const mov of so.movements) {
        if (mov.type === 'OUT') {
            ...
            console.log(
                `Returned quantity ${mov.quantity} for variant ${mov.productVariantId}`,
            );
        }
    }
    ...
    console.log(
        `Successfully reversed stock and cancelled SO, DO, and Invoice for ${orderNumber}`,
    );
}
