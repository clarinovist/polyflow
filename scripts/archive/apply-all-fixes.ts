...
async function step1() {
    ...
    const get = async (code: string) =>
        (await prisma.account.findUnique({ where: { code } }))?.id;
    ...
    const id12400acc = (
        await prisma.account.findUnique({ where: { code: '12400' } })
    )?.id;
    ...
    if (id11340) {
        ...
        if (pkg.count)
            console.log(`  ✅ ${pkg.count} PACKAGING products → 11340`);
    }
    ...
    if (id11350) {
        ...
        if (scrap.count)
            console.log(`  ✅ ${scrap.count} SCRAP products → 11350`);
    }
...
async function step2() {
    ...
    for (const { from, to, ref } of reclassMap) {
        const alreadyDone = await prisma.journalEntry.findFirst({
            where: { reference: ref, status: 'POSTED' },
        });
        if (alreadyDone) {
            console.log(
                `  ℹ️  ${from}→${to}: sudah dikerjakan (${alreadyDone.entryNumber})`,
            );
            continue;
        }
        ...
        if (!accFrom) continue;
        const balance = accFrom.journalLines.reduce(
            (s, l) => s + Number(l.debit) - Number(l.credit),
            0,
        );
        if (balance <= 0) {
            console.log(`  ⏭  ${from}: saldo 0, skip`);
            continue;
        }

        const toAcc = await prisma.account.findUniqueOrThrow({
            where: { code: to },
        });
                    ...
                    create: [
                        {
                            ...
                            debit: balance,
                            credit: 0,
                            description: `Reclass masuk dari ${from}`,
                        },
                        {
                            accountId: accFrom.id,
                            debit: 0,
                            credit: balance,
                            description: `Reclass keluar ke ${to}`,
                        },
                    ],
                },
            },
        ...
        console.log(
            `  ✅ ${entryNum}: Rp ${balance.toLocaleString()} ${from} → ${to}`,
        );
    }
...
async function step3() {
    ...
    await prisma.account.update({
        where: { code: '11330' },
        data: { name: 'Barang Jadi - Roll/Gulungan' },
    });
    await prisma.account.update({
        where: { code: '11340' },
        data: { name: 'Barang Jadi - Pack/Kresek' },
    });
...
async function step6() {
    ...
    if (exists) {
        console.log(`  ℹ️  Sudah dilakukan (${exists.entryNumber}), skip`);
        return;
    }

    const acc11310 = await prisma.account.findUniqueOrThrow({
        where: { code: '11310' },
    });
    const acc11340 = await prisma.account.findUniqueOrThrow({
        where: { code: '11340' },
    });

    const pkgIds = (
        await prisma.productVariant.findMany({
            where: { product: { productType: 'PACKAGING' } },
            select: { id: true },
        })
    ).map((v) => v.id);
    ...
    const movIds = (
        await prisma.stockMovement.findMany({
            where: { productVariantId: { in: pkgIds } },
            select: { id: true },
        })
    ).map((m) => m.id);
    ...
    if (total > 0) {
        ...
        await prisma.journalEntry.create({
            data: {
                ...
                lines: {
                    create: [
                        {
                            accountId: acc11340.id,
                            debit: total,
                            credit: 0,
                            description:
                                'Reklasifikasi masuk: Barang Jadi Pack/Kresek',
                        },
                        {
                            accountId: acc11310.id,
                            debit: 0,
                            credit: total,
                            description:
                                'Reklasifikasi keluar dari Raw Materials',
                        },
                    ],
                },
            },
        ...
        console.log(
            `  ✅ ${entryNum}: Rp ${total.toLocaleString()} dari 11310 → 11340`,
        );
    } else {
...
async function step7() {
    ...
    for (const code of ['11310', '11320', '11330', '11340', '11350']) {
        ...
        if (acc) {
            const bal = acc.journalLines.reduce(
                (s, l) => s + Number(l.debit) - Number(l.credit),
                0,
            );
            total += bal;
            console.log(
                `  ${code} (${acc.name}): Rp ${Math.round(bal).toLocaleString()}`,
            );
        }
    }
    console.log(
