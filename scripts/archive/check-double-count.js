...
async function check() {
    ...
    for (const [code, obAmount] of Object.entries(OB_ACCOUNTS)) {
        ...
        const totalOpnameDebit = opnameLines.reduce(
            (s, l) => s + Number(l.debit),
            0,
        );
        const totalOpnameCredit = opnameLines.reduce(
        ...
        const totalBalance = allLines.reduce(
        ...
        console.log(
            '  Stock Opname journals: ' +
                opnameNet.toLocaleString() +
                ' (Feb 2 adjustments)',
        );
        console.log(
            '  Overlap (double-count): ' +
                Math.min(obAmount, opnameNet).toLocaleString(),
        );
        console.log(
            '  Current total balance: ' + totalBalance.toLocaleString(),
        );
        console.log(
            '  Correct balance would: ' +
                (totalBalance - Math.min(obAmount, opnameNet)).toLocaleString(),
        );
        console.log('');
    }
    ...
    if (gainAcc) {
        ...
        const gainBalance = gainLines.reduce(
            (s, l) => s + Number(l.credit) - Number(l.debit),
            0,
        );
        console.log('--- 81100 Inventory Adjustment Gain ---');
        console.log(
            '  Current balance (revenue): ' + gainBalance.toLocaleString(),
        );
        console.log(
            '  After correction: ' +
                (
                    gainBalance -
                    Object.values(OB_ACCOUNTS).reduce((a, b) => a + b, 0)
                ).toLocaleString(),
        );
    }
