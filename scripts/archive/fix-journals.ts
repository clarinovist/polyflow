...
async function main() {
    console.log(
        'Starting script to fix Production Output Trade Receivables issue...',
    );
    ...
    console.log(
        `Found ${products.length} Products with incorrect inventory account (Trade Receivables).`,
    );
    ...
    console.log(
        `Found ${linesToFix.length} JournalLines erroneously hitting Trade Receivables for Production Output.`,
    );
    ...
    for (const line of linesToFix) {
        ...
        console.log(
            `Updated Journal Line ${line.id} (Entry: ${line.journalEntry.entryNumber})`,
        );
    }
