...
async function testManualJournal() {
    console.log('🔍 Verifying manual journal entry for account 11512...');
    ...
    if (!account) {
        throw new Error(
            'Account 11512 (Piutang Fadilla) not found after migration',
        );
    }
    ...
    try {
        ...
        console.log(
            '✅ SUCCESS: Manual journal entry created for account 11512!',
        );
        ...
        console.error(
            '❌ FAILED: Still unable to create manual journal entry.',
        );
        console.error(error);
        process.exit(1);
    }
...
testManualJournal().finally(async () => {
