...
async function main() {
    ...
    if (available + 1e-9 < QTY) {
        throw new Error(
            `Insufficient pelembab at RM: have ${available}, need ${QTY}`,
        );
    }
    if (
        !pelembab.product.inventoryAccountId ||
        !pelembab.product.wipAccountId
    ) {
        throw new Error(
            'Pelembab product missing inventoryAccountId or wipAccountId',
        );
    }
    if (totalAmount <= 0) {
