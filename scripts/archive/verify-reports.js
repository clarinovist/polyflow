...
async function verify() {
    ...
    const netIncome = accountsIS.reduce((sum, a) => {
        ...
        const balance = a.journalLines.reduce(
    ...
    const assets = accountsBS
        .filter((a) => a.type === 'ASSET')
        .reduce(
            (sum, a) =>
                sum +
                a.journalLines.reduce(
                    (s, l) => s + (Number(l.debit) - Number(l.credit)),
                    0,
                ),
            0,
        );
    const liabilities = accountsBS
    ...
    const equityExcluding33000 = accountsBS
        .filter((a) => a.type === 'EQUITY' && a.code !== '33000')
        .reduce(
            (sum, a) =>
                sum +
                a.journalLines.reduce(
    ...
    console.log(
        'Balance Sheet 33000 (Closed) :',
        balance33000.toLocaleString('id-ID'),
    );
    console.log(
        'Balance Sheet Unposted       :',
        unpostedEarnings.toLocaleString('id-ID'),
    );
    console.log(
        'Total Laba di Balance Sheet  :',
        totalLabaBS.toLocaleString('id-ID'),
    );
    console.log('-----------------------');
    console.log(
        'MATCH:',
        Math.abs(netIncome - totalLabaBS) < 0.01 ? 'YA ✅' : 'TIDAK ❌',
    );
