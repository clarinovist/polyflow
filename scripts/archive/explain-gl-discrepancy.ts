...
async function main() {
    ...
    for (const code of codes) {
        ...
        if (acc)
            glByAccount[code] = acc.journalLines.reduce(
    ...
    const physByType: Record<
        string,
        { qty: number; value: number; zeroCount: number }
    > = {};
    ...
    for (const item of stock) {
        ...
        if (!physByType[type])
            physByType[type] = { qty: 0, value: 0, zeroCount: 0 };
    ...
    for (const code of codes) {
        ...
        totalPhys += phys;
        console.log(
            `${code}   | Rp ${gl.toLocaleString().padStart(15)} | Rp ${phys.toLocaleString().padStart(13)} | ${diff >= 0 ? '+' : ''}Rp ${diff.toLocaleString()}`,
        );
    }
    console.log(
        '--------------------------------------------------------------',
    ...
    console.log(
        `TOTAL   | Rp ${totalGL.toLocaleString().padStart(15)} | Rp ${totalPhys.toLocaleString().padStart(13)} | +Rp ${(totalGL - totalPhys).toLocaleString()}`,
    );
    ...
    console.log(
        '  - GL mencakup pembelian RM historis yang sudah dipakai, tapi masih ada saldo negatif dari pemakaian',
    );
    ...
    console.log(
        '  Kemungkinan: Rp 96M dari 12500 direklas ke sini, tapi physical FINISHED_GOOD hanya Rp 9M',
    );
    console.log(
        '  → Inventory fisik Roll sudah habis terjual/dipakai, tapi jurnal belum ada untuk COGS-nya',
    );
    ...
    console.log(
        '  Kemungkinan: WIP GL masih ada dari produksi yang belum di-complete journal-nya',
    );
    ...
    const fgStock = await prisma.inventory.findMany({
        ...
        select: {
            quantity: true,
            averageCost: true,
            productVariant: { select: { name: true, standardCost: true } },
    ...
    fgStock.forEach((i) => {
        const cost =
            ...
            Number(i.productVariant.standardCost ?? 0);
        console.log(
            `  ${i.productVariant.name} @ ${i.location.name}: qty=${Number(i.quantity).toFixed(2)} cost=${cost.toLocaleString()} val=Rp ${(Number(i.quantity) * cost).toLocaleString()}`,
        );
    });
}
