...
async function main() {
    console.log(
        'Starting data migration for MaterialIssue and ScrapRecord locations...',
    );
    ...
    console.log(
        `Found ${issues.length} MaterialIssue records without locationId.`,
    );
    ...
    for (const issue of issues) {
        ...
        if (movement && movement.fromLocationId) {
            ...
            console.log(
                `Updated MaterialIssue ${issue.id} with location ${movement.fromLocationId}`,
            );
        } else {
            console.warn(
                `Could not find movement for MaterialIssue ${issue.id} (Order: ${orderNumber})`,
            );
            // Fallback to rm_warehouse slug if possible
            const rmLoc = await prisma.location.findUnique({
            ...
            if (rmLoc) {
                ...
                console.log(
                    `Applied fallback 'rm_warehouse' to MaterialIssue ${issue.id}`,
                );
            }
        }
    }
    ...
    console.log(
        `Found ${scraps.length} ScrapRecord records without locationId.`,
    );
    ...
    for (const scrap of scraps) {
        ...
        if (movement && movement.toLocationId) {
            ...
            console.log(
                `Updated ScrapRecord ${scrap.id} with location ${movement.toLocationId}`,
            );
        } else {
            console.warn(
                `Could not find movement for ScrapRecord ${scrap.id} (Order: ${orderNumber})`,
            );
            // Fallback to scrap_warehouse slug if possible
            const scrapLoc = await prisma.location.findUnique({
            ...
            if (scrapLoc) {
                ...
                console.log(
                    `Applied fallback 'scrap_warehouse' to ScrapRecord ${scrap.id}`,
                );
            }
        }
    }
