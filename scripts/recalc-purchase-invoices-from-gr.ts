...
async function calcPoTotalFromGr(po: {
  ...
  goodsReceipts: Array<{ items: Array<{ productVariantId: string; receivedQty: { toNumber(): number } }> }>;
}): Promise<number> {
  ...
  for (const gr of po.goodsReceipts) {
    for (const it of gr.items) {
      ...
      receivedMap.set(it.productVariantId, (receivedMap.get(it.productVariantId) ?? 0) + qty);
    }
  }
  let total = 0;
...
async function main() {
  ...
  const pos = await prisma.purchaseOrder.findMany({
    ...
    select: {
      ...
      goodsReceipts: {
        select: {
          items: { select: { productVariantId: true, receivedQty: true } },
        },
      },
      invoices: {
        select: { id: true, invoiceNumber: true, totalAmount: true, status: true, paidAmount: true },
      },
    },
  ...
  for (const po of pos) {
    ...
    for (const inv of po.invoices) {
      ...
      if (Math.abs(delta) > 0.01) {
        ...
        const isFixable =
          inv.status === 'DRAFT' ||
          (inv.status === 'UNPAID' && inv.paidAmount.toNumber() === 0);
        console.log(
        ...
        if (apply && isFixable) {
          ...
          console.log(`  -> Updated invoice ${inv.invoiceNumber} to ${grTotal}`);
          fixedCount++;
        }
      }
    }
