...
async function main() {
  ...
  for (const sq of quotations) {
        ...
        if (APPLY) {
          await prisma.salesOrder.update({
            ...
            data: {
              ...
              validUntil: existingSO.validUntil ?? sq.validUntil ?? undefined,
              subject: existingSO.subject ?? sq.subject ?? undefined,
              paymentTerms:
                existingSO.paymentTerms ?? sq.paymentTerms ?? undefined,
              shippingTerms:
                existingSO.shippingTerms ?? sq.shippingTerms ?? undefined,
              termsConditions:
                existingSO.termsConditions ?? sq.termsConditions ?? undefined,
            },
          });
        }
        linked++;
    ...
    if (!targetStatus) {
      skipped++;
      console.log(`  [SKIP] ${sq.quotationNumber} — unknown status "${sqStatus}"`);
      continue;
    }
    ...
    if (APPLY) {
      const so = await prisma.salesOrder.create({
        data: {
          ...
          legacyQuotationId: sq.id,
          quotationSentAt:
            sqStatus === "SENT" ? (sq.updatedAt ?? sq.createdAt) : undefined,
          priceStatus:
            targetStatus === "DRAFT"
              ? "PROVISIONAL"
              : sqStatus === "SENT"
