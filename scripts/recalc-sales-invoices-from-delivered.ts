...
async function calcSalesTotalFromDelivered(so: {
  ...
  for (const it of so.items) {
    ...
    const ppnRes = calculatePpn(afterDiscount, taxPct, it.ppnMode as PpnMode);
    totalGoods += ppnRes.total;
  }

  let shipping = 0;
  if (so.deliveryOrders.length > 0) {
    for (const d of so.deliveryOrders) shipping += d.totalCharge?.toNumber() ?? 0;
  } else {
    ...
    for (const inv of so.invoices) {
      ...
      if (Math.abs(delta) > 0.01) {
        ...
        if (apply && isFixable) {
          ...
          console.log(`  -> Updated invoice ${inv.invoiceNumber} to ${deliveredTotal}`);
          fixedCount++;
        }
      }
    }
