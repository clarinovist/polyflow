...
async function main() {
    ...
    for (const code of GHOST_CODES) {
        ...
        const lineCount = await tenantDb.journalLine.count({
    ...
    for (const c of candidates) {
        const marker = c.safe ? '🟢' : '🔴';
        console.log(
            `  ${marker} ${c.code.padEnd(8)} ${c.name.padEnd(30)} ${c.journalLineCount} journal lines`,
        );
    }
    ...
    if (!isApply) {
        ...
        if (unsafeCount > 0 && !force) {
            console.log(
                `   Use --force to deactivate accounts with journal lines.`,
            );
        }
        await tenantDb.$disconnect();
        await mainPrisma.$disconnect();
