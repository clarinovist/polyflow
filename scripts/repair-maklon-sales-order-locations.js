...
const DEFAULT_TARGET_DESCRIPTION =
    'Default customer-owned warehouse for Maklon Jasa sales orders';
...
async function main() {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (!targetLocation && options.dryRun) {
            ...
            targetLocation = await ensureTargetLocation(
                tx,
                options,
        ...
        console.log(
            `Found ${invalidOrders.length} Maklon Jasa sales orders with non-customer-owned source locations:`,
        );
        ...
        if (options.dryRun) {
            console.log(
                'Dry-run complete. Re-run with --execute --yes to update these orders.',
            );
            return;
        }
        ...
        for (const order of invalidOrders) {
            ...
            updatedCount += 1;
            console.log(
                `Updated ${order.orderNumber} -> ${targetLocation.name} (${targetLocation.slug})`,
            );
        }
