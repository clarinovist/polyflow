...
async function main() {
    ...
    const csvPath = path.join(
        __dirname,
        '../docs/data-import/melindo-initial/ar-opening-detail-from-sheets.csv',
    );
    ...
    if (!arAccount)
        arAccount = await prisma.account.findFirst({
            where: { code: { contains: '115' }, type: 'ASSET' },
        });
    if (!arAccount)
        throw new Error('AR Account not found! Searched: 11210, 1-115b');
    console.log(
        `AR Account: ${arAccount.code} - ${arAccount.name} (${arAccount.id})`,
    );
    ...
    for (const row of rows) {
        ...
        if (!customerId) {
            console.log(`  ✗ Unknown customer: ${customerName}`);
            errors++;
            continue;
        }
        if (existingSet.has(soNumber)) {
            skipped++;
            continue;
        }
    ...
    const totalAR = await prisma.invoice.aggregate({
        _sum: { totalAmount: true },
    });
    const jlStats = await prisma.journalLine.aggregate({
        where: {
            journalEntry: {
                description: { contains: 'Opening Balance AR' },
                status: JournalStatus.POSTED,
            },
        },
    ...
    console.log(
        `  Total AR:     Rp ${Number(totalAR._sum.totalAmount || 0).toLocaleString('id-ID')}`,
    );
