...
async function provisionTenant() {
    ...
    if (args.length < 3) {
        console.error(
            'Usage: npx tsx scripts/provision-tenant.ts <tenant_name> <subdomain> <db_url>',
        );
        console.log(
            "Example: npx tsx scripts/provision-tenant.ts 'PT ABC' pt-abc postgresql://user:pass@localhost:5432/pt_abc",
        );
        process.exit(1);
    }
    ...
    try {
        ...
        const { stdout, stderr } = await execPromise(
            `DATABASE_URL="${dbUrl}" npx prisma@5.22.0 migrate deploy`,
        );
        ...
        console.log(
            `   1. Seed COA: DATABASE_URL="${dbUrl}" npx tsx prisma/seed.ts`,
        );
        console.log(
            `   2. Seed roles: npx tsx scripts/seed-tenant-account-roles.ts`,
        );
    } catch (error) {
