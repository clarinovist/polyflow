...
async function main() {
    ...
    for (const acc of allAccounts) {
        ...
        if (Math.abs(net) > 0.01) {
            console.error(
                `❌ Ghost ${acc.code} ${acc.name} has non-zero net ${net}`,
            );
            fail++;
        }
    }
    ...
    const roles = await mainPrisma.tenantAccountRole.findMany({
        where: { tenantId: tenant.id },
    });
    const ghostRoles = roles.filter(
        (r) => r.accountCode && GHOST_CODES.has(r.accountCode),
    );
    if (ghostRoles.length > 0) {
        console.error(`❌ TenantAccountRole pointing to ghost:`);
        for (const r of ghostRoles)
            console.error(`  - ${r.role} -> ${r.accountCode} ${r.accountName}`);
        fail++;
    } else {
  ...
  `)) as any[];
    if (productsWithGhostInv.length > 0) {
        ...
        for (const p of productsWithGhostInv)
            console.error(`  - ${p.name} -> ${p.inv_code}`);
        fail++;
    } else {
