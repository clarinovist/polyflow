...
const SUPPLIER_MAP: Record<string, { code: string; name: string }> = {
    ...
    'INTERA LESTARI POLIMER, PT': {
        code: 'SUP-INTERA',
        name: 'PT INTERA LESTARI POLIMER',
    },
    'SOLO MULTIPACKING, PT': {
        code: 'SUP-SOLOMP',
        name: 'PT SOLO MULTIPACKING',
    },
    ...
    try {
        ...
        if (error.code === 'P2025') {
            ...
            const nextVal = latest
                ? parseInt(latest.entryNumber.split('-').pop()!) + 1
                : 1;
...
async function main() {
    ...
    const csvPath = path.join(
        __dirname,
        '../docs/data-import/melindo-initial/ap-opening-detail-from-sheets.csv',
    );
    ...
    let equityAccount = await prisma.account.findUnique({
        where: { code: EQUITY_ACCOUNT_CODE },
    });
    if (!equityAccount) {
        equityAccount = await prisma.account.create({
            data: {
                code: EQUITY_ACCOUNT_CODE,
    ...
    for (const row of rows) {
        ...
        if (!supplierId) {
            console.log(`  ✗ Unknown: ${supplierName}`);
            errors++;
            continue;
        }
        ...
        if (isNaN(amount) || amount <= 0) {
            console.log(`  ✗ Bad amount: ${invoiceNumber}`);
            errors++;
            continue;
        }
                ...
                async (tx) => {
                    ...
                    const pi = await tx.purchaseInvoice.create({
                        data: {
                            ...
                            termOfPaymentDays: 0,
                            notes:
                                row['keterangan'] || 'Opening Balance Transfer',
                        },
                    });
    ...
    const totalAP = await prisma.purchaseInvoice.aggregate({
        _sum: { totalAmount: true },
    });
    const jlStats = await prisma.journalLine.aggregate({
        where: {
            journalEntry: {
                description: { contains: 'Opening Balance AP' },
                status: JournalStatus.POSTED,
            },
        },
    ...
    console.log(
        `  Total AP:     Rp ${Number(totalAP._sum.totalAmount || 0).toLocaleString('id-ID')}`,
    );
    ...
    console.log(
        `  Balanced:     ${Math.abs(dr - cr) < 0.01 ? '✓' : '✗ diff ' + (dr - cr)}`,
    );
}
