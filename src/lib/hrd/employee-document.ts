...
export const EmployeeDocumentService = {
    ...
    async create(db: PrismaClient, data: CreateDocumentInput) {
        ...
        if (!data.fileUrl?.trim())
            throw new BusinessRuleError('File dokumen wajib diunggah');
    ...
    async archive(db: PrismaClient, documentId: string) {
        ...
        if (doc.status === 'ARCHIVED')
            throw new BusinessRuleError('Dokumen sudah diarsipkan');
    ...
    async restore(db: PrismaClient, documentId: string) {
        ...
        if (doc.status === 'ACTIVE')
            throw new BusinessRuleError('Dokumen sudah aktif');
        return db.employeeDocument.update({
            where: { id: documentId },
            data: { status: 'ACTIVE' },
        });
