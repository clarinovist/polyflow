...
export async function createSalaryHistory(
    ...
    return db.employeeSalaryHistory.create({
        data: {
            ...
            monthlySalary:
                after.monthlySalary != null
                    ? Number(after.monthlySalary)
                    : null,
            overtimeHourlyRate:
                after.overtimeHourlyRate != null
                    ? Number(after.overtimeHourlyRate)
                    : null,
            standardDayHours:
                after.standardDayHours != null
                    ? Number(after.standardDayHours)
                    : null,
            bpjsParticipant: (after.bpjsParticipant as boolean) ?? null,
            bpjsEmployeeDeduction:
                after.bpjsEmployeeDeduction != null
                    ? Number(after.bpjsEmployeeDeduction)
                    : null,
            bpjsEmployerCost:
                after.bpjsEmployerCost != null
                    ? Number(after.bpjsEmployerCost)
                    : null,
