/** Main sidebar navigation */
export const mainNavLabels = {
    sales: 'Penjualan',
    purchasing: 'Pembelian',
    production: 'Produksi',
    inventory: 'Stok',
    accounting: 'Finance',
    hrd: 'HRD',
    maklon: 'Maklon',

    // Master data
    productCatalog: 'Katalog Produk',
    boms: 'BOM / Formula',
    machines: 'Mesin (Master)',
    employees: 'Karyawan',

    // Maklon (legacy flat labels — kept for any remaining references)
    maklonReceipts: 'Penerimaan Maklon',
    maklonReturns: 'Retur Maklon',

    findings: 'Catatan CEO',

    // Support
    help: 'Bantuan',
} as const;

export const hrdSidebarLabels = {
    homeBoard: 'Hari Ini',
    dashboard: 'Dashboard HRD',
    attendance: 'Rekap Kehadiran',
    payrollWeekly: 'Gaji Mingguan (Borongan)',
    pieceRates: 'Tarif Borongan',
    payrollMonthly: 'Gaji Bulanan',
    bpjs: 'Rekap BPJS',
    loans: 'Kasbon Karyawan',
    leave: 'Cuti & Izin',
    disciplinary: 'Sanksi Disiplin',
    alerts: 'Alert HR',
    employees: 'Karyawan',
} as const;

export const maklonSidebarLabels = {
    dashboard: 'Dashboard Maklon',
    receipts: 'Penerimaan Maklon',
    newReceipt: 'Penerimaan Baru',
    returns: 'Retur Maklon',
    newReturn: 'Retur Baru',
    warehousePortal: 'Portal Gudang',
} as const;

/** Sidebar by domain */
export const productionSidebarLabels = {
    overview: 'Papan Produksi',
    // Production Planning
    workOrders: 'SPK',
    bom: 'BOM / Formula',
    incomingRequests: 'Permintaan FG',
    productionSchedule: 'Jadwal Produksi',
    materialRequirements: 'MRP',
    productionAnalytics: 'Analitik',
    // Production Floor
    dailyProduction: 'Board Proses',
    machineBoard: 'Status Mesin Saat Ini',
    floorStock: 'Stok Lantai',
    teamShifts: 'Tim',
    outputLogs: 'Log & Bukti',
    packingMonthlyReport: 'Packing',
    dailyReport: 'Harian / Mesin',
    outputReport: 'Produk / Operator',
    workShifts: 'Pengaturan Shift',
    operatorKiosk: 'Kiosk Operator',
    costingDashboard: 'Costing',
} as const;

export const warehouseSidebarLabels = {
    homeBoard: 'Hari Ini',
    jobQueue: 'Antrian Tugas',
    incomingReceipts: 'Penerimaan Barang',
    outgoingOrders: 'Antrian Muat',
    materialProduction: 'Bahan Produksi',
    stockOpname: 'Stock Opname',
    stockOverview: 'Stok',
    stockTransfer: 'Transfer Stok',
    stockAdjustment: 'Penyesuaian Stok',
    locations: 'Lokasi',
    analyticsDashboard: 'Analitik Gudang',
    stockAging: 'Aging Stok',
    stockMovement: 'Mutasi Stok',
    transferAnalytics: 'Analitik Transfer',
    adjustmentAnalytics: 'Analitik Penyesuaian',
} as const;

export const financeSidebarLabels = {
    dashboard: 'Papan Keuangan',
    pettyCash: 'Petty Cash',
    pettyCashDailyReport: 'Laporan Harian',
    pettyCashCashOpname: 'Cash Opname',
    pettyCashRekap: 'Rekap Kas',
    bankReconciliation: 'Rekonsiliasi Bank',
    fixedAssets: 'Aset Tetap',
    budgetPlanning: 'Anggaran',
    budgeting: 'Anggaran',
    budgetInput: 'Input Anggaran',
    aging: 'Aging',
    fohAllocation: 'Alokasi Biaya Pabrik',
    costingDashboard: 'Costing Dashboard',
    hppReport: 'Laporan HPP',
    quickEntry: 'Quick Entry',
    receivables: 'Invoice Sales',
    salesReturns: 'Retur Penjualan',
    customerPayments: 'Terima Bayar',
    payables: 'Invoice Purchase',
    supplierPayments: 'Bayar Supplier',
    rekapHutang: 'Rekap Hutang Dagang',
    rekapPiutang: 'Rekap Piutang Dagang',
    rekapPiutangKaryawan: 'Rekap Piutang Karyawan',
    journalEntries: 'Jurnal',
    balanceSheet: 'Neraca',
    incomeStatement: 'Laba Rugi',
    cashFlowStatement: 'Arus Kas',
    trialBalance: 'Neraca Saldo',
    budgetVariance: 'Varians Anggaran',
    taxReport: 'Laporan Pajak',
    maklonProfitability: 'Profitabilitas Maklon',
    generalLedger: 'Buku Besar',
    chartOfAccounts: 'Bagan Akun (COA)',
    fiscalPeriods: 'Periode Fiskal',
    openingBalance: 'Saldo Awal',
    paymentBanks: 'Bank Pembayaran',
    reportsHub: 'Laporan',
    revenueRules: 'Revenue Rules',
} as const;

export const salesSidebarLabels = {
    salesDashboard: 'Papan Sales',
    salesOrders: 'Sales Order',
    salesInvoices: 'Invoice & Piutang',
    deliveryTracking: 'Surat Jalan',
    deliverySchedules: 'Jadwal Kirim',
    vehicles: 'Armada',
    shippingTariffs: 'Tarif Pengiriman',
    shippingCostReport: 'Biaya Pengiriman',
    salesPerformance: 'Performa Penjualan',
    salesReturns: 'Retur Penjualan',
    customerManagement: 'Customer',
    salesTeam: 'Daftar Sales',
    mobileMode: 'Mode Mobile',
    dailyRoutes: 'Rute Harian',
    quotations: 'Penawaran',
    visits: 'Kunjungan',
    prospects: 'Prospek',
    targets: 'Target Sales',
    commissionReport: 'Laporan Komisi',
    priceList: 'Price List',
    collection: 'Penagihan',
} as const;
export const purchasingSidebarLabels = {
    homeBoard: 'Papan Pembelian',
    purchasingDashboard: 'Dashboard Pembelian',
    purchaseRequests: 'Permintaan (PR)',
    purchaseOrders: 'Order Pembelian (PO)',
    purchaseReturns: 'Retur',
    supplierManagement: 'Supplier',
    procurementAnalytics: 'Analitik Pembelian',
} as const;
