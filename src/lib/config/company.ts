...
export function getCompanyConfig(): CompanyConfig {
    return {
        ...
        paperSize: {
            widthCm:
                parseFloat(process.env.COMPANY_PAPER_WIDTH_CM || '') || 24.13,
            heightCm:
                parseFloat(process.env.COMPANY_PAPER_HEIGHT_CM || '') || 13.97,
            marginMm:
                parseFloat(process.env.COMPANY_PAPER_MARGIN_MM || '') || 5,
        },
    };
}
