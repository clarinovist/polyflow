...
import type { HelpConversation, HelpMessage, HelpConversationStatus } from '@prisma/client';
...
export async function validateConversationOwnership(input: {
  ...
  return conversation.tenantId === input.tenantId && conversation.userId === input.userId;
}
...
export async function loadConversationContext(
  ...
  const resolvedEntities = new Map<string, { type: string; id: string; label: string }>();
  for (const msg of messages) {
    if (msg.evidenceJson && typeof msg.evidenceJson === 'object') {
      const evidence = msg.evidenceJson as { entities?: Array<{ type: string; id: string; label: string }> };
