...
type SoRow = { id: string; orderNumber: string; customer: string | null; status: string };
...
type OverviewStockRow = { product: string; location: string; quantity: Prisma.Decimal };
...
type CriticalStockRow = { product: string; qty: Prisma.Decimal; threshold: Prisma.Decimal };
...
type PendingSalesRow = { orderNumber: string; customer: string | null; total: Prisma.Decimal };
...
async function executeGetPendingSalesOverview(
  ...
  if (!rows.length) {
    return createEvidence({
      summary: 'Tidak ada sales order pending. Semua pesanan terpantau aman.',
      facts: [],
      source: 'tenant-data',
      completeness: 'partial',
    });
...
export const toolRegistry: AssistantToolDefinition[] = [
  ...
  },
  {
    ...
    sensitivity: 'normal',
    inputSchema: z.object({
      searchTerm: z.string().min(1, 'Nomor order atau nama customer harus diisi'),
    }),
    execute: (args, ctx) =>
  ...
  },
  {
    ...
    description: 'Ranking 12 stok terbanyak saat ini di gudang secara umum.',
    requiredResources: ['/warehouse/inventory'],
    sensitivity: 'normal',
    inputSchema: z.object({}),
    execute: (args, ctx) =>
  ...
  },
  {
    ...
    description: 'Daftar produk yang stoknya kurang dari nilai ambang batas minimum.',
    requiredResources: ['/warehouse/inventory'],
    sensitivity: 'normal',
    inputSchema: z.object({}),
    execute: (args, ctx) =>
  ...
  },
  {
    ...
    description: 'Daftar ringkas sales orders yang statusnya belum selesai.',
    requiredResources: ['/sales/orders'],
    sensitivity: 'normal',
    inputSchema: z.object({}),
    execute: (args, ctx) =>
  ...
  },
  {
    ...
    description: 'Cari artikel panduan / FAQ / troubleshooting dari Knowledge Base. WAJIB gunakan tool ini untuk pertanyaan cara pakai, bagaimana cara, tutorial, atau troubleshooting sebelum menjawab sendiri.',
    requiredResources: [],
    sensitivity: 'normal',
    inputSchema: z.object({
    ...
    execute: (args, ctx) =>
      executeSearchHelpArticles(args as { query: string; module?: string }, ctx),
  },
  ...
  // get_delivery_status
  {
    ...
    description: 'Cek status pengiriman, jadwal DO/SJ, atau apakah pesanan sudah dikirim.',
    ...
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      ...
      const facts = rows.map((r: { orderNumber: string; status: string; customer: string | null; deliveryDate: Date | null }) => ({
        label: r.orderNumber,
        value: `${r.status} — ${r.customer || '-'} — ${r.deliveryDate ? new Date(r.deliveryDate).toLocaleDateString('id-ID') : 'Belum dijadwalkan'}`,
      }));

      const entities = rows.map((r: { id: string; orderNumber: string }) => ({
        type: 'DeliveryOrder',
        id: r.id,
        label: r.orderNumber,
  ...
  // get_invoice_status
  {
    ...
    description: 'Cek status invoice penjualan, apakah sudah lunas, partial, atau overdue.',
    requiredResources: ['/finance/invoices/sales'],
    sensitivity: 'financial',
    inputSchema: z.object({
      searchTerm: z.string().min(1, 'Nomor invoice atau nama customer harus diisi'),
    }),
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      const { searchTerm } = args as { searchTerm: string };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const facts = rows.map((r: { invoiceNumber: string; status: string; totalAmount: Prisma.Decimal; paidAmount: Prisma.Decimal; customer: string | null; dueDate: Date | null }) => {
      ...
      const entities = rows.map((r: { id: string; invoiceNumber: string }) => ({
        type: 'Invoice',
        id: r.id,
        label: r.invoiceNumber,
  ...
  // get_purchase_order
  {
    ...
    description: 'Cek status PO, apakah sudah diterima, partial, atau outstanding.',
    requiredResources: ['/purchasing/orders'],
    sensitivity: 'normal',
    inputSchema: z.object({
      searchTerm: z.string().min(1, 'Nomor PO atau nama supplier harus diisi'),
    }),
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      const { searchTerm } = args as { searchTerm: string };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const facts = rows.map((r: { orderNumber: string; status: string; totalAmount: Prisma.Decimal; supplier: string | null; orderDate: Date | null }) => ({
        label: r.orderNumber,
  ...
  // get_stock_movements
  {
    ...
    description: 'Cek riwayat pergerakan stok (masuk/keluar) untuk produk tertentu.',
    ...
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      ...
      const facts = rows.map((r: { type: string; quantity: Prisma.Decimal; createdAt: Date; reference: string | null; location: string | null }) => ({
  ...
  // diagnose_so_fulfillment
  {
    ...
    description: 'Diagnosa mengapa Sales Order tidak dapat diproses: cek stok, reservasi, produksi, dan jadwal kirim. Gunakan untuk pertanyaan "kenapa pesanan belum bisa dikirim".',
    requiredResources: ['/sales/orders', '/warehouse/inventory'],
    sensitivity: 'normal',
    inputSchema: z.object({
      searchTerm: z.string().min(1, 'Nomor SO atau nama customer harus diisi'),
    }),
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      const { searchTerm } = args as { searchTerm: string };
      const facts: { label: string; value: string }[] = [];
      ...
      facts.push({ label: 'Status SO', value: `${order.orderNumber} [${order.status}] — ${order.customer || 'Guest'}` });
      ...
      for (const item of items) {
        ...
        const status = available >= requested ? '✅ Cukup' : `❌ Kurang ${requested - available}`;
        facts.push({ label: `Item: ${item.variant}`, value: `Diminta: ${requested} — Tersedia: ${available} — ${status}` });
      ...
      if (productionOrders.length > 0) {
        for (const po of productionOrders) {
          facts.push({ label: `Produksi: ${po.orderNumber}`, value: `Status: ${po.status}` });
          entities.push({ type: 'ProductionOrder', id: po.orderNumber, label: po.orderNumber, href: '/production/orders' });
        }
      } else {
        facts.push({ label: 'Produksi', value: 'Tidak ada SPK terkait' });
      }
      ...
      if (deliveries.length > 0) {
        for (const d of deliveries) {
          facts.push({ label: `Pengiriman: ${d.orderNumber}`, value: `Status: ${d.status}` });
        }
      } else {
        facts.push({ label: 'Pengiriman', value: 'Belum ada jadwal pengiriman' });
      }
  ...
  // diagnose_stock_discrepancy
  {
    ...
    description: 'Diagnosa mengapa stok dianggap kurang padahal ada: cek reservasi, pending SO, dan pergerakan stok.',
    ...
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      ...
      facts.push({ label: 'Stok Terpesan (SO Pending)', value: `${reserved.toFixed(2)}` });

      const available = totalStock - reserved;
      facts.push({ label: 'Stok Tersedia', value: `${available.toFixed(2)} ${available < 0 ? '⚠️ NEGATIF' : ''}` });

      if (reserved > 0) {
        facts.push({ label: 'Penjelasan', value: `Stok ${totalStock.toFixed(2)} sudah dikurangi ${reserved.toFixed(2)} untuk pesanan lain. Sisa tersedia: ${available.toFixed(2)}` });
      }
  ...
  // get_attendance_summary
  {
    ...
    description: 'Cek ringkasan kehadiran karyawan: hadir, terlambat, tidak hadir. Hanya untuk data diri sendiri atau karyawan yang diizinkan.',
    ...
    execute: async (args, _ctx): Promise<ToolEvidence> => {
      const { employeeName, startDate, endDate } = args as { employeeName?: string; startDate?: string; endDate?: string };
      ...
      const facts = rows.map((r: { employee: string; date: Date; status: string; checkInTime: string | null }) => ({
 ...
 */
export function getToolsForContext(context: AssistantUserContext): AssistantToolDefinition[] {
 ...
 */
export function getToolByName(name: string): AssistantToolDefinition | undefined {
...
export function toolsToOpenAiFormat(tools: AssistantToolDefinition[]): Array<{
  ...
  return tools.map((tool) => ({
    ...
    function: {
      ...
      parameters: tool.inputSchema instanceof z.ZodObject
 ...
 */
function zodSchemaToOpenAi(schema: z.ZodObject<z.ZodRawShape>): Record<string, unknown> {
