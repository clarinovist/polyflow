...
export async function logVirtualCsEvent(
    input: VirtualCsAuditInput,
): Promise<string | null> {
    ...
    if (level === 'error') logger.error('Virtual CS request failed', context);
        ...
        logger.warn('Virtual CS request blocked by guardrails', context);
    ...
    try {
        await logActivity({
            ...
            action: input.allowed
                ? 'VIRTUAL_CS_QUERY_ALLOWED'
                : 'VIRTUAL_CS_QUERY_BLOCKED',
