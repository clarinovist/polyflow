...
import { getToolsForContext, toolsToOpenAiFormat, getToolByName } from './tool-registry';
...
export async function generateVirtualCsReply(
  ...
  if (context?.tenantId && context?.sessionUser) {
    assistantCtx = buildAssistantContext(context.sessionUser, context.tenantId);
  }
  ...
  const availableTools = assistantCtx
    ? getToolsForContext(assistantCtx)
    : [];
...
Di akhir jawaban, tawarkan bantuan atau pertanyaan lanjutan yang relevan secara ramah.`
  ...
  try {
    ...
    for (let loop = 0; loop < 4; loop++) {
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        for (const toolCall of responseMessage.tool_calls) {
          ...
          if (AGENTIC_DEBUG) {
            console.debug(`[AGENTIC] Calling tool: ${toolName} with args:`, rawArgs);
          }
          ...
          if (assistantCtx) {
            const authResult = checkToolAuthorization(toolDef, assistantCtx);
            if (!authResult.allowed) {
              if (AGENTIC_DEBUG) {
                console.debug(`[AGENTIC] Tool ${toolName} DENIED: ${authResult.reason}`);
              }
              messages.push({
                role: 'tool',
                tool_call_id: toolCall.id,
            ...
            if (toolName === 'search_help_articles') {
              ...
              for (const entity of evidence.entities || []) {
                if (entity.type === 'HelpArticle' && entity.href) {
                  ...
                  if (!collectedCited.some((c) => c.slug === slug)) {
                    collectedCited.push({
                      slug,
                      title: entity.label,
                      summary: evidence.facts.find((f) => f.label === entity.label)?.value,
                    });
                  }
                }
              }
    ...
    const evidenceChips = collectedEvidence.map((e) => ({
      source: e.source,
      label:
        e.source === 'tenant-data' ? `Data tenant — dicek ${formatTimeWib(e.checkedAt)}` :
    ...
    if (activeConversationId) {
      const answerText = finalAnswer || 'Maaf, saya belum dapat merangkum analisis pada saat ini.';
      saveMessage({
        conversationId: activeConversationId,
        role: 'USER',
      ...
      saveMessage({
        ...
        role: 'ASSISTANT',
        content: answerText,
        evidenceJson: { entities: collectedEvidence.flatMap((e) => e.entities || []) },
      }).catch(() => { /* non-blocking */ });
    }
