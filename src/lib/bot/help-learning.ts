            ...
            messages: [
                {
                    role: 'system',
                    content:
                        'Kamu adalah technical writer ERP. Output JSON saja, no extra prose.',
                },
...
export async function runLearningDraftJob(
    ...
    try {
        const row = await mainDb.helpSettings.findUnique({
            where: { key: 'autoLearnDraftEnabled' },
        });
        if (row) {
            const enabled = JSON.parse(row.valueJson);
            if (enabled === false) {
                result.errors.push(
                    'autoLearnDraftEnabled is OFF — job skipped',
                );
                return result;
            }
        }
    } catch {
        /* ignore — run anyway if no setting */
    }

    // Find minClusterSize from settings if exists
    try {
        const row = await mainDb.helpSettings.findUnique({
            where: { key: 'minClusterSizeForDraft' },
        });
        if (row) {
    ...
    const candidates = clusters
        .filter((c) => !draftClusterIds.has(c.id))
        .slice(0, maxDrafts);
        ...
        try {
            ...
            await mainDb.helpLearningDraft.create({
                data: {
                    ...
                    modules: cluster.suggestedModule
            ...
            result.errors.push(
                `${cluster.id}: ${e instanceof Error ? e.message : String(e)}`,
            );
            result.skipped++;
        }
    }
