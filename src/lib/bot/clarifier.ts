...
const AMBIGUOUS_PATTERNS = [
  { pattern: /^(cek|lihat|tampil(?:kan)?)\s+(stok|barang|pesanan|order|invoice|spk|po)\s*$/i, category: 'no-entity' },
  { pattern: /\b(berapa|ada berapa|sisa)\b.*\b(stok|barang|qty|kuantitas)\b.*$/i, category: 'which-location' },
  { pattern: /\b(budi|andi|siti|ahmad|agus)\b/i, category: 'ambiguous-name' },
  { pattern: /^(itu|yang\s+tadi|yang\s+sebelumnya|yang\s+ini)$/i, category: 'pronoun-reference' },
  { pattern: /^(kenapa|kok|gimana|gimana\s+cara)\s*$/i, category: 'incomplete-question' },
  { pattern: /\b(error|gagal|tidak\s+bisa|nggak\s+bisa)\b(?!\s+(karena|dikarenakan))/i, category: 'no-context' },
];
...
export function analyzeForClarification(
  ...
  const hasPronoun = /^(itu|yang\s+tadi|yang\s+sebelumnya|kenapa\s+belum|kenapa\s+kok|kok\s+belum)/i.test(q);
  if (hasPronoun && history.length === 0) {
    return {
      needsClarification: true,
      category: 'pronoun-reference',
      suggestions: ['Maksudnya yang mana? Bisa sebutkan nomor atau nama?'],
      clarificationPrompt: 'Bisa sebutkan nomor atau nama yang Anda maksud?',
    };
  }
  ...
  const noResults = toolResults.every((r) => r.completeness === 'partial' && r.facts.length === 0);
  if (noResults && q.length < 30) {
    return {
      ...
      clarificationPrompt: 'Data tidak ditemukan. Bisa sebutkan nomor atau nama yang lebih spesifik?',
    };
  }
...
function getSuggestionsForCategory(category: string): string[] {
  switch (category) {
    case 'no-entity':
      return ['Sebutkan nama barang atau nomor transaksi', 'Sebutkan lokasi gudang jika ingin cek stok'];
    case 'which-location':
      return LOCATION_CLARIFICATIONS;
    case 'ambiguous-name':
      return ['Sebutkan nomor transaksi', 'Sebutkan context: customer, order, atau produk?'];
    case 'pronoun-reference':
      return ['Sebutkan nomor atau nama yang Anda maksud'];
    case 'incomplete-question':
      return ['Jelaskan masalah yang Anda hadapi', 'Sebutkan modul atau menu terkait'];
    case 'no-context':
...
export function resolvePronouns(
  ...
  const pronounPattern = /^(itu|yang\s+tadi|yang\s+sebelumnya|kenapa\s+belum|kenapa\s+kok|kok\s+belum)\s*(.*)/i;
