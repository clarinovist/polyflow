...
function normalizeQuestion(q: string): string {
    return q
        ...
        .replace(/[^\w\s]/g, ' ')
        .replace(
            /\b(cara|gimana|bagaimana|kenapa|apa|tolong|minta|bisa|tidak|nggak|gak)\b/gi,
            ' ',
        )
        .replace(/\s+/g, ' ')
        .trim()
...
function suggestModule(question: string): string | null {
    ...
    if (
        ...
        return 'sales';
    if (
        /(stok|stock|gudang|warehouse|inventory|lokasi|rak|opname|incoming|outgoing|terima barang)/.test(
            lower,
        )
    )
        return 'warehouse';
    if (
        /(spk|produksi|production|bom\b|kiosk|mesin|batch|backflush|bahan baku)/.test(
            lower,
        )
    )
        return 'production';
    if (
        /(finance|keuangan|jurnal|journal|period|tutup buku|hutang|piutang|petty cash)/.test(
            lower,
        )
    )
        return 'finance';
    if (/(role|permission|akses|access|user|menu tidak muncul)/.test(lower))
        return 'access';
    if (/(hrd|karyawan|employee|cuti|gaji|sanksi)/.test(lower)) return 'hrd';
    if (/(purchasing|po\b|supplier|pembelian|walk-in)/.test(lower))
        return 'purchasing';
    return 'global';
}
