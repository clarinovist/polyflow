 ...
 */
const SENSITIVITY_POLICY: Record<ToolSensitivity, { requiredResources?: string[]; requiredFeatures?: string[] }> = {
  normal: {},
  financial: {
 ...
 */
export function filterAuthorizedTools<T extends { name: string; requiredResources: string[]; sensitivity: ToolSensitivity; requiredFeatures?: string[] }>(
  tools: T[],
  context: AssistantUserContext,
): T[] {
 ...
 */
export function deniedEvidence(toolName: string): { text: string; allowed: false; reason: string } {
  return {
