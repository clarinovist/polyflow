...
export function getHelpLinkForError(
    code: string,
 ...
 */
export function getHelpLinkForMessage(
    message: string,
): { href: string; title: string } | null {
    ...
    if (
        lower.includes('stok') &&
        (lower.includes('tidak cukup') ||
            lower.includes('kurang') ||
            lower.includes('insufficient'))
    ) {
        return {
            href: '/support/cara-confirm-so-stok-kurang',
            title: 'Stok kurang? Lihat panduan',
        };
    }

    if (
        lower.includes('backflush') ||
        (lower.includes('bahan') && lower.includes('kurang'))
    ) {
        return {
            href: '/support/error-backflush-atau-stok-bahan',
            title: 'Backflush gagal? Lihat panduan',
        };
    }
    ...
    if (lower.includes('period') && lower.includes('lock')) {
        return {
            href: '/support/error-period-locked-finance',
            title: 'Period locked? Lihat panduan',
        };
    }
    ...
    if (
        ...
        return {
            href: '/support/menu-tidak-muncul-permission',
            title: 'Akses ditolak? Lihat panduan',
        };
    }
