...
function getSecret(): string {
    return (
        ...
        'polyflow-employee-secret-change-me'
    );
}
...
export async function createEmployeeSessionToken(
...
export const requireEmployeeSession = withTenantPage(
...
export const getCurrentEmployeeFull = withTenantPage(
    async function getCurrentEmployeeFull() {
        const session = await getEmployeeSession();
        if (!session) return null;
        const emp = await prisma.employee.findUnique({
            where: { id: session.employeeId },
