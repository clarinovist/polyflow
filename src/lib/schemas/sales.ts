...
export const createSalesOrderSchema = z
    ...
    .superRefine((data, ctx) => {
        ...
        if (data.intent === 'order') {
            if (
                data.orderType === SalesOrderType.MAKE_TO_STOCK &&
                !data.customerId
            ) {
                ctx.addIssue({
            ...
            if (
                ...
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    message:
                        'Customer is required for Make to Order / Maklon Jasa',
                    path: ['customerId'],
                });
            }
        }
    ...
    items: z
        .array(
            z.object({
                ...
                verifiedQuantity: z.coerce
                    .number()
                    .min(0, 'Qty verifikasi tidak boleh negatif'),
            }),
        )
        .min(1),
});
...
export const createVehicleTariffSchema = z.object({
    ...
    chargeRate: z.coerce
        .number()
        .min(0, 'Biaya ke customer tidak boleh negatif'),
