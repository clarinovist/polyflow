...
const consumptionRuleSchema = z.enum([
    'PROPORTIONAL',
    'FLOOR_ENTERED_BAL',
    'CEIL_ENTERED_BAL',
]);
...
export const productVariantSchema = z.object({
    ...
    buyPrice: z.coerce
        .number()
        .nonnegative('Buy price must be non-negative')
        .optional()
        .nullable(),
    minStockAlert: z.coerce
        .number()
        .nonnegative('Min stock alert must be non-negative')
        .optional()
        .nullable(),
...
export const createProductSchema = z
    ...
    .superRefine((data, ctx) => {
        ...
        const duplicates = skuCodes.filter(
        ...
        if (
            data.productType === ProductType.SCRAP ||
            data.productType === ProductType.RAW_MATERIAL
        ) {
