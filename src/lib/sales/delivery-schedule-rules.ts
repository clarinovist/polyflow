 ...
 */
export function canActivateSchedule(tripCount: number): {
    ok: boolean;
    warning?: string;
} {
    if (tripCount === 0) {
        return {
            ok: true,
            warning:
                'Jadwal belum memiliki trip. Disarankan minimal 1 trip sebelum mengaktifkan.',
        };
    }
    return { ok: true };
}
 ...
 */
export function canCloseSchedule(trips: Array<{ status: TripStatus }>): {
    ok: boolean;
    error?: string;
} {
...
export function validateDepartureInWeek(
    ...
    if (!departureDate) {
        return {
            ok: false,
            error: 'Tanggal berangkat wajib diisi sebelum konfirmasi.',
        };
    }
...
export function isDOAlreadyAssigned(
    ...
    return existingStops.some(
        (s) =>
            s.deliveryOrderId === deliveryOrderId && s.status !== 'CANCELLED',
    );
}
 ...
 */
export function canRemoveStop(stopStatus: ScheduleStopStatus): {
    ...
    return {
        action: 'cancel',
        warning:
            'Stop ini terkait Surat Jalan. Stop akan dibatalkan, tapi SJ tetap ada.',
    };
}
