...
export function generateStockCSVTemplate(): string {
    const headers = ['sku_code', 'location', 'quantity'];
        ...
        Papa.parse(file, {
            ...
            complete: (results) => {
                if (results.errors.length > 0) {
                    ...
                    const validRows = (results.data as unknown[]).filter(
                        (row: unknown) => {
 ...
 */
export function generateStockErrorReport(
    results: Array<{
        row: number;
 ...
 */
export function downloadStockErrorReport(
    results: Array<{
        row: number;
        errors: Array<{ field: string; message: string }>;
    }>,
