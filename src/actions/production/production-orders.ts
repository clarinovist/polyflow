...
export const createProductionOrder = withTenant(
    async function createProductionOrder(
        data: CreateProductionOrderValues & { createPath?: string },
    ) {
        return safeAction(async () => {
        ...
        return safeAction(async () => {
            try {
                ...
                if (!bomId || plannedQuantity <= 0 || !machineId) {
                    throw new BusinessRuleError(
                        'Produk, jumlah, dan mesin wajib diisi',
                    );
                }
                ...
                let locationId = resolveOutputLocationId(
                    allLocations,
                    stage,
                    false,
                );
                ...
                if (!locationId || isRiskyOutputLocation(targetLoc)) {
                    const activeNonRisky = allLocations.find(
                        (l) =>
                            !isInactiveLocation(l) && !isRiskyOutputLocation(l),
                    );
    ...
    async function splitProductionOrders(data: SplitProductionOrdersValues) {
        return safeAction(async () => {
            ...
            try {
                ...
                const createdOrders =
                    await ProductionService.splitOrdersFromSales({
                        ...result.data,
                        userId: session?.user?.id,
                    });
