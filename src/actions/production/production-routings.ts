...
export const listProcesses = withTenant(async function listProcesses(filter?: { isActive?: boolean; search?: string }) {
...
export const createProcess = withTenant(async function createProcess(raw: CreateProductionProcessValues) {
  return safeAction(async () => {
...
export const updateProcess = withTenant(async function updateProcess(raw: UpdateProductionProcessValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const deleteProcess = withTenant(async function deleteProcess(id: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const listMachineCapabilities = withTenant(async function listMachineCapabilities(machineId?: string, processId?: string) {
  return safeAction(async () => {
    await requirePlanningRole();
    const data = await ProductionRoutingService.listMachineCapabilities(machineId, processId);
    return serializeData(data);
  });
});
...
export const createRoute = withTenant(async function createRoute(raw: CreateRouteValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
...
export const updateRoute = withTenant(async function updateRoute(raw: UpdateRouteValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    const session = await requirePlanningRole();
    const parsed = updateRouteSchema.parse(raw);
    const { id, ...rest } = parsed;
    const result = await ProductionRoutingService.updateRoute(id, rest, (session as { user?: { id?: string } })?.user?.id);
    revalidatePath('/production/routings');
...
export const duplicateRoute = withTenant(async function duplicateRoute(id: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    const session = await requirePlanningRole();
    const result = await ProductionRoutingService.duplicateRoute(id, (session as { user?: { id?: string } })?.user?.id);
    revalidatePath('/production/routings');
    return serializeData(result);
  });
});
...
export const validateRouteAction = withTenant(async function validateRouteAction(routeId: string) {
  return safeAction(async () => {
    await requirePlanningRole();
    const result = await ProductionRoutingService.validateRoute(routeId);
    return serializeData(result);
  });
});

export const publishRoute = withTenant(async function publishRoute(routeId: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    const session = await requirePlanningRole();
    const result = await ProductionRoutingService.publishRoute(routeId, (session as { user?: { id?: string } })?.user?.id);
    revalidatePath('/production/routings');
...
export const archiveRoute = withTenant(async function archiveRoute(routeId: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const setDefaultRoute = withTenant(async function setDefaultRoute(routeId: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const addRouteStep = withTenant(async function addRouteStep(raw: CreateRouteStepValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const updateRouteStep = withTenant(async function updateRouteStep(raw: UpdateRouteStepValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
    const parsed = updateRouteStepSchema.parse(raw);
    const { id, ...rest } = parsed;
    const result = await ProductionRoutingService.updateRouteStep(id, rest as never);
    revalidatePath('/production/routings');
    return serializeData(result);
  });
});

export const deleteRouteStep = withTenant(async function deleteRouteStep(id: string) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
...
export const reorderRouteSteps = withTenant(async function reorderRouteSteps(raw: ReorderRouteStepsValues) {
  return safeAction(async () => {
    await assertRoutingEnabled();
    await requirePlanningRole();
    const parsed = reorderRouteStepsSchema.parse(raw);
    const result = await ProductionRoutingService.reorderSteps(parsed.routeId, parsed.orderedIds);
    revalidatePath('/production/routings');
    revalidatePath(`/production/routings/${parsed.routeId}`);
    return serializeData(result);
  });
