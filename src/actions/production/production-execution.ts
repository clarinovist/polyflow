        ...
        return safeAction(async () => {
            ...
            try {
                ...
                if (
                    ...
                    if (
                        errMsg.includes(
                            'ProductionExecution_productionOrderId_fkey',
                        )
                    ) {
                        throw new BusinessRuleError(
                    ...
                    if (
                        errMsg.includes('ProductionExecution_operatorId_fkey')
                    ) {
                        throw new BusinessRuleError(
    ...
    async function getActiveExecutions() {
        return safeAction(async () => {
            try {
                ...
                const executions =
                    await ProductionService.getActiveExecutions();
    ...
    async function getProductionHistory(filter?: ProductionHistoryFilter) {
        return safeAction(async () => {
            ...
            if (filter?.from || filter?.to) {
                const startBound = filter.from
                    ? getWibDayBoundsFromStr(filter.from).start
                    : getWibDayBoundsFromStr('2020-01-01').start;
                const endBound = filter.to
                    ? getWibDayBoundsFromStr(filter.to).end
                    : new Date();
            ...
            if (filter?.q) {
                ...
                filtered = completions.filter((exec) => {
                    const orderNum =
                        exec.productionOrder.orderNumber.toLowerCase();
                    const prodName =
                        exec.productionOrder.bom.productVariant.name.toLowerCase();
            ...
            for (const exec of filtered) {
                ...
                group.executions.push({
                    ...
                    status: exec.status,
                    enteredQuantity: exec.enteredQuantity
                    ...
                    shift: exec.shift
                        ? {
                              shiftName: exec.shift.shiftName,
                              operator: exec.shift.operator
                                  ? { name: exec.shift.operator.name }
                                  : null,
                          }
                        : null,
                ...
                if (exec.endTime) {
                    if (
                        !group.latestEndTime ||
                        exec.endTime > group.latestEndTime
                    ) {
                        group.latestEndTime = exec.endTime;
                    }
                    if (
                        !group.earliestEndTime ||
                        exec.endTime < group.earliestEndTime
                    ) {
                ...
                if (
                    exec.machine?.code &&
                    !group.machineCodes.includes(exec.machine.code)
                ) {
                    group.machineCodes.push(exec.machine.code);
                }
                const execOperatorName =
                    exec.shift?.operator?.name || exec.operator?.name;
                if (
                    execOperatorName &&
                    !group.operatorNames.includes(execOperatorName)
                ) {
                    group.operatorNames.push(execOperatorName);
                }
                if (
                    exec.shift?.shiftName &&
                    !group.shiftNames.includes(exec.shift.shiftName)
                ) {
            ...
            const summary: ProductionHistorySummary = {
                ...
                executionCount: groups.reduce(
                    (sum, g) => sum + g.executions.length,
                    0,
                ),
                orderCount: groups.length,
                missingPhotoCount: groups.filter((g) => g.photoCount === 0)
                    .length,
                limit,
    ...
    async function voidProductionOutput(
        ...
        return safeAction(async () => {
            try {
                ...
                await ProductionService.voidExecution(
                    executionId,
                    session.user.id,
                );
    ...
    async function getOperatorTodaySummary(operatorId: string) {
        return safeAction(async () => {
            ...
            const uniqueOrders = new Set(
                executions.map((e) => e.productionOrderId),
            );
            const jobCount = uniqueOrders.size;
            const goodQty = executions.reduce(
