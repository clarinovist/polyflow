...
export const createAccount = withTenant(async function createAccount(data: {
    code: string;
    name: string;
    type: AccountType;
...
export const getTrialBalance = withTenant(async function getTrialBalance(
    startDate?: Date,
    endDate?: Date,
) {
    return safeAction(async () => {
...
export const getIncomeStatement = withTenant(async function getIncomeStatement(
    startDate: Date,
    endDate: Date,
) {
    return safeAction(async () => {
...
export const getBalanceSheet = withTenant(async function getBalanceSheet(
    asOfDate: Date,
) {
    return safeAction(async () => {
        await requireFinanceAccess();
...
export const getGeneralLedger = withTenant(async function getGeneralLedger(
    startDate?: Date,
    endDate?: Date,
) {
    return safeAction(async () => {
...
export const createFiscalPeriod = withTenant(async function createFiscalPeriod(
    year: number,
    month: number,
) {
    return safeAction(async () => {
...
export const closeFiscalPeriod = withTenant(async function closeFiscalPeriod(
    id: string,
) {
    return safeAction(async () => {
...
export const getBudgets = withTenant(async function getBudgets(
    year: number,
    month: number,
) {
    return safeAction(async () => {
...
export const setBudget = withTenant(async function setBudget(data: {
    accountId: string;
    year: number;
    month: number;
    amount: number;
...
export const getBudgetVariance = withTenant(async function getBudgetVariance(
    year: number,
    month: number,
) {
    return safeAction(async () => {
...
export const getAccountingDashboardData = withTenant(
    async function getAccountingDashboardData() {
        return safeAction(async () => {
            ...
            const performance = await AccountingService.getIncomeStatement(
                startOfMonth,
                now,
            );
            ...
            const cashAccounts = coa.filter(
                (a: { isCashAccount: boolean }) => a.isCashAccount,
            );
            // Filter by codes seeded in seed-coa.ts
            const arAccounts = coa.filter((a: { code: string }) =>
                a.code.startsWith('112'),
            ); // Accounts Receivable group
            const apAccounts = coa.filter((a: { code: string }) =>
            ...
            // 4. Recent Journals
            const recentJournalsRes = await AccountingService.getJournals({
                status: 'POSTED',
                limit: 5,
            });
            ...
            return serializeData({
                performance: {
                    ...
                    expense:
                        performance.totalCOGS +
                        performance.totalOpEx +
                        performance.totalOther,
