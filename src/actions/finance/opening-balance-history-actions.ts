            ...
            try {
                ...
                const history = [
                    ...arInvoices.map((invoice) => ({
                        ...
                        entityName:
                            invoice.salesOrder?.customer?.name || 'Unknown',
                    ...
                    ...apInvoices.map((invoice) => ({
                        ...
                        invoiceNumber: invoice.invoiceNumber,
                        entityName:
                            invoice.purchaseOrder?.supplier?.name || 'Unknown',
                        date: invoice.invoiceDate,
                        amount: Number(invoice.totalAmount),
                        createdAt: invoice.createdAt,
                    })),
                ...
                logger.error('Failed to fetch opening balance history', {
                    error,
                    module: 'OpeningBalanceActions',
                });
                throw new BusinessRuleError(
                    'Failed to fetch opening balance history.',
                );
            }
        });
    },
            ...
            try {
                await prisma.$transaction(async (tx) => {
                    if (type === 'AR') {
                        ...
                        if (!invoice)
                            throw new NotFoundError('Purchase invoice', id);
                ...
                logger.error('Failed to delete opening balance', {
                    error,
                    id,
                    type,
                    module: 'OpeningBalanceActions',
