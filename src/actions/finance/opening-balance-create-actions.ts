...
export const getAccountsForOpeningBalance = withTenant(
    async function getAccountsForOpeningBalance() {
        return safeAction(async () => {
            ...
            const equityResolved = await resolveAccount(
...
export const saveUnifiedOpeningBalance = withTenant(
    async function saveUnifiedOpeningBalance(
        ...
        return safeAction(async () => {
            ...
            try {
                await prisma.$transaction(
                    async (tx) => {
                        const equityAccount =
                            await ensureOpeningBalanceEquityAccount(tx);

                        if (data.arEntries.length > 0) {
                            const subLedgerAccount =
                                await getSubLedgerAccountOrThrow(tx, 'AR');
                        ...
                        if (data.apEntries.length > 0) {
                            const subLedgerAccount =
                                await getSubLedgerAccountOrThrow(tx, 'AP');
                        ...
                        await postGeneralOpeningBalanceJournal(
                            data,
                            equityAccount.id,
                            session.user.id,
                            tx,
                ...
                logger.error('Failed to save unified opening balance', {
                    error,
