...
export const getBudgetVsActuals = withTenant(async function getBudgetVsActuals(
    year: number,
    month: number,
) {
        ...
        const results = await Promise.all(
            budgets.map(async (b) => {
                ...
                const totalDebit = lines.reduce(
                    (acc, curr) =>
                        acc + (curr.debit ? curr.debit.toNumber() : 0),
                    0,
                );
                const totalCredit = lines.reduce(
                    (acc, curr) =>
                        acc + (curr.credit ? curr.credit.toNumber() : 0),
                    0,
                );

                // Depending on account type. Expense: Debit is positive. Revenue: Credit is positive.
                let actualAmount = 0;
                if (b.account.category.includes('EXPENSE'))
                    actualAmount = totalDebit - totalCredit;
                else if (b.account.category.includes('REVENUE'))
                ...
                return {
                    ...
                    variancePercentage:
                        b.amount.toNumber() === 0
                            ? 0
                            : ((b.amount.toNumber() - actualAmount) /
                                  b.amount.toNumber()) *
                              100,
                };
            }),
