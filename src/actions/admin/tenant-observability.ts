 ...
 */
export async function getAllTenantStats(): Promise<
...
> {
    ...
    prisma
            ...
            tenants.map((t) => {
                ...
                return prisma.tenant.update({
                    ...
                    data: {
                        ...
                        cachedDbSizeBytes:
                            s.dbSizeBytes != null
                                ? BigInt(s.dbSizeBytes)
                                : null,
                        cachedLastActivityAt: s.lastActivityAt
        ...
        .catch((e) =>
            console.error('[getAllTenantStats] cache write failed:', e),
        );
 ...
 */
export async function getTenantStatsCached(): Promise<
    Record<string, TenantStats>
> {
    const session = await auth();
    if (!session?.user || !session.user.isSuperAdmin) {
    ...
    for (const t of tenants) {
        map[t.id] = {
            ...
            lastActivityAt: t.cachedLastActivityAt
                ...
                : null,
            dbSizeBytes:
                t.cachedDbSizeBytes != null
                    ? Number(t.cachedDbSizeBytes)
                    : null,
            cachedAt: t.cachedStatsAt
                ? new Date(t.cachedStatsAt).toISOString()
                : null,
        };
    }
    return map;
...
async function collectOneTenantStats(
    tenantId: string,
    dbUrl: string,
): Promise<TenantStats> {
    if (!dbUrl) {
    ...
    const [userCount, activeUserCount, lastActivity, sizeRows] =
        await Promise.all([
            db.user.count(),
    ...
    const dbSizeBytes =
        sizeRows?.[0]?.size != null ? Number(sizeRows[0].size) : null;
