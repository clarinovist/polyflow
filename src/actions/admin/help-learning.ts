...
async function requireSuperAdmin() {
    ...
    if (
        ...
        throw new ValidationError(
            'Only super-admin can manage learning queue.',
        );
    }
    return session;
}
...
export async function ignoreCluster(id: string) {
    ...
    const cluster = await mainDb.helpQuestionCluster.findUnique({
        where: { id },
    });
...
export async function approveDraftAsArticle(
    ...
    const existingSlug = await mainDb.helpArticle.findUnique({
        where: { slug: slugBase },
    });
...
export async function rejectDraft(id: string, reviewNote?: string) {
    ...
    if (draft.clusterId) {
        await mainDb.helpQuestionCluster
            .update({
                where: { id: draft.clusterId },
                data: { status: 'OPEN' },
            })
