...
function makeR2Client() {
    if (
        ...
        throw new BusinessRuleError(
            'Kredensial tulis R2/S3 belum dikonfigurasi di server.',
        );
    }
    return new S3Client({
...
export async function backupTenant(
    ...
    if (!tenant || !tenant.dbUrl) {
        throw new BusinessRuleError(
            'Tenant atau URL database tidak ditemukan.',
        );
    }
 ...
 */
export async function deleteTenant(tenantId: string, confirmSubdomain: string) {
    const session = await requireSuperAdmin();
    ...
    if (tenant.status !== 'SUSPENDED') {
        throw new BusinessRuleError(
            'Tenant harus SUSPENDED sebelum dapat dihapus. Suspend terlebih dahulu (masa tunggu).',
        );
    }
    ...
    try {
        ...
        if (!backup.success)
            throw new BusinessRuleError('Backup mengembalikan kegagalan');
    } catch (err) {
        throw new BusinessRuleError(
            `Pre-delete backup failed. Refusing to drop DB. Error: ${err instanceof Error ? err.message : 'unknown'}`,
        );
    }
    ...
    if (!dbName) {
        throw new BusinessRuleError(
            'Tidak dapat mengurai nama database dari dbUrl.',
        );
    }
    ...
    try {
        ...
        throw new BusinessRuleError(
            `DROP DATABASE failed: ${err instanceof Error ? err.message : 'unknown'}. Backup is safe in R2.`,
        );
    }
...
export async function restoreTenantBackup(
    ...
    try {
        ...
        if (!resp.Body)
            throw new BusinessRuleError('File backup kosong di R2.');
        // Collect stream body into a buffer, then write to temp file.
        const chunks: Uint8Array[] = [];
        for await (const chunk of resp.Body as AsyncIterable<
            Uint8Array | string
        >) {
        ...
        if (stderr)
            console.warn(
                '[restoreTenantBackup] pg_restore stderr:',
                stderr.slice(0, 500),
            );
