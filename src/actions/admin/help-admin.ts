...
export async function getHelpSettingValue<T>(
    key: string,
    fallback: T,
): Promise<T> {
    const mainDb = getMainPrisma();
...
export async function getHelpDashboardStats(days = 7) {
    ...
    const [total, byOutcome, byFeedback, avgLatency, byChannel, topGaps] =
        await Promise.all([
    ...
    return {
        ...
        byFeedback: Object.fromEntries(
            byFeedback.map((f) => [f.feedback || 'null', f._count.feedback]),
        ),
        avgLatency: Math.round(avgLatency._avg.latencyMs || 0),
        byChannel: Object.fromEntries(
            byChannel.map((c) => [c.channel, c._count.channel]),
        ),
        topGaps: topGaps.map((q) => ({
            question: q.question,
