...
export const setEmployeePin = withTenant(async function setEmployeePin(
    employeeId: string,
    pin: string,
) {
    try {
        ...
        logger.error('Failed to set employee PIN', {
            error,
            employeeId,
...
export const clearEmployeePin = withTenant(async function clearEmployeePin(
    employeeId: string,
) {
    try {
        await db.employee.update({
        ...
        logger.error('Failed to clear employee PIN', {
            error,
            employeeId,
            module: 'AttendanceActions',
        });
...
export const updateWorkShiftPlannedHours = withTenant(
    async function updateWorkShiftPlannedHours(
        id: string,
        ...
        try {
            ...
            logger.error('Failed to update planned hours', {
                error,
                shiftId: id,
                module: 'AttendanceActions',
            });
...
export const clockInManual = withTenant(async function clockInManual(
    employeeCode: string,
    workShiftId: string,
) {
    try {
        const session = await requireAuth();
        const result = await AttendanceService.clockInAsAdmin(db, {
            employeeCode,
            workShiftId,
            source: 'MANUAL',
        });
        ...
        logger.error('Manual clock-in failed', {
            error,
            employeeCode,
            module: 'AttendanceActions',
        });
        return {
            ...
            error:
                error instanceof Error
                    ? error.message
                    : 'Gagal clock-in manual',
        };
    }
});

export const clockOutManual = withTenant(async function clockOutManual(
    employeeCode: string,
) {
    try {
        const session = await requireAuth();
        const result = await AttendanceService.clockOutAsAdmin(db, {
            employeeCode,
        });
        await logActivity({
            userId: session.user.id,
        ...
        logger.error('Manual clock-out failed', {
            error,
            employeeCode,
            module: 'AttendanceActions',
        });
        return {
            ...
            error:
                error instanceof Error
                    ? error.message
                    : 'Gagal clock-out manual',
        };
    }
});

export const setAbsent = withTenant(async function setAbsent(
    employeeId: string,
    workDate: string,
    workShiftId: string,
    notes?: string,
    ...
    try {
        const session = await requireAuth();
        const result = await AttendanceService.setAbsent(
            db,
            employeeId,
            new Date(workDate),
            workShiftId,
        ...
        logger.error('Set absent failed', {
            error,
            employeeId,
            module: 'AttendanceActions',
        });
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Gagal set absent',
        };
    }
});

export const correctAttendance = withTenant(async function correctAttendance(
    recordId: string,
    data: { clockInAt?: string; clockOutAt?: string; notes?: string },
) {
    try {
        const session = await requireAuth();
        const updateData: {
        ...
        logger.error('Correct attendance failed', {
            error,
            recordId,
            module: 'AttendanceActions',
        });
        return {
            ...
            error:
                error instanceof Error
                    ? error.message
                    : 'Gagal koreksi absensi',
        };
    }
});

export const listAttendance = withTenant(async function listAttendance(
    date: string,
    filters?: ListFilters,
) {
    try {
        const records = await AttendanceService.listByDate(
            db,
            new Date(date),
            filters,
        );
        ...
        logger.error('List attendance failed', {
            error,
            date,
            module: 'AttendanceActions',
        });
...
export const listAttendanceRange = withTenant(
    async function listAttendanceRange(
        from: string,
        to: string,
        filters?: ListFilters,
    ) {
        try {
            const records = await AttendanceService.listByRange(
                db,
                new Date(from),
                new Date(to),
                filters,
            ...
            logger.error('List attendance range failed', {
                error,
                from,
                to,
                module: 'AttendanceActions',
            ...
            return {
                success: false,
                error: 'Gagal memuat data absensi rentang',
            };
        }
    },
);
...
export const getAttendanceWeeklySummary = withTenant(
    async function getAttendanceWeeklySummary(
        weekStart: string,
        weekEnd: string,
    ) {
        try {
            ...
            logger.error('Get weekly summary failed', {
                error,
                weekStart,
                weekEnd,
                module: 'AttendanceActions',
...
export const getAttendanceMonthlySummary = withTenant(
    async function getAttendanceMonthlySummary(year: number, month: number) {
        try {
            ...
            logger.error('Get monthly summary failed', {
                error,
                year,
                month,
                module: 'AttendanceActions',
...
export const listKioskEmployees = withTenant(
    async function listKioskEmployees() {
        try {
            ...
            logger.error('List kiosk employees failed', {
                error,
                module: 'AttendanceActions',
            });
...
export const kioskClockIn = withTenant(async function kioskClockIn(
    ...
    try {
        ...
        const { success } = rateLimit(
            `kiosk:${ip}:${employeeCode}`,
            KIOSK_RATE_LIMIT,
            KIOSK_RATE_WINDOW_MS,
        );
        if (!success) {
            return {
                success: false,
...
export const kioskClockOut = withTenant(async function kioskClockOut(
    employeeCode: string,
    pin: string,
