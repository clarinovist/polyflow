...
export const getSupplierPerformanceStats = withTenant(
    async function getSupplierPerformanceStats(supplierId: string) {
        return safeAction(async () => {
            ...
            const withReceipt = recentOrders.filter(
                (o) => o.goodsReceipts.length > 0,
            );
            let totalLeadDays = 0;
            let onTimeCount = 0;
            for (const o of withReceipt) {
                ...
                totalLeadDays += diff;
                if (o.expectedDate) {
                    if (new Date(receiptDate) <= new Date(o.expectedDate))
                        onTimeCount += 1;
                }
            }
            ...
            const avgLeadDays =
                withReceipt.length > 0
                    ? Math.round(totalLeadDays / withReceipt.length)
                    : null;
            const onTimeRate =
                withReceipt.length > 0
                    ? Math.round((onTimeCount / withReceipt.length) * 100)
                    : null;

            const returnsCount = await prisma.purchaseReturn.count({
                where: { supplierId },
            });
