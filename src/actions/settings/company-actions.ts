 ...
 */
export const getCompanySettings = withTenant(
...
export const updateCompanySettings = withTenant(
    async function updateCompanySettings(input: UpdateCompanyInput) {
        return safeAction(async () => {
            ...
            for (const [field, value] of entries) {
                if (
                    field === 'bankAccountsNonPPN' ||
                    field === 'bankAccountsPPN'
                ) {
                    let parsed: unknown;
                    try {
                        ...
                        throw new ValidationError(
                            `Format ${field} tidak valid. Harus JSON array [{holder,bank,account}].`,
                        );
                    }
                    if (!Array.isArray(parsed)) {
                        throw new ValidationError(
                            `Format ${field} harus berupa array.`,
                        );
                    }
                }
            }
...
export const uploadCompanyLogo = withTenant(async function uploadCompanyLogo(
    formData: FormData,
) {
    return safeAction(async () => {
        const adminId = await requireAdminId();
