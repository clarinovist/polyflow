...
export const getMyWeeklyPayroll = withTenant(async function getMyWeeklyPayroll(
    dateInWeek?: string,
) {
    try {
        ...
        const data = await PayrollService.getWeeklyPayroll(
            db,
            session.employeeId,
            ws,
            we,
        ...
        return {
            success: false,
            error: (e as Error).message || 'Gagal memuat gaji',
        };
    }
});
...
export const getMyAttendanceMonth = withTenant(
    async function getMyAttendanceMonth(year: number, month: number) {
        try {
            ...
            const data = await AttendanceService.listByEmployee(
                db,
                session.employeeId,
                from,
                to,
            ...
            return {
                success: false,
                error: (e as Error).message || 'Gagal memuat absensi',
            };
        }
    },
);

export const getMyProductions = withTenant(async function getMyProductions(
    from?: string,
    to?: string,
) {
    try {
        ...
        return {
            success: false,
            error: (e as Error).message || 'Gagal memuat produksi',
        };
    }
});
...
export const getMyPayslips = withTenant(async function getMyPayslips() {
    try {
        ...
        return {
            success: false,
            error: (e as Error).message || 'Gagal memuat slip',
        };
    }
});
            ...
            data: {
                ...
                bpjs: employee
                    ? {
                          ...
                          deduction: employee.bpjsEmployeeDeduction
                              ? Number(employee.bpjsEmployeeDeduction)
                              : 0,
