...
export const loginEmployee = withTenant(async function loginEmployee(
    phoneInput: string,
    pin: string,
) {
    try {
        ...
        if (!success) {
            return {
                success: false,
                error: 'Terlalu banyak percobaan. Coba lagi 5 menit.',
            };
        }
        ...
        return {
            success: true,
            data: { name: employee.name, code: employee.code },
        };
    } catch (e) {
...
export const logoutEmployee = withTenant(async function logoutEmployee() {
