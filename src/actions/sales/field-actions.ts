  ...
  async function getFieldSalesOrderById(id: string) {
    return safeAction(async () => {
      ...
      const order = await prisma.salesOrder.findUnique({
        where: { id },
        include: {
          customer: { select: { id: true, name: true, phone: true, billingAddress: true, latitude: true, longitude: true } },
          sourceLocation: true,
          items: {
    ...
    return safeAction(async () => {
      ...
      const [assigned, completed, extraCalls] = await Promise.all([
        ...
        prisma.salesRoutePlanItem.count({
          where: { ...routeWhere, status: { in: ["COMPLETED", "VISITING"] } },
        }),
        prisma.salesVisit.count({
          where: {
            ...(scope.isGlobalViewer ? {} : { userId: scope.actorUserId }),
            isExtraCall: true,
