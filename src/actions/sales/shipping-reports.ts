        ...
        return safeAction(async () => {
            ...
            if (filters.ownershipType || filters.rateType) {
                ...
                if (filters.ownershipType) {
                    where.vehicle.ownershipType =
                        filters.ownershipType as OwnershipType;
                }
                if (filters.rateType) {
            ...
            if (filters.routeName) {
                where.appliedRouteName = {
                    contains: filters.routeName,
                    mode: 'insensitive',
                };
            }
            ...
            const byVehicle = Array.from(byVehicleMap.values()).map((v) => ({
                ...v,
                avgCostPerDelivery:
                    v.totalDeliveries > 0 ? v.totalCost / v.totalDeliveries : 0,
            }));
            ...
            const byRoute = Array.from(byRouteMap.values()).sort(
                (a, b) => b.totalCharge - a.totalCharge,
            );
            ...
            const summary: ShippingReportSummary = {
                ...
                avgCostPerDelivery:
                    deliveries.length > 0 ? totalCost / deliveries.length : 0,
                avgChargePerDelivery:
                    deliveries.length > 0 ? totalCharge / deliveries.length : 0,
                byVehicle,
                byMonth,
                byOwnership,
                byRoute,
