        ...
        return safeAction(async () => {
            try {
                ...
                throw new BusinessRuleError(
                    'Gagal menghitung analytics. Silakan coba lagi.',
                );
            }
        });
    },
...
export const exportProductionAnalytics = withTenant(
    async function exportProductionAnalytics(dateRange: DateRange) {
        return safeAction(async () => {
            try {
                const data =
                    await AnalyticsService.getProductionAnalytics(dateRange);
                ...
                const qualityData = [
                    ...
                    [
                        'Pass Rate %',
                        data.quality.inspections.passRate.toFixed(2),
                    ],
                    [''],
                    ['Scrap by Reason', 'Quantity', 'Percentage %'],
                    ...data.quality.scrapByReason.map((s) => [
                        s.reason,
                        s.quantity,
