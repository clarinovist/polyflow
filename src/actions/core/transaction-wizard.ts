...
export const createWizardTransaction = withTenant(
    async function createWizardTransaction(data: TransactionWizardValues) {
        return safeAction(async () => {
            ...
            const config = TRANSACTION_TYPES.find(
                (t) => t.id === data.transactionTypeId,
            );
            if (!config) {
            ...
            try {
                if (config.requiresInvoice && data.invoiceId) {
                    ...
                    const targetAccountId = isSales
                        ? data.customDebitAccountId
                        : data.customCreditAccountId;
                    ...
                    if (targetAccountId) {
                        bankAcc = await prisma.account.findUnique({
                            where: { id: targetAccountId },
                        });
                    } else {
                        const resolved = await resolveAccount(
                            isSales
                                ? config.debitAccountRole
                                : config.creditAccountRole,
                        );
                        bankAcc = await prisma.account.findUnique({
                    ...
                    const pettyCash = await resolveAccount('petty-cash').catch(
                        () => null,
                    );
                    const isPettyCash =
                        !!bankAcc &&
                ...
                if (config.category === 'ASSET')
                    referenceType =
                        ReferenceType.GOODS_RECEIPT as ReferenceType;
                if (config.category === 'SALES')
                    referenceType =
                        ReferenceType.SALES_INVOICE as ReferenceType;
                ...
                logger.error('Wizard Transaction Error', {
                    error,
                    data,
                    module: 'TransactionWizard',
                });
