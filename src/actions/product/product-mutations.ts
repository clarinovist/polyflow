...
export const updateProduct = withTenant(async function updateProduct(
    ...
    return safeAction(async () => {
        ...
        try {
            ...
            const currentSkuCodes = currentProduct.variants.map(
                (v) => v.skuCode,
            );
            ...
            await prisma.$transaction(async (tx) => {
                ...
                const existingVariantIds = currentProduct.variants.map(
                    (v) => v.id,
                );
 ...
 */
export const quickCreateProduct = withTenant(
    async function quickCreateProduct(data: {
        productName: string;
        variantName: string;
        skuCode: string;
        primaryUnit: string;
        ...
        return safeAction(async () => {
            ...
            if (existingSku) {
                throw new BusinessRuleError(
                    `SKU ${data.skuCode.toUpperCase()} sudah ada`,
                );
            }
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                const variant = await tx.productVariant.create({
                    data: {
                        productId: product.id,
                        name:
                            data.variantName.trim() || data.productName.trim(),
        ...
        try {
            ...
            if (variantIds.length > 0) {
                ...
                if (productionMaterialCount > 0)
                    referenced.push(
                        `ProductionMaterial (${productionMaterialCount})`,
                    );
