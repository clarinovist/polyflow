    ...
    async function getPlanningDashboardStats() {
        return safeAction(async () => {
            ...
            const plannedJobsCount = await prisma.productionOrder.count({
                where: {
                    status: {
                        in: [ProductionStatus.DRAFT, ProductionStatus.RELEASED],
                    },
                },
            });
            ...
            const activeMachines =
                machineStats.find((s) => s.status === 'ACTIVE')?._count
                    .status || 0;
            const maintenanceMachines =
                machineStats.find((s) => s.status === 'MAINTENANCE')?._count
                    .status || 0;
            const brokenMachines =
                machineStats.find((s) => s.status === 'BROKEN')?._count
                    .status || 0;
            const totalMachines =
                activeMachines + maintenanceMachines + brokenMachines;
