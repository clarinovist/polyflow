...
export const getProductionLiveOverview = withTenant(
    async function getProductionLiveOverview() {
        return safeAction(async () => {
            ...
            const { startOfDay: yesterdayStart, endOfDay: yesterdayEnd } =
                getWibDayBounds(yesterdayStr);
            ...
            for (const exec of executionsYesterday) {
                ...
                addOutput(
                    'yesterday',
                    cat,
                    Number(exec.quantityProduced || 0),
                    0,
                );
            }
            ...
            for (const o of activeOrders) {
                ...
                if (o.status === 'IN_PROGRESS')
                    processes[processKey].activeJobs += 1;
                if (o.status === 'RELEASED')
                    processes[processKey].released += 1;
                if (o.status === 'WAITING_MATERIAL')
                    processes[processKey].waiting += 1;
            ...
            const totals = {
                activeJobs: PROCESS_KEYS.reduce(
                    (s, k) => s + processes[k].activeJobs,
                    0,
                ),
                released: PROCESS_KEYS.reduce(
                    (s, k) => s + processes[k].released,
                    0,
                ),
                waiting: PROCESS_KEYS.reduce(
                    (s, k) => s + processes[k].waiting,
                    0,
                ),
