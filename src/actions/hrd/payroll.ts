    ...
    async function getEmployeeWeeklyPayroll(
        ...
        return safeAction(async () => {
            try {
                ...
                return await PayrollService.getWeeklyPayroll(
                    prisma,
                    employeeId,
                    weekStart,
                    weekEnd,
                ...
                logger.error('Failed to get weekly payroll', {
                    error,
                    employeeId,
                    module: 'PayrollActions',
                });
                if (error instanceof BusinessRuleError) throw error;
                throw new BusinessRuleError(
                    'Gagal memuat perhitungan gaji mingguan',
                );
            }
        });
    },
    ...
    async function getAllWeeklyPayroll(dateInWeek?: string) {
        return safeAction(async () => {
            try {
                ...
                const weekStart = startOfWeek(baseDate);
                const weekEnd = endOfWeek(baseDate);
                return await PayrollService.getWeeklyPayrollForAll(
                    prisma,
                    weekStart,
                    weekEnd,
                );
            } catch (error) {
                logger.error('Failed to get all weekly payroll', {
                    error,
                    module: 'PayrollActions',
                });
