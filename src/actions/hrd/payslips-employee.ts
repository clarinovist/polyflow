    ...
    async function listPayslipsByEmployee(employeeId: string) {
        return safeAction(async () => {
            try {
                ...
                return await PayrollMonthlyService.listByEmployee(
                    prisma,
                    employeeId,
                );
            } catch (error) {
                logger.error('Failed to list payslips by employee', {
                    error,
                    employeeId,
                    module: 'PayslipEmployeeActions',
                });
                throw error;
            }
        });
    ...
    async function listActiveAllowances(employeeId: string) {
        return safeAction(async () => {
            try {
                ...
                logger.error('Failed to list active allowances', {
                    error,
                    employeeId,
                    module: 'PayslipEmployeeActions',
                });
