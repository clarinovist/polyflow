    ...
    async function getCustomerOwnedLocations() {
        return safeAction(async () => {
            try {
                ...
                logger.error('Failed to fetch customer-owned locations', {
                    error,
                    module: 'LocationsActions',
                });
                throw new ExternalServiceError(
...
export const createLocation = withTenant(async function createLocation(
    data: CreateLocationValues,
) {
    return safeAction(async () => {
        try {
            ...
            const { name, slug, description, locationType, locationPurpose } =
                parsed.data;
            ...
            if (existing) {
                throw new ValidationError(
                    'A location with a similar name or slug already exists',
                    undefined,
                );
            }
...
export const updateLocation = withTenant(async function updateLocation(
    id: string,
    data: UpdateLocationValues,
) {
    return safeAction(async () => {
        try {
