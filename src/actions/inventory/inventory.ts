...
export const getInventoryStats = withTenant(
    async function getInventoryStats(searchParams?: {
        locationId?: string;
        type?: string;
    }) {
...
export const getLocations = withTenant(async function getLocations() {
    return safeAction(async () => {
        await requireAuth();
...
export const transferStockBulk = withTenant(async function transferStockBulk(
    ...
    return safeAction(async () => {
        ...
        await InventoryMovementService.transferStockBulk(
            result.data,
            currentUserId,
        );
        // Audit log is already recorded inside the service layer within the transaction
...
export const adjustStock = withTenant(async function adjustStock(
    data: AdjustStockWithBatchValues,
    _userId?: string,
) {
    return safeAction(async () => {
        const session = await requireAuth();
...
export const adjustStockBulk = withTenant(async function adjustStockBulk(
    data: BulkAdjustStockValues,
    _userId?: string,
) {
    return safeAction(async () => {
        const session = await requireAuth();
...
export const updateThreshold = withTenant(async function updateThreshold(
    productVariantId: string,
    minStockAlert: number,
) {
    return safeAction(async () => {
        await requireAuth();
        await InventoryCoreService.updateThreshold(
            productVariantId,
            minStockAlert,
        );
...
export const getStockMovements = withTenant(async function getStockMovements(
    limit = 50,
    startDate?: Date,
    endDate?: Date,
) {
    return safeAction(async () => {
        await requireAuth();
        return await InventoryQueryService.getStockMovements({
            limit,
            startDate,
            endDate,
        });
...
export const getDashboardStats = withTenant(async function getDashboardStats() {
    return safeAction(async () => {
        await requireAuth();
...
export const getInventoryAsOf = withTenant(async function getInventoryAsOf(
    targetDate: Date,
    locationId?: string,
) {
    return safeAction(async () => {
...
export const acknowledgeHandover = withTenant(
    async function acknowledgeHandover(movementId: string) {
        return safeAction(async () => {
            ...
            if (!movement)
                throw new ValidationError('Pergerakan stok tidak ditemukan');

            // Append an acknowledgement note to the reference for auditability
            const newReference =
                `${movement.reference || ''}`.trim() +
                ` | ACK:${currentUserId}:${new Date().toISOString()}`;
...
export const getRealtimeStock = withTenant(async function getRealtimeStock(
    locationId: string,
    productVariantId: string,
) {
    return safeAction(async () => {
