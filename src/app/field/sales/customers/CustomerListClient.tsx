...
import { createProspectAction, checkCustomerDuplicateAction } from "@/actions/sales/field-prospect";
...
export function CustomerListClient({ customers, showStartVisit }: CustomerListClientProps) {
  ...
  const checkDuplicate = async (name: string, phone: string) => {
    ...
    try {
      const res = await checkCustomerDuplicateAction({ name, phone: phone || undefined });
  ...
  const handleCreateProspect = async () => {
    ...
    try {
      ...
      if (res?.success && res.data) {
        ...
        toast.error((res as { error?: string })?.error || "Gagal membuat toko baru");
      }
    } catch {
      ...
      {showStartVisit && !showNewStore && (
        ...
        >
          ...
          <div className="text-left">
            <p className="font-semibold text-sm text-emerald-800 dark:text-emerald-300">Toko / Prospek Baru</p>
            <p className="text-xs text-emerald-600/80 dark:text-emerald-400/70">Buat customer baru dan langsung check-in</p>
          </div>
        </button>
      )}
        ...
        <div className="border rounded-xl p-4 bg-card space-y-3">
          ...
          {duplicateWarning && duplicateWarning.length > 0 && (
            <div className="p-2 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-lg text-xs">
              <p className="font-semibold text-amber-800 dark:text-amber-300 mb-1">Kemungkinan duplikat:</p>
              {duplicateWarning.map((d) => (
                <button
                  key={d.id}
                  onClick={() => router.push(`/field/sales/customers/${d.id}`)}
