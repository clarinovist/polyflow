...
export async function POST(request: NextRequest) {
    try {
        ...
        const deliveryOrderId = formData.get('deliveryOrderId') as
            | string
            | null;
        ...
        if (!deliveryOrderId) {
            return NextResponse.json(
                { error: 'deliveryOrderId required' },
                { status: 400 },
            );
        }
        ...
        if (photoType !== 'vehicle' && photoType !== 'proof_of_delivery') {
            return NextResponse.json(
                { error: "photoType must be 'vehicle' or 'proof_of_delivery'" },
                { status: 400 },
            );
        }

        // Allow empty MIME (iOS camera) → infer from filename
        ...
        const key = buildDeliveryPhotoKey(
            tenant,
            deliveryOrderId,
            photoType,
            file.name,
