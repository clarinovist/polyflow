...
export const POST = withTenantRoute(async function POST(req: NextRequest) {
  ...
  if (body.articles.length > MAX_ARTICLES_PER_REQUEST) {
    return NextResponse.json(
      { success: false, error: `Maximum ${MAX_ARTICLES_PER_REQUEST} articles per request.` },
      { status: 400 },
    );
  }
  ...
  for (const article of body.articles) {
    try {
      if (!article.slug || !article.title || !article.bodyMd) {
        results.push({ slug: article.slug || 'unknown', success: false, error: 'Missing required fields (slug, title, bodyMd)' });
        continue;
      }

      if (article.bodyMd.length > MAX_IMPORT_SIZE) {
        results.push({ slug: article.slug, success: false, error: `Body too long (max ${MAX_IMPORT_SIZE} chars)` });
        continue;
      }
