    ...
    try {
        ...
        try {
            ...
            const { autoExpireQuotations } =
                await import('@/services/sales/orders-service');
            expiredQuotationsCount = await autoExpireQuotations();
            if (expiredQuotationsCount > 0) {
                console.log(
                    `[Cron] Auto-expired ${expiredQuotationsCount} quotation(s) past validUntil.`,
                );
            }
        } catch (expireErr) {
            console.error(
                'Failed to auto-expire quotations during cron:',
                expireErr,
            );
        }
