...
function AccountInner() {
    ...
    const handleUnlink = async () => {
        if (
            !confirm(
                'Putuskan koneksi Telegram? Anda perlu link ulang untuk akses Mini App.',
            )
        )
            return;
        setUnlinking(true);
        try {
            const res = await fetch('/api/telegram/mini-app/unlink', {
                method: 'POST',
                credentials: 'include',
            });
            if (res.ok) {
            ...
            <div className="space-y-3">
                ...
                <div className="tg-card p-4">
                    <div className="text-[11px] opacity-60">
                        Tenant & koneksi Telegram
                    </div>
                    <div className="mt-1 text-xs">
                        Tenant ID:{' '}
                        <span className="font-mono">
                            {data.tenant.id.slice(0, 8)}…
                        </span>
                    </div>
                    <div className="text-xs">
                        Telegram UID hash-safe:{' '}
                        {String(data.connection.telegramUserId).slice(0, 6)}…
                    </div>
                    <div className="text-xs">
                        Terhubung:{' '}
                        {new Date(data.connection.linkedAt).toLocaleString(
                            'id-ID',
                ...
                <div className="tg-card p-4">
                    <div className="text-[11px] opacity-60">
                        Domain diizinkan (role-aware)
                    </div>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        {data.user.allowedDomains.length === 0 ? (
                            <span className="text-xs opacity-60">—</span>
                        ) : (
                            data.user.allowedDomains.map((d) => (
                                <span
                                    key={d}
                    ...
                    <div className="mt-2 text-[10px] opacity-40">
                        Resource detail tidak ditampilkan demi privasi. Backend
                        tetap cek ulang setiap request.
                    </div>
                </div>
                ...
                <div className="tg-card p-4">
                    ...
                    <div className="mt-1 text-xs">
                        Enabled:{' '}
                        {data.features.notificationsEnabled ? 'Ya' : 'Tidak'} •
                        Stok kritis:{' '}
                        {data.features.criticalStock ? 'Aktif' : 'Off'}
                    </div>
                    <div className="mt-1 text-[10px] opacity-50">
                        Preference per user — quiet hours Asia/Jakarta,
                        deduplication, kill switch ready (foundation Phase 1).
                    </div>
                </div>
                ...
                <div className="tg-card p-4">
                    ...
                    <div className="text-xs">
                        Versi Mini App: {data.version}
                    </div>
                    <div className="mt-1 text-xs opacity-60">
                        Bot: @pico2004_bot • URL:
                        https://melindo.polyflow.uk/telegram
                    </div>
                    <a
                        href="/support"
                        target="_blank"
