...
export default async function MyHomePage() {
    ...
    const [weeklyRes, prodRes, attRes, loanRes] = await Promise.all([
        getMyWeeklyPayroll().catch(
            (): WeeklyRes => ({ success: false, error: 'Gagal' }),
        ),
        getMyProductions().catch(
            (): ProdRes => ({ success: false, error: 'Gagal' }),
        ),
        getMyAttendanceMonth(
            new Date().getFullYear(),
            new Date().getMonth() + 1,
        ).catch((): AttRes => ({ success: false, error: 'Gagal' })),
        getMyLoansAndBpjs().catch(
            (): LoanRes => ({ success: false, error: 'Gagal' }),
        ),
    ]);
        ...
        <div className="space-y-4">
            <div className="rounded-2xl bg-gradient-to-br from-primary to-primary/70 text-primary-foreground p-5 shadow-sm">
                ...
                <div className="text-xs opacity-90 mt-1">
                    Performa & gaji kamu update real-time dari kiosk.
                </div>
                {weekly && (
                    <div className="mt-4 grid grid-cols-2 gap-3">
                        <div className="bg-white/15 backdrop-blur rounded-xl p-3">
                            ...
                            <div className="text-[10px] mt-1 opacity-80">
                                {weekly.daysWorked ?? 0} hari • {totalKgWeek} Kg
                            </div>
                        </div>
                        <div className="bg-white/15 backdrop-blur rounded-xl p-3">
                            <div className="text-[10px] opacity-80 uppercase tracking-wider">
            ...
            {!empFull?.phone && (
                <div className="flex gap-2 text-xs bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-3">
                    ...
                    No HP belum diisi. Hubungi HRD supaya bisa login pakai No
                    HP.
                </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Card className="hover:shadow-md transition-shadow h-full">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm flex items-center gap-2">
                                <Wallet className="h-4 w-4" /> Gaji Saya
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-[11px] text-muted-foreground">
                                Estimasi, potongan BPJS, kasbon, slip.
                            </div>
                            {weekly && (
                ...
                <Link href="/my/produksi">
                    <Card className="hover:shadow-md transition-shadow h-full">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm flex items-center gap-2">
                                <Factory className="h-4 w-4" /> Hasil Saya
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-[11px] text-muted-foreground">
                                Output borongan & harian.
                            </div>
                            <div className="mt-2 text-sm font-bold">
                                {prods.length} laporan (30 hari)
                            </div>
                        </CardContent>
                    </Card>
                </Link>
                <Link href="/my/absensi">
                    <Card className="hover:shadow-md transition-shadow h-full">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm flex items-center gap-2">
                                <Calendar className="h-4 w-4" /> Absensi
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-[11px] text-muted-foreground">
                                Hadir, izin, alfa bulan ini.
                            </div>
                            <div className="mt-2 text-sm font-bold">
                                {hadirBulanIni} hari hadir
                            </div>
                        </CardContent>
                    </Card>
                </Link>
            ...
            <div className="space-y-2">
                <h3 className="text-sm font-bold tracking-tight">
                    Produksi Terbaru (My Input → Gaji)
                </h3>
                {prods.length === 0 ? (
                    <Card>
                        <CardContent className="py-6 text-xs text-muted-foreground text-center">
                            Belum ada input 30 hari terakhir. Yuk input di kiosk
                            — gaji naik tiap input.
                        </CardContent>
                    </Card>
                ) : (
                    <div className="space-y-2">
                            ...
                            <Card key={p.id} className="py-0">
                                <CardContent className="p-3 flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                        ...
                                        {p.pieceEarnings != null && (
                                            <div className="text-[10px] text-emerald-600">
                                                {fmtIdr(p.pieceEarnings)}
                                            </div>
                                        )}
                                    </div>
                                </CardContent>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <Card>
                        ...
                        <CardContent className="text-[11px] space-y-1">
                            ...
                            {finance.bpjs?.deduction ? (
                                <div>
                                    Potongan: {fmtIdr(finance.bpjs.deduction)}
                                </div>
                            ) : null}
                            {finance.salary?.bankName && (
                                <div>
                                    {finance.salary.bankName} •{' '}
                                    {finance.salary.bankNo}
                                </div>
                            )}
                        </CardContent>
                    </Card>
