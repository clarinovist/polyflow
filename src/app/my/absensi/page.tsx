...
export default async function MyAbsensiPage({
    searchParams,
}: {
        ...
        <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-1">
                    <a
                        href={`/my/absensi?year=${prevMonth.year}&month=${prevMonth.month}`}
                        className="px-3 py-1.5 rounded-full border text-xs"
                    >
                        ‹
                    </a>
                    <a
                        href={`/my/absensi?year=${nextMonth.year}&month=${nextMonth.month}`}
                        className="px-3 py-1.5 rounded-full border text-xs"
                    >
                        ›
                    </a>
            ...
            <div className="grid grid-cols-4 gap-2">
                ...
                <Card className="py-0">
                    <CardContent className="p-3">
                        ...
                        <div className="font-black text-rose-600">{absent}</div>
                    </CardContent>
                </Card>
                <Card className="py-0">
                    <CardContent className="p-3">
                        ...
                        <div className="font-black text-amber-600">{leave}</div>
                    </CardContent>
                </Card>
                <Card className="py-0">
                    <CardContent className="p-3">
                        ...
                        <div className="font-black">{hours.toFixed(1)}</div>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-xs">Riwayat</CardTitle>
                </CardHeader>
                            ...
                            {records.map((r) => (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <div
                                            className={`text-[10px] px-2 py-0.5 rounded-full inline-block ${r.status === 'PRESENT' ? 'bg-emerald-100 text-emerald-700' : r.status === 'ABSENT' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}
                                        >
                                            {r.status}
                                        </div>
                                        <div className="text-[11px] mt-1">
                                            {r.actualHours != null
                                                ? `${Number(r.actualHours).toFixed(1)}j`
                                                : '-'}
                                        </div>
                                    </div>
                                </div>
