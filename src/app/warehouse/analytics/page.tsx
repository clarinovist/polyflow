...
function computeLowStockVariantIds(
    ...
    for (const item of inventory) {
        ...
        if (item.productVariant?.minStockAlert != null && !thresholds.has(id)) {
            thresholds.set(
                id,
                toDecimalNumber(item.productVariant.minStockAlert),
            );
        }
    }
...
export default async function AnalyticsDashboard() {
    ...
    const dashboard =
        dashboardRes.success && dashboardRes.data
            ? dashboardRes.data
            : { totalStock: 0, lowStockCount: 0, suggestedPurchasesCount: 0 };
    const agingSummary = agingSummaryRes;
    const suggestedPurchases =
        suggestedPurchasesRes.success && suggestedPurchasesRes.data
            ? suggestedPurchasesRes.data
            : [];
    const liveInventory =
    ...
    const lowStockVariantIds = computeLowStockVariantIds(
    ...
    const slowMoving = agingDetails
        ...
        .sort(
            (a, b) =>
                (b.buckets['90+']?.quantity || 0) -
                (a.buckets['90+']?.quantity || 0),
        )
        .slice(0, 10);
        ...
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Kesehatan stok + daftar yang perlu ditindak.
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <KPICard
                    title="Perlu Reorder"
                    value={(
                        dashboard.suggestedPurchasesCount ||
                    ...
                    <CardContent>
                        <div className="flex items-center gap-6">
                            <ABCBar
                                ...
                                count={abcSummary.a}
                                total={
                                    abcSummary.a + abcSummary.b + abcSummary.c
                                }
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-bold">
                                    {agingSummary.totalItems.toLocaleString()}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-sm text-muted-foreground">
                                ...
                                <span className="text-sm font-bold">
                                    {agingSummary.avgDays.toFixed(0)} hari
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-sm text-muted-foreground">
                                ...
                                <span
                                    className={cn(
                                        'text-sm font-bold',
                                        agingSummary.slowMovingCount > 0
                                            ? 'text-red-600 dark:text-red-400'
                                            : '',
                                    )}
                                >
                                    {agingSummary.slowMovingCount.toLocaleString()}{' '}
                                    item
                                </span>
                            </div>
                            <Link
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <Link href="/warehouse/inventory">
                    <QuickLinkCard
                        title="Stok"
                        description="Lihat tabel inventaris lengkap"
                    />
                </Link>
                <Link href="/warehouse/inventory/aging">
                    <QuickLinkCard
                        title="Detail Aging"
                        description="Analisis umur stok per item"
                    />
                </Link>
                <Link href="/warehouse/inventory/history">
                    <QuickLinkCard
                        title="Riwayat Mutasi"
                        description="Lacak semua pergerakan stok"
                    />
                </Link>
            </div>
        </div>
