...
export default function KioskJobList({
    initialOrders,
    employees,
    machines,
}: KioskJobListProps) {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
                <p className="text-muted-foreground text-lg mb-4">
                    Belum ada operator yang dipilih.
                </p>
                <Link href="/kiosk">
                    <Button
                        variant="outline"
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-card p-4 md:p-6 rounded-xl border-2 shadow-sm gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <form
                        action={refreshKioskData}
                        className="w-full md:w-auto"
                    >
                        <Button
                            variant="secondary"
                            size="lg"
                            className="w-full md:w-auto h-12 md:h-14 md:px-8 text-base md:text-lg font-bold border-2 active:scale-95"
                        >
            ...
            {hasFilter && (
                <div className="flex justify-start">
                    <Button
                        variant="outline"
                        onClick={clearFilter}
                        className="h-10 border-2 text-muted-foreground font-bold text-sm px-4"
                    >
                        <Search className="mr-2 h-4 w-4" /> HAPUS FILTER:{' '}
                        {searchParams.get('q')}
                    </Button>
                </div>
            )}
            ...
            {selectedOperatorId && operatorMachineIds.length === 0 && (
                <div className="bg-amber-50 dark:bg-amber-950 border-2 border-amber-200 dark:border-amber-800 rounded-xl p-4 text-amber-800 dark:text-amber-200">
                    ...
                    <p className="text-sm">
                        Anda belum ditugaskan ke mesin tertentu. Hubungi
                        supervisor.
                    </p>
                </div>
            )}
                        ...
                        {hasFilter ? (
                            <>
                                <p className="text-2xl font-bold">
                                    {kioskLabels.emptyNoFilterMatch} &quot;
                                    {searchParams.get('q')}&quot;
                                </p>
                                <Button
                                    variant="link"
                                    ...
                                    className="text-xl"
                                >
                                    {kioskLabels.emptyClearFilter}
                                </Button>
                            </>
                        ) : operatorMachineIds.length > 0 ? (
                            <>
                                <p className="text-2xl font-bold">
                                    {kioskLabels.emptyNoJobsForMachine.toUpperCase()}
                                </p>
                                <p className="text-lg">
                                    {kioskLabels.emptyWaitingRelease}
                                </p>
                            </>
                        ) : (
                            <>
                                <p className="text-2xl font-bold">
                                    {kioskLabels.emptyNoJobsReady.toUpperCase()}
                                </p>
                                <p className="text-lg">
                                    {kioskLabels.emptyWaitingRelease}
                                </p>
