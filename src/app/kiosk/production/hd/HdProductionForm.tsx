...
export default function HdProductionForm({
    ...
    const fetchShiftsForOrder = useCallback(
        ...
        try {
            const reports = data.reports.map((report) => {
                const order = orders.find(
                    (item) => item.id === report.productionOrderId,
                );
            ...
            if (res.status === 207) {
                ...
                toast.warning(
                    `${successCount} laporan berhasil, ${failedCount} gagal. Cek konsol untuk detail.`,
                );
            ...
            if (!res.ok) {
                throw new Error(
                    json.message || json.error || 'Gagal menyimpan laporan',
                );
            }

            toast.success(
                `${json.count ?? reports.length} laporan berhasil disubmit!`,
            );
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                {fields.map((field, index) => {
                    const selectedOrder = orders.find(
                        (order) =>
                            order.id ===
                            form.watch(`reports.${index}.productionOrderId`),
                    );
                    const unitMeta = getProductionUnitMeta(
                        selectedOrder?.bom?.productVariant || {},
                    );

                    return (
                        <div
                            key={field.id}
                            className="relative p-6 rounded-2xl bg-white/5 dark:bg-white/10 border border-white/10 dark:border-white/20 backdrop-blur-md shadow-2xl flex flex-col gap-6 transition-all hover:bg-white/10 dark:hover:bg-white/15"
                        >
                            {/* Header Panel */}
                            <div className="flex justify-between items-center border-b border-white/10 dark:border-white/20 pb-4">
                                <h3 className="text-xl font-bold text-primary flex items-center gap-2">
                                    <span className="bg-primary/20 dark:bg-primary/30 text-primary h-8 w-8 rounded-full flex items-center justify-center text-sm">
                                        {index + 1}
                                    </span>
                                    Laporan Output Baru
                                </h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-white/70 dark:text-white/80">
                                                {productionLabels.workOrder}
                                            </FormLabel>
                                            <Select
                                                onValueChange={(val) => {
                                                    field.onChange(val);
                                                    fetchShiftsForOrder(val);
                                                }}
                                                value={field.value}
                                            >
                                                <FormControl>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => {
                                        const currentOrderId = form.watch(
                                            `reports.${index}.productionOrderId`,
                                        );
                                        const availableShifts = currentOrderId
                                            ? shiftsByOrder[currentOrderId] ||
                                              []
                                            : [];
                                        const isLoading = currentOrderId
                                            ? loadingShifts[currentOrderId]
                                            : false;
                            ...
                            <FormField
                                ...
                                render={({ field }) => {
                                    const currentOperatorId = form.watch(
                                        `reports.${index}.operatorId`,
                                    );
                                    const eligibleHelpers = employees.filter(
                                        (e) => e.id !== currentOperatorId,
                                    );
                                    const currentHelpers: string[] =
                                        field.value ?? [];

                                    const toggleHelper = (empId: string) => {
                                        const next = currentHelpers.includes(
                                            empId,
                                        )
                                            ? currentHelpers.filter(
                                                  (id) => id !== empId,
                                              )
                                        ...
                                        <FormItem>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3 bg-black/20 dark:bg-black/40 rounded-lg border border-white/5 dark:border-white/10">
                                                {eligibleHelpers.map((e) => (
                                                    ...
                                                    >
                                                        <Checkbox
                                                            checked={currentHelpers.includes(
                                                                e.id,
                                                            )}
                                                            onCheckedChange={() =>
                                                                toggleHelper(
                                                ...
                                                {eligibleHelpers.length ===
                                                    0 && (
                                                    <p className="text-white/30 dark:text-white/50 text-xs col-span-4">
                                                        Pilih operator terlebih
                                                        dahulu
                                                    </p>
                                                )}
                                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-black/20 dark:bg-black/40 p-4 rounded-xl">
                                        ...
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input
                                                    type="number"
                                                    step="any"
                                                    className="bg-slate-900/80 dark:bg-slate-900/95 border-white/10 dark:border-white/20 text-primary font-bold text-lg"
                                                    {...field}
                                                />
                                            </FormControl>
                                            <FormMessage />
                                        ...
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input
                                                    type="number"
                                                    step="any"
                                                    className="bg-slate-900/80 dark:bg-slate-900/95 border-white/10 dark:border-white/20 text-yellow-400 dark:text-yellow-300 font-bold text-lg"
                                                    {...field}
                                                />
                                            </FormControl>
                                            <FormMessage />
                                ...
                                <FormField
                                    control={form.control}
                                    name={`reports.${index}.quantityProduced`}
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-white/70 dark:text-white/80">
                                                Netto / Valid (
                                            ...
                                            <FormControl>
                                                <Input
                                                    type="number"
                                                    step="any"
                                                    className="bg-slate-900/80 dark:bg-slate-900/95 border-white/20 dark:border-white/30 text-green-400 dark:text-green-300 font-black text-xl shadow-[0_0_15px_rgba(74,222,128,0.2)] dark:shadow-[0_0_20px_rgba(74,222,128,0.3)]"
                                                    {...field}
                                                />
                                            </FormControl>
                                            <FormMessage />
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input
                                                    {...field}
                                                    placeholder="Contoh: 12g"
                                                    className="bg-slate-900/80 dark:bg-slate-900/95 border-white/10 dark:border-white/20 text-white"
                                                />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <FormField
                                    control={form.control}
                                    name={`reports.${index}.scrapProngkolQty`}
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-white/70 dark:text-white/80">
                                                Scrap Lumps (Kg)
                                            ...
                                            <FormControl>
                                                <Input
                                                    type="number"
                                                    step="any"
                                                    className="bg-slate-900/80 dark:bg-slate-900/95 border-white/10 dark:border-white/20 text-red-400 dark:text-red-300"
                                                    {...field}
                                                />
                                            </FormControl>
                                            <FormMessage />
                                ...
                                <FormField
                                    control={form.control}
                                    name={`reports.${index}.scrapDaunQty`}
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-white/70 dark:text-white/80">
                                                Scrap Daun (Kg)
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-card/10 dark:bg-card/20 p-6 rounded-2xl border-t-4 border-primary/50 dark:border-primary/60 shadow-lg backdrop-blur-xl sticky bottom-4 z-50">
                    ...
                    <Button
                        ...
                        disabled={
                            isSubmitting ||
                            fields.some((_, i) => {
                                const orderId = form.watch(
                                    `reports.${i}.productionOrderId`,
                                );
                                const shifts = orderId
                                    ? shiftsByOrder[orderId] || []
                                    : [];
                                return (
                                    orderId &&
                                    shifts.length > 0 &&
                                    !form.watch(`reports.${i}.shiftId`)
                                );
                            })
                        }
                        size="lg"
                    ...
                    >
                        {isSubmitting ? (
                            <div className="flex items-center">
                                <div className="animate-spin h-5 w-5 border-2 border-white dark:border-white border-t-transparent dark:border-t-transparent rounded-full mr-2" />{' '}
                                Mengirim...
                            </div>
                        ) : (
                            <>
                                <CheckCircle2 className="h-6 w-6 mr-2" /> KIRIM
                                SEMUA LAPORAN
                            </>
                        )}
                    </Button>
