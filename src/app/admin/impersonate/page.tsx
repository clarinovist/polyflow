...
export default async function ImpersonationView({
    ...
    if (
        !session?.user ||
        !(session.user as { impersonatedBy?: string }).impersonatedBy
    ) {
    ...
    const expiresAt = (session.user as { impersonationExpiresAt?: number })
        ...
        <div className="min-h-screen bg-secondary/30">
            ...
            <div className="sticky top-0 z-50 bg-amber-500 text-black px-4 py-2 shadow-lg">
                <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-2 text-sm font-semibold">
                        ...
                        <span>
                            IMPERSONATING:{' '}
                            {impersonatedUser?.name ?? 'tenant user'} (
            ...
            <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            You are acting as{' '}
                            <strong>
                                {impersonatedUser?.email ?? 'tenant user'}
                            </strong>{' '}
                            (role: ADMIN) on tenant{' '}
                            <strong>{tenant.name}</strong>. All actions taken
                            here are audit-logged under your superadmin ID.
                        </p>
                    </div>
                    <Button asChild variant="outline" size="sm">
                        <a
                            href={`https://${tenant.subdomain}.polyflow.uk`}
                            target="_blank"
                            rel="noreferrer"
                        >
                            <ExternalLink className="h-4 w-4" /> Buka workspace
                            tenant (login sendiri)
                        </a>
                    </Button>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base">
                            Users di tenant ini ({users.length})
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Table>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base">
                            Recent tenant activity ({recentAudit.length})
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        {recentAudit.length === 0 ? (
                            <p className="text-sm text-muted-foreground">
                                Belum ada aktivitas di tenant ini.
                            </p>
                        ) : (
                            <div className="space-y-2">
                                {recentAudit.map((l) => (
                                    <div
                                        key={l.id}
                                        className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-sm border-b pb-2 last:border-0"
                                    >
                                        <div>
                                            <Badge className="font-mono text-[10px] mr-2">
                                                {l.action}
                                            </Badge>
