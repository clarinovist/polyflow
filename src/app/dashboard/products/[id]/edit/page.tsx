...
function resolveConsumptionRule(
    attributes: unknown,
): ProductVariantFormValues['consumptionRule'] {
    if (
        !attributes ||
    ...
    return CONSUMPTION_RULES.has(
        rule as ProductVariantFormValues['consumptionRule'],
    )
        ? ((rule as ProductVariantFormValues['consumptionRule']) ??
              'PROPORTIONAL')
        : 'PROPORTIONAL';
...
export default async function EditProductPage({
    ...
    const [productRes, productTypesRes, unitsRes, accountsRes] =
        await Promise.all([
            getProductById(id),
            getProductTypes(),
            getUnits(),
    ...
    const firstVariantUseful = (
        productData.variants[0]?.attributes as {
            usefulLifeMonths?: number;
        } | null
    )?.usefulLifeMonths;
    const formData = {
