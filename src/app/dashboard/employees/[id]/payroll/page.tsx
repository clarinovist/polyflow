            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        ...
                        <Badge
                            variant="outline"
                            className={`ml-2 ${isPiece ? 'text-amber-600 border-amber-500/30' : ''}`}
                        >
                ...
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                            <Link
                                href={`/dashboard/employees/${id}/payroll?week=${prevWeek.toISOString().slice(0, 10)}`}
                            >
                            ...
                            <Link
                                href={`/dashboard/employees/${id}/payroll?week=${nextWeek.toISOString().slice(0, 10)}`}
                            >
                                <Button variant="ghost" size="icon">
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="bg-muted/30 p-4 rounded-lg">
                            ...
                            <div className="text-2xl font-bold">
                                {formatHours(payroll.totalActualHours)}
                            </div>
                        </div>
                        {isPiece ? (
                            ...
                            <div className="bg-muted/30 p-4 rounded-lg">
                                ...
                                <div className="text-2xl font-bold">
                                    {formatHours(payroll.totalOvertimeHours)}
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        {isPiece ? (
                            <>
                                <div className="bg-amber-500/5 p-4 rounded-lg border border-amber-500/10">
                                    ...
                                    <div className="text-xl font-bold">
                                        {formatIdr(payroll.totalPieceEarnings)}
                                    </div>
                                    <div className="text-xs text-muted-foreground mt-1">
                                        {payroll.totalKgPaid} kg dibayar
                                    </div>
                                </div>
                                {payroll.totalKgUnpaid > 0 && (
                                    <div className="bg-rose-500/5 p-4 rounded-lg border border-rose-500/10">
                                        <div className="text-[10px] uppercase tracking-wider text-rose-700 dark:text-rose-400">
                                            Kg Exception
                                        </div>
                                        <div className="text-xl font-bold text-rose-700 dark:text-rose-400">
                                            {payroll.totalKgUnpaid}
                                        </div>
                                        <div className="text-xs text-muted-foreground mt-1">
                                            tanpa absen / rate missing
                                        </div>
                                    </div>
                                )}
                            </>
                            ...
                            <>
                                <div className="bg-primary/5 p-4 rounded-lg border border-primary/10">
                                    ...
                                    <div className="text-xl font-bold">
                                        {formatIdr(payroll.totalDailyEarnings)}
                                    </div>
                                </div>
                                <div className="bg-primary/5 p-4 rounded-lg border border-primary/10">
                                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                                ...
                                <TableBody>
                                    {payroll.pieceLines.map((line) => (
                                        <TableRow key={line.executionId}>
                                            ...
                                            <TableCell className="text-right font-mono">
                                                {line.ratePerKg != null
                                                    ? formatIdr(line.ratePerKg)
                                                    : '-'}
                                            </TableCell>
                                            <TableCell className="text-right font-mono font-semibold">
                                                {line.pieceEarnings != null
                                                    ? formatIdr(
                                                          line.pieceEarnings,
                                                      )
                                                    : '-'}
                                            ...
                                            <TableCell>
                                                {line.paid ? (
                                                    <Badge
                                                        variant="outline"
                                                        className="bg-emerald-500/10 text-emerald-600 border-emerald-500/10"
                                                    >
                                                        Dibayar
                                                    </Badge>
                                                ) : (
                                                    <Badge
                                                        variant="outline"
                                                        className="bg-rose-500/10 text-rose-600 border-rose-500/10"
                                                    >
                                                        {line.reason ===
                                                        'NO_ATTENDANCE'
                                                            ? 'Tanpa Absen'
                                ...
                                <TableBody>
                                    {payroll.records.length === 0 ? (
                                        <TableRow>
                                            ...
                                            >
                                                Tidak ada data absensi di minggu
                                                ini.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        payroll.records.map((record) => (
                                            <TableRow
                                                key={`${record.date.toISOString()}-${record.shiftName}`}
                                            >
                                                <TableCell>
