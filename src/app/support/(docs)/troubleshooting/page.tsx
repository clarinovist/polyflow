...
export default async function SupportTroubleshootPage({
    ...
    // shared predicate with docs-sidebar.tsx so both views agree on what
    // counts as a troubleshooting article.
    const withErrorCodes = await listPublishedArticles({
        module: moduleFilter,
        q,
        limit: 30,
    }).then((all) => all.filter(isTroubleshootArticle));
    ...
    return (
        <>
            <SupportPageHeader
                ...
                subtitle="Solusi untuk error umum, stok, period locked, permission, dan backflush."
            />

            <div className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    <SupportSearchBox
                        basePath="/support/troubleshooting"
                        q={q}
                    />
                    <SupportModuleChips
                        module={moduleFilter}
                ...
                {q && (
                    <p className="text-xs text-muted-foreground">
                        ...
                        {merged.length} artikel ditemukan.
                    </p>
                )}
