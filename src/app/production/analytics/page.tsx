...
export default async function ProductionAnalyticsPage(props: {
    ...
    const avgYield =
        data.realization.reduce((acc, curr) => acc + curr.yieldRate, 0) /
        (data.realization.length || 1);
    const totalScrap = data.quality.scrapByReason.reduce(
        (acc, curr) => acc + curr.quantity,
        0,
    );
    const passRate = data.quality.inspections.passRate;
    const activeMachines = data.machinePerformance.filter(
        (m) => m.totalOperatingHours > 0,
    ).length;
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Kinerja periode + item yang perlu ditindak.
                    </p>
                </div>
                <AnalyticsToolbar />
            </div>
                ...
                <CardContent>
                    <Tabs defaultValue="scrap" className="w-full">
                        ...
                        <TabsContent value="scrap">
                            {data.quality.scrapByProduct.length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Link
                                        href="/production/history?from=today&to=today"
                                        className="text-xs font-semibold text-primary hover:underline inline-flex items-center gap-1"
                                    >
                                        Lihat log hasil{' '}
                                        <ExternalLink className="h-3 w-3" />
                        ...
                        <TabsContent value="yield">
                            {data.realization.filter((r) => r.yieldRate < 90)
                                .length === 0 ? (
                                <p className="text-sm text-muted-foreground py-4 text-center">
                                    Semua yield di atas threshold.
                                </p>
                            ) : (
                                <div className="rounded-md border overflow-x-auto">
                                    <Table>
                                                            ...
                                                            <Link
                                                                href={
                                                                    item.orderId
                                                                        ...
                                                                        : `/production/orders?status=${item.status || 'IN_PROGRESS'}`
                                                                }
                                                                className="text-xs font-semibold text-primary hover:underline inline-flex items-center gap-1"
                                                            >
                                                                Buka SPK{' '}
