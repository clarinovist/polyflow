    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Job Dispatcher</h2>
                    <p className="text-muted-foreground">Manage active SPKs and floor assignments.</p>
                </div>
            </div>
                                        ...
                                        <CardContent className="flex-1 p-4 lg:p-6 space-y-4">
                                            <div className="grid gap-3 md:grid-cols-[1.5fr,1fr] items-start">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-xs text-muted-foreground">Formula: {order.bom.name}</p>
                                                    ...
                                                    <div className="flex flex-wrap gap-2 md:justify-end">
                                                        {order.machine ? (
                                                            ...
                                                            <div className="flex items-center gap-2 text-red-600 font-semibold bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-full border border-red-100 dark:border-red-800/30">
                                                                <Factory className="h-3 w-3" /> NO MACHINE
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="text-[11px] text-muted-foreground">
                                                        Qty: {Number(order.plannedQuantity).toLocaleString()} {order.bom.productVariant.primaryUnit}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
                                                <div className="flex items-center justify-between gap-3 mb-1.5">
                                                    <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Shift Kerja</div>
                                                ...
                                                {order.shifts.length === 0 ? (
                                                    <div className="text-[11px] text-muted-foreground">
                                                        Belum ada tim. Klik <span className="font-semibold">Add Shift</span> untuk atur jadwal.
                                                    </div>
                                                ) : (
                                                    <div className="space-y-1.5">
                                                        {order.shifts.slice(0, 2).map((shift) => (
                                                            <div key={shift.id} className="flex items-center justify-between text-xs p-1.5 rounded bg-slate-50 dark:bg-slate-800/30">
                                                                <div className="flex items-center gap-1.5">
                                                                    ...
                                                                    <span className="font-medium">{shift.operator?.name || 'Unassigned'}</span>
                                                                </div>
                                                                <span className="text-muted-foreground text-[10px]">{format(new Date(shift.startTime), 'HH:mm')}</span>
                                                            </div>
                                                        ))}
                                                        {order.shifts.length > 2 && (
                                                            <div className="text-[10px] text-muted-foreground">+{order.shifts.length - 2} more shifts</div>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center justify-between gap-2 pt-2">
                                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                    <User size={14} />
                                                    <span>{order.executions[0]?.operator?.name || 'Unassigned'}</span>
                                                </div>
                ...
                <div className="space-y-6">
                    <Card>
                        ...
                        <CardContent className="grid gap-3">
                            ...
                            <Button className="w-full justify-start text-xs h-9" variant="outline">
                                <Clock className="mr-2 h-4 w-4" /> Reschedule Order
                            </Button>
                            <Button className="w-full justify-start text-xs h-9" variant="outline">
                                <User className="mr-2 h-4 w-4" /> Change Team / Shift
                            </Button>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-slate-900 text-white">
                        <CardHeader>
                            <CardTitle className="text-sm">Shift Handover Note</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-[11px] text-slate-400 italic">
                                &quot;Mesin EXT-01 agak lambat di heater no 3. Sudah dilaporkan ke maintenance.&quot;
                            </p>
                            <Button variant="link" className="text-sky-400 text-xs p-0 mt-2 h-auto">View All Notes</Button>
                        </CardContent>
                    </Card>
                </div>
            </div>
