...
export default async function PackingMonthlyReportPage({
    searchParams,
}: PageProps) {
    ...
    const totalKarungConsumed = data.reduce(
        (sum, item) => sum + item.karungConsumed,
        0,
    );
    const totalKarungCost = data.reduce(
        (sum, item) => sum + item.karungCost,
        0,
    );
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground">
                        Laporan Produksi Packing
                    </h1>
                    <p className="text-muted-foreground text-sm">
                        Laporan total hasil produksi barang jadi
                        {showCostData ? ' dan HPP ' : ' '}dari Packing Area
                        untuk periode {monthName}.
                    </p>
                </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <h3 className="text-2xl font-bold text-foreground mt-1">
                                    {totalKarungConsumed.toLocaleString(
                                        'id-ID',
                                    )}
                                </h3>
                            </div>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                        <CardTitle className="text-lg font-bold">
                            Rincian Hasil Packing
                        </CardTitle>
                        <p className="text-xs text-muted-foreground mt-1">
                            Akumulasi hasil produksi
                            {showCostData
                                ? ', karung terpakai, dan HPP'
                                : ' dan karung terpakai'}{' '}
                            per varian barang jadi.
                        </p>
                    </div>
                </CardHeader>
                <CardContent>
                                    ...
                                    data.map((item) => (
                                        ...
                                        >
                                            ...
                                            <TableCell className="text-right font-bold text-sm text-foreground">
                                                ...
                                                <span className="text-[10px] ml-1 text-muted-foreground font-normal uppercase">
                                                    {item.primaryUnit}
                                                </span>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right text-sm text-foreground">
                                                {item.karungConsumed.toLocaleString(
                                                    'id-ID',
                                                )}
                                            </TableCell>
                                            {showCostData && (
                                            ...
                                            {showCostData && (
                                                <TableCell className="text-right">
                                                    <Badge
                                                        variant="outline"
                                                        className="border-emerald-200 dark:border-emerald-900/50 bg-emerald-50/30 text-emerald-700 dark:text-emerald-400 font-semibold text-xs"
                                                    >
                                                        {formatRupiah(
                                                            item.averageHpp,
                                                        )}
