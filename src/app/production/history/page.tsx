...
export default async function ProductionHistoryPage({
    ...
    const machineId =
        typeof params.machineId === 'string' ? params.machineId : undefined;
    const operatorId =
        typeof params.operatorId === 'string' ? params.operatorId : undefined;
    const shiftId =
        typeof params.shiftId === 'string' ? params.shiftId : undefined;
    const productVariantId =
        typeof params.productVariantId === 'string'
    ...
    const rawData =
        historyResult.success && historyResult.data ? historyResult.data : null;
    const groups = rawData?.groups ?? [];
    const summary = (rawData?.summary ??
        defaultSummary) as ProductionHistorySummary;
    const filterOptions =
        filterOptionsResult.success && filterOptionsResult.data
            ? filterOptionsResult.data
            : { machines: [], operators: [], shifts: [], products: [] };
    ...
    const fromToIsDefault = !from && !to;
    const scrapPct =
        summary.totalGood + summary.totalScrap > 0
            ? (
                  (summary.totalScrap /
                      (summary.totalGood + summary.totalScrap)) *
                  100
              ).toFixed(1)
            : '0.0';
    const kpiCaption = hasActiveFilter
        ? 'Dari filter aktif'
        : fromToIsDefault
          ? 'Hari ini WIB'
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Dokumentasi output per entri — verifikasi, bukti foto,
                        koreksi.
                    </p>
                </div>
            </div>
            ...
            {summary.isTruncated && (
                <p className="text-xs text-muted-foreground text-center -mt-2">
                    Menampilkan {summary.executionCount} entri teratas (batas{' '}
                    {summary.limit}). Ubah filter untuk menyempitkan.
                </p>
            )}
