...
const getReport = withTenantPage(async (from: string, to: string) => {
    ...
    return RekapDagangService.getHutangRecap({ from, to });
});
...
export default async function RekapHutangPage({ searchParams }: PageProps) {
    ...
    return (
        <RecapReportView
            title="Rekap Hutang Dagang"
            description="Mutasi masuk (invoice pembelian), mutasi keluar (pembayaran), dan saldo per supplier."
