...
export default function BudgetVariancePage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (result && 'success' in result && result.success) {
                ...
                console.error(
                    'Failed to load variance report:',
    ...
    const handleDownload = () => {
        ...
        downloadCsv(
            reportFilename('Budget_Variance', `${year}-${month}`),
            headers,
            rows,
        );
    };
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">
                        {reportLabels.variansAnggaran}
                    </h1>
                    <p className="text-muted-foreground">
                        {reportLabels.variansAnggaranDesc}{' '}
                            ...
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">
                                        {reportLabels.anggaran}
                                    </TableHead>
                                    <TableHead className="text-right">
                                        {reportLabels.aktual}
                                    </TableHead>
                                    <TableHead className="text-right">
                                        {reportLabels.varians}
                                    </TableHead>
                                    <TableHead className="text-right">
                                        %
                                    </TableHead>
                                ...
                                {loading ? (
                                    ...
                                    <TableRow>
                                        <TableCell
                                            ...
                                            className="h-24 text-center"
                                        >
                                            Tidak ada budget untuk periode ini.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    data.map((item) => {
                                        ...
                                        return (
                                            <TableRow key={item.accountCode}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-bold ${isOver ? 'text-blue-600' : isUnder ? 'text-amber-600' : ''}`}
                                                >
                                                    {formatRupiah(
                                                        item.variance,
                                                    )}
