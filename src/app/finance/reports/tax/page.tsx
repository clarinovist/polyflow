...
export default function TaxReportPage() {
  ...
  const handleDownload = () => {
    ...
    const rows: (string | number)[][] = [
      ["PPN Keluaran", rupiahForCsv(data.vatOutputBalance)],
      ["PPN Masukan", rupiahForCsv(data.vatInputBalance)],
      ["PPN Terutang (Net)", rupiahForCsv(data.netVatPayable)],
      ["PPh 21 Terutang", rupiahForCsv(data.incomeTaxPayable)],
    ];
    const dateStr = format(date, "yyyy-MM");
    downloadCsv(reportFilename("Laporan_Pajak", dateStr), headers, rows);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Ringkasan PPN Keluaran, PPN Masukan, dan PPh 21 Terutang untuk
        ...
        <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="flex justify-between items-center border-b pb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Pajak yang dipungut dari penjualan
                  </p>
                </div>
                <div className="text-lg font-mono">
                  {formatRupiah(Math.max(0, data.vatOutputBalance))}
                </div>
              </div>
              <div className="flex justify-between items-center border-b pb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Pajak yang dibayarkan dari pembelian
                  </p>
                </div>
                <div className="text-lg font-mono">
                  ({formatRupiah(Math.abs(data.vatInputBalance))})
                </div>
              </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {data.netVatPayable > 0
                      ? "Pajak Kurang Bayar (Payable)"
                      : "Pajak Lebih Bayar (Refundable)"}
                  </p>
                </div>
                <div
                  className={`text-2xl font-bold ${data.netVatPayable > 0 ? "text-destructive" : "text-emerald-600"}`}
                >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Ringkasan kewajiban PPh 21</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center border-b pb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Pajak penghasilan yang dipotong dari gaji
                  </p>
                </div>
                <div className="text-lg font-mono">
