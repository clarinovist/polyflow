...
export default function FOHAllocationPage() {
    ...
    useEffect(() => {
        ...
        const fetchAllocation = async () => {
            setLoading(true);
            try {
                const res = await getFOHAllocation(
                    year,
                    month,
                    selectedAccount,
                );
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Distribusikan biaya overhead ke order produksi
                        berdasarkan jumlah
                    </p>
                </div>
            </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Total Overhead:{' '}
                        <strong>
                            {data ? formatRupiah(data.totalOverhead) : '-'}
                        </strong>{' '}
                        | Total Unit Diproduksi:{' '}
                        <strong>
                            {data
                                ? data.totalQuantity.toLocaleString('id-ID')
                                : '-'}
                        </strong>
                    </CardDescription>
                </CardHeader>
                                    ...
                                    {data.allocations.map((row, i: number) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-4 align-middle font-mono text-right">
                                                {row.actualQuantity.toLocaleString(
                                                    'id-ID',
                                                )}
                                            </td>
                                            <td className="p-4 align-middle font-mono text-right">
                                                {(
                                                    row.allocationRatio * 100
                                                ).toFixed(2)}
                                                %
                                            </td>
