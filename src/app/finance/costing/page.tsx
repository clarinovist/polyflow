...
import { buildCostAuditRows, summarizeCostAuditRows } from '@/lib/utils/cost-audit';
import { formatCostGapLabel, getCostAlertShortLabel, getCostSourceLabel } from '@/lib/utils/cost-diagnostics';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { AlertTriangle, Calculator, FileText, Filter, Package, Settings, TrendingUp, Users } from 'lucide-react';
import Link from 'next/link';

const SOURCE_OPTIONS = ['all', 'inventory_average', 'standard_cost', 'explicit_current_cost', 'buy_price', 'price', 'zero'] as const;
const FLAG_OPTIONS = ['all', 'inventory_standard_gap', 'low_stock_cost_outlier'] as const;
...
function parseSourceFilter(value?: string): SourceFilter {
    return SOURCE_OPTIONS.some((option) => option === value) ? (value as SourceFilter) : 'all';
}

function parseFlagFilter(value?: string): FlagFilter {
    return FLAG_OPTIONS.some((option) => option === value) ? (value as FlagFilter) : 'all';
}

export default async function CostingPage(props: {
    searchParams: Promise<{ start?: string; end?: string; anomaly?: string; source?: string; flag?: string }>;
}) {
    ...
    const variants = variantsRes.success && variantsRes.data ? variantsRes.data : [];
    ...
    const buildFilterHref = (overrides: Partial<{ anomaly: string | undefined; source: SourceFilter; flag: FlagFilter }>) => {
        const params = new URLSearchParams();

        if (searchParams.start) params.set('start', searchParams.start);
        if (searchParams.end) params.set('end', searchParams.end);

        const nextAnomaly = overrides.anomaly !== undefined ? overrides.anomaly : (anomalyOnly ? '1' : undefined);
            ...
            <Card className="border-amber-200/70">
                <CardHeader>
                    <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Review variant cost basis, gap vs standard, dan anomaly signal untuk cari penyebab selisih antar family/ukuran.
                            </CardDescription>
                        </div>
                        <div className="text-sm text-muted-foreground">
                            Showing {filteredAuditRows.length} of {allAuditRows.length} variants
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        <Card>
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium">Variants Scanned</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold">{auditSummary.totalVariants}</div>
                                <p className="text-xs text-muted-foreground mt-1">Semua variant dengan cost basis yang bisa diaudit</p>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="pb-2">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-amber-600">{auditSummary.reviewNeeded}</div>
                                <p className="text-xs text-muted-foreground mt-1">Variant punya minimal satu anomaly flag</p>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium">Low Stock Outlier</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-red-600">{auditSummary.lowStockOutliers}</div>
                                <p className="text-xs text-muted-foreground mt-1">Biasanya residue stock tipis / sisa cost lama</p>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium">Inventory Avg Basis</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold">{auditSummary.inventoryAverageCount}</div>
                                <p className="text-xs text-muted-foreground mt-1">
                                    {auditSummary.standardFallbackCount} variant masih fallback ke standard cost
                                </p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        <Button variant={!anomalyOnly && sourceFilter === 'all' && flagFilter === 'all' ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ anomaly: undefined, source: 'all', flag: 'all' })}>All Variants</Link>
                        </Button>
                        <Button variant={anomalyOnly ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ anomaly: anomalyOnly ? undefined : '1', source: 'all', flag: 'all' })}>Review Needed</Link>
                        </Button>
                        <Button variant={sourceFilter === 'inventory_average' ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ source: 'inventory_average', anomaly: undefined, flag: 'all' })}>Inventory Avg</Link>
                        </Button>
                        <Button variant={sourceFilter === 'standard_cost' ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ source: 'standard_cost', anomaly: undefined, flag: 'all' })}>Standard Fallback</Link>
                        </Button>
                        <Button variant={flagFilter === 'low_stock_cost_outlier' ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ flag: 'low_stock_cost_outlier', anomaly: '1', source: 'all' })}>Low Stock Outlier</Link>
                        </Button>
                        <Button variant={flagFilter === 'inventory_standard_gap' ? 'default' : 'outline'} asChild>
                            <Link href={buildFilterHref({ flag: 'inventory_standard_gap', anomaly: '1', source: 'all' })}>Std Gap</Link>
                        </Button>
                    </div>
                            ...
                            <TableBody>
                                {filteredAuditRows.length === 0 ? (
                                    ...
                                    filteredAuditRows.map((row) => {
                                        const gapLabel = formatCostGapLabel(row.currentCost, row.standardCost);

                                        return (
                                            <TableRow key={row.variantId} className={row.flags.length > 0 ? 'bg-amber-50/40 dark:bg-amber-950/10' : ''}>
                                                <TableCell>
                                                ...
                                                <TableCell className="text-right font-medium">{formatRupiah(row.currentCost)}</TableCell>
                                                <TableCell className="text-right text-muted-foreground">{formatRupiah(row.standardCost)}</TableCell>
                                                <TableCell>
                                                    <Badge variant="outline">{getCostSourceLabel(row.source)}</Badge>
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    <div className="flex flex-col items-end">
                                                        <span>{row.stockQty.toLocaleString('id-ID')}</span>
                                                        <span className="text-[10px] text-muted-foreground">{formatRupiah(row.stockValue)} value</span>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {gapLabel ? (
                                                        <Badge variant={row.flags.length > 0 ? 'destructive' : 'outline'}>{gapLabel}</Badge>
                                                    ) : (
                                                ...
                                                <TableCell>
                                                    <div className="flex flex-wrap gap-1">
                                                        <Badge variant={row.flags.length > 0 ? 'destructive' : 'secondary'}>
                                                            {row.health === 'review_needed' ? 'Review Needed' : 'Within Range'}
                                                        </Badge>
                                                        {row.flags.map((flag) => (
                                                            <Badge key={`${row.variantId}-${flag}`} variant="outline">
