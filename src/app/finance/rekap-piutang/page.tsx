...
const getReport = withTenantPage(async (from: string, to: string) => {
    ...
    return RekapDagangService.getPiutangRecap({ from, to });
});
...
export default async function RekapPiutangPage({ searchParams }: PageProps) {
    ...
    return (
        <RecapReportView
            title="Rekap Piutang Dagang"
            description="Mutasi masuk (invoice sales), mutasi keluar (penerimaan pembayaran), dan saldo per customer."
            ...
            filter={
                <RecapDateFilter
                    basePath="/finance/rekap-piutang"
                    from={from}
                    to={to}
                />
            }
