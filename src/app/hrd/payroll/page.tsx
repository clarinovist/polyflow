            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        ...
                        . Harian dari absensi · Borongan dari output operator ·
                        BPJS di minggu terakhir bulan
                        {weekStart && weekEnd
                            ? ` · ${weekLabel(new Date(weekStart), new Date(weekEnd))}`
                            : ''}
                    </p>
                </div>
            ...
            <Card className="bg-background/40 backdrop-blur-xl border-white/10 shadow-xl border-0 overflow-hidden">
                ...
                <CardContent className="p-0">
                            ...
                            <tbody>
                                ...
                                {rows.map((r) => (
                                    <tr key={r.employeeId} className="border-t">
                                        ...
                                        <td className="p-3 text-right">
                                            {formatIdr(r.totalDailyEarnings)}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(r.totalOvertimeEarnings)}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(r.totalPieceEarnings)}
                                        </td>
                                        <td className="p-3 text-right">
                            ...
                            {rows.length > 0 && (
                                <tfoot>
                                    <tr className="border-t-2 bg-muted/30 font-semibold">
                                        ...
                                        <td className="p-3 text-right">
                                            {formatIdr(
                                                totals.totalDailyEarnings,
                                            )}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(
                                                totals.totalOvertimeEarnings,
                                            )}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(
                                                totals.totalPieceEarnings,
                                            )}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(totals.totalEarnings)}
                                        </td>
                                        <td className="p-3 text-right text-red-600">
                                            {totals.bpjsDeduction > 0
                                                ? `-${formatIdr(totals.bpjsDeduction)}`
                                                : '—'}
                                        </td>
                                        <td className="p-3 text-right">
                                            {formatIdr(totals.netPay)}
                                        </td>
                                    </tr>
                                </tfoot>
                            )}
                    ...
                    <p className="p-3 text-xs text-muted-foreground border-t">
                        Exception kg = output tanpa absensi PRESENT di hari yang
                        sama, atau rate proses belum di-set. BPJS potongan
                        bulanan penuh hanya di minggu yang memuat akhir bulan
                        (harian/borongan).{' '}
