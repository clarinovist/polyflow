...
const BrandCard = React.forwardRef<HTMLDivElement, BrandCardProps>(
    (
        { className, variant = 'default', isInteractive = false, ...props },
        ref,
    ) => {
        const variants = {
            default:
                'bg-brand-glass backdrop-blur-brand border-brand shadow-xl',
        ...
        return (
            <div
                ...
                className={cn(
                    ...
                    variants[variant],
                    isInteractive &&
                        'hover:bg-brand-glass/20 hover:scale-[1.01] active:scale-[0.99] cursor-pointer',
                    className,
                )}
                {...props}
            />
...
>(({ className, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            'p-8 pb-4 flex items-center justify-between border-b border-brand',
            className,
        )}
        {...props}
    />
