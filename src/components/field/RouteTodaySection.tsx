...
export function RouteTodaySection({
    ...
    const stats = useMemo(() => {
        ...
        const completed = journeyPlan.filter(
            (j) => j.status === 'COMPLETED',
        ).length;
        const visiting = journeyPlan.filter(
            (j) => j.status === 'VISITING',
        ).length;
                ...
                {journeyPlan.length > 0 ? (
                    ...
                    <div className="p-4 rounded-xl bg-muted/30 text-center">
                        ...
                        <p className="text-sm font-medium text-muted-foreground">
                            Belum ada rute dari admin
                        </p>
                        <p className="text-[11px] text-muted-foreground/70 mt-1">
                            Hubungi supervisor untuk mengatur rute harian
                        </p>
                    </div>
                )}
            </div>
                            ...
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <h4
                                            className={`font-semibold text-sm truncate ${isCompleted ? 'text-muted-foreground line-through' : 'text-foreground'}`}
                                        >
                                            {item.name}
                                        </h4>
