...
import {
    AccountingInput,
    formatAccounting,
} from '@/components/finance/accounting/accounting-input';
import { Badge } from '@/components/ui/badge';
import { toast } from 'sonner';
import {
    saveUnifiedOpeningBalance,
    CreateOpeningBalanceInput,
} from '@/actions/finance/opening-balance';
...
export function OpeningBalanceSpreadsheet({
    accounts,
    customers,
    suppliers,
    arAccountId,
    apAccountId,
}: OpeningBalanceSpreadsheetProps) {
    const [date, setDate] = React.useState<Date>(new Date());
    const [isSubmitting, setIsSubmitting] = React.useState(false);

    // General Lines State
    const [generalLines, setGeneralLines] = React.useState<
        Record<string, GeneralLine>
    >({});

    // AR/AP State
    const [arEntries, setArEntries] = React.useState<
        CreateOpeningBalanceInput[]
    >([]);
    const [apEntries, setApEntries] = React.useState<
        CreateOpeningBalanceInput[]
    >([]);
    ...
    const [tempInvoice, setTempInvoice] = React.useState<
        Partial<CreateOpeningBalanceInput>
    >({
        date: new Date(),
        dueDate: new Date(),
        amount: 0,
    ...
    const handleGeneralChange = (
        ...
        setGeneralLines((prev) => ({
            ...
            [accountId]: {
                ...
                credit:
                    field === 'credit' ? value : prev[accountId]?.credit || 0,
            },
        }));
    };

    // Calculate Totals
    const generalDebit = Object.values(generalLines).reduce(
        (sum, l) => sum + l.debit,
        0,
    );
    const generalCredit = Object.values(generalLines).reduce(
        (sum, l) => sum + l.credit,
        0,
    );
    ...
    const handleSave = async () => {
        ...
        try {
            const formattedGeneralLines = Object.values(generalLines).filter(
            ...
            const result = await saveUnifiedOpeningBalance({
                ...
                arEntries: arEntries.map((e) => ({
                    ...e,
                    date,
                    dueDate: date,
                })),
                apEntries: apEntries.map((e) => ({
                    ...e,
                    date,
                    dueDate: date,
                })),
    ...
    const renderAccountRow = (account: Account) => {
        ...
        if (isAR) {
            return (
                <TableRow
                    key={account.id}
                    className="bg-blue-500/5 hover:bg-blue-500/10 border-l-4 border-l-blue-500/50"
                >
                    ...
                    <TableCell>
                        <div className="flex items-center gap-2">
                            ...
                            <Badge
                                variant="secondary"
                                className="text-[10px] h-4 uppercase tracking-wider bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20"
                            >
                                Control Account
                            </Badge>
                        </div>
                    ...
                    <TableCell colSpan={2}>
                        ...
                        >
                            <span className="text-xs">
                                Manage Outstanding Invoices ({arEntries.length})
                            </span>
        ...
        if (isAP) {
            return (
                <TableRow
                    key={account.id}
                    className="bg-orange-500/5 hover:bg-orange-500/10 border-l-4 border-l-orange-500/50"
                >
                    <TableCell className="font-mono text-xs text-muted-foreground">
                        {account.code}
                    </TableCell>
                    <TableCell>
                        <div className="flex items-center gap-2">
                            ...
                            <Badge
                                variant="secondary"
                                className="text-[10px] h-4 uppercase tracking-wider bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20"
                            >
                                Control Account
                            </Badge>
                        </div>
                    ...
                    <TableCell colSpan={2}>
                        ...
                        >
                            <span className="text-xs">
                                Manage Unpaid Bills ({apEntries.length})
                            </span>
                            <span className="font-mono font-bold">
        ...
        return (
            <TableRow
                key={account.id}
                className={hasValue ? 'bg-muted/30' : ''}
            >
                <TableCell className="font-mono text-xs text-muted-foreground">
                    {account.code}
                </TableCell>
                ...
                <TableCell>
                    <AccountingInput
                        value={line.debit}
                        onValueChange={(val) =>
                            handleGeneralChange(account.id, 'debit', val)
                        }
                ...
                <TableCell>
                    <AccountingInput
                        value={line.credit}
                        onValueChange={(val) =>
                            handleGeneralChange(account.id, 'credit', val)
                        }
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                    <div className="space-y-1">
                        ...
                        <CardDescription>
                            Enter initial balances for all accounts via one
                            journal.
                        </CardDescription>
                    </div>
                    ...
                    <CardContent className="p-3">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                            <div className="space-y-0.5">
                                <Label className="text-[10px] text-muted-foreground block uppercase tracking-wider">
                                    Total Debit
                                </Label>
                                <div className="text-lg font-mono font-bold">
                                    {formatAccounting(totalDebit)}
                                </div>
                            </div>
                            <div className="space-y-0.5">
                                <Label className="text-[10px] text-muted-foreground block uppercase tracking-wider">
                            ...
                            <div className="space-y-0.5">
                                ...
                                >
                                    {formatAccounting(Math.abs(equityOffset))}{' '}
                                    {equityOffset > 0
                                        ? 'Cr'
                                        : equityOffset < 0
                                          ? 'Dr'
            ...
            <Dialog open={isArDialogOpen} onOpenChange={setIsArDialogOpen}>
                <DialogContent className="max-w-3xl">
                    <DialogHeader>
                        <DialogTitle>
                            Manage Outstanding Invoices (Piutang)
                        </DialogTitle>
                        <DialogDescription>
                            Add valid unpaid invoices from customers.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-12 gap-2 items-end border-b pb-4">
                            ...
                            <div className="col-span-3">
                                <Button onClick={addArEntry} className="w-full">
                    ...
                    <DialogFooter>
                        <div className="mr-auto text-sm text-muted-foreground flex items-center">
                            ...
                            <span className="ml-2 font-mono font-bold text-foreground">
                                {formatAccounting(arTotal)}
                            </span>
                        </div>
                        <Button onClick={() => setIsArDialogOpen(false)}>
                            Done
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <Dialog open={isApDialogOpen} onOpenChange={setIsApDialogOpen}>
                <DialogContent className="max-w-3xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Add valid unpaid bills from suppliers.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-12 gap-2 items-end border-b pb-4">
                            ...
                            <div className="col-span-3">
                                <Button onClick={addApEntry} className="w-full">
                                    <Plus className="mr-2 h-4 w-4" /> Add
                                </Button>
                            </div>
                        </div>
                    ...
                    <DialogFooter>
                        <div className="mr-auto text-sm text-muted-foreground flex items-center">
                            Total Payables:{' '}
                            <span className="ml-2 font-mono font-bold text-foreground">
                                {formatAccounting(apTotal)}
                            </span>
                        </div>
                        <Button onClick={() => setIsApDialogOpen(false)}>
                            Done
                        </Button>
                    </DialogFooter>
                </DialogContent>
