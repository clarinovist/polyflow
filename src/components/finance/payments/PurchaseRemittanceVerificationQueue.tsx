...
type PurchaseRemittanceItemRow = {
    id: string;
...
export type PurchaseRemittanceQueueRow = {
    id: string;
    ...
    items: PurchaseRemittanceItemRow[];
};
...
interface PurchaseRemittanceVerificationQueueProps {
    initialRemittances: PurchaseRemittanceQueueRow[];
}

export function PurchaseRemittanceVerificationQueue({
    initialRemittances,
}: PurchaseRemittanceVerificationQueueProps) {
    const [remittances, setRemittances] =
        useState<PurchaseRemittanceQueueRow[]>(initialRemittances);
    ...
    const refresh = async () => {
        try {
            ...
            if (res?.success && res.data) {
                setRemittances(
                    res.data as unknown as PurchaseRemittanceQueueRow[],
                );
            }
        } catch {
    ...
    const handleVerify = async (remittanceId: string) => {
        ...
        try {
            const result = await verifyPurchaseRemittanceAction({
                remittanceId,
            });
            if (result.success) {
    ...
    const handleReject = async (remittanceId: string) => {
        ...
        setProcessingId(remittanceId);
        try {
            const result = await rejectPurchaseRemittanceAction({
                remittanceId,
        ...
        return (
            <Card>
                <CardContent className="p-8 text-center text-sm text-muted-foreground">
                    Tidak ada setoran pembayaran supplier yang menunggu
                    verifikasi.
                </CardContent>
            </Card>
        );
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {fmtDate(r.paidAt)} — diajukan oleh{' '}
