    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast({
                title: 'Gagal',
                description:
                    'Gagal mencatat pembayaran supplier. Silakan coba lagi.',
                variant: 'destructive',
            });
        } finally {
            setLoading(false);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Catat pembayaran yang dikirim ke supplier untuk invoice
                        mereka.
                    </DialogDescription>
                </DialogHeader>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex flex-col min-w-0 flex-1">
                                                            <span
                                                                className={`truncate font-medium flex items-center gap-1 ${overdue ? 'text-red-700' : ''}`}
                                                            >
                                                                {
                                                                    inv.invoiceNumber
                                                                }
                        ...
                        <div className="p-3 bg-muted rounded-md text-sm space-y-1">
                            ...
                            {selectedInvoice.dueDate && (
                                <div className="flex justify-between">
                                    ...
                                    <span
                                        className={`font-medium ${isInvoiceOverdue(selectedInvoice.dueDate, selectedInvoice.status) ? 'text-red-600 font-bold' : ''}`}
                                    >
                                        {format(
                                            new Date(selectedInvoice.dueDate),
                                            'dd MMM yyyy',
                                        )}
                                        {selectedInvoice.termOfPaymentDays !=
                                            null &&
                                            ` (${selectedInvoice.termOfPaymentDays === 0 ? 'Cash' : `${selectedInvoice.termOfPaymentDays} hari`})`}
                                    </span>
                                </div>
                            )}
                        ...
                        {parseFloat(amount) > remainingBalance &&
                            remainingBalance > 0 && (
                                <p className="text-xs text-red-500 mt-1">
                                    ⚠️ Jumlah melebihi sisa tagihan (
                                    {formatRupiah(remainingBalance)})
                                </p>
                            )}
                    </div>
