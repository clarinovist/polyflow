...
import {
    createDetailJournalAction,
    updateDetailJournalAction,
...
export default function DetailJournalForm({
    ...
    const initialTemplateKey = (defaultValues?.templateKey ||
        defaultValues?.type ||
        'DIRECT_LABOR') as DetailJournalTemplateKey;
    const [selectedTemplateKey, setSelectedTemplateKey] =
        useState<string>(initialTemplateKey);
    const template =
        DETAIL_JOURNAL_TEMPLATES[
            selectedTemplateKey as DetailJournalTemplateKey
        ];

    // Resolve default accounts from template
    const defaultPrimaryId = useMemo(() => {
        if (mode === 'edit' && defaultValues?.primaryAccountId)
            return defaultValues.primaryAccountId;
        return template
            ? findDefaultAccountId(
                  accounts,
                  template.primaryAccountCodes,
                  template.primaryNameHints,
              ) || ''
            : '';
    }, [template, accounts, mode, defaultValues?.primaryAccountId]);

    const defaultCounterId = useMemo(() => {
        if (mode === 'edit' && defaultValues?.counterAccountId)
            return defaultValues.counterAccountId;
        return template
            ? findDefaultAccountId(
                  accounts,
                  template.counterAccountCodes,
                  template.counterNameHints,
              ) || ''
            : '';
    ...
    const form = useForm<DetailJournalValues>({
        resolver: zodResolver(
            detailJournalSchema,
        ) as Resolver<DetailJournalValues>,
    ...
    const previewLines = useMemo(() => {
        if (totalDetail <= 0 || !primaryAccountId || !counterAccountId)
            return [];
        ...
        return [
            {
                ...
                credit: 0,
                description:
                    watch('description') || template?.label || 'Detail Journal',
            },
            {
                accountId: primaryAccountId,
                debit: 0,
    ...
    function handleTemplateChange(newKey: string) {
        const newTemplate =
            DETAIL_JOURNAL_TEMPLATES[newKey as DetailJournalTemplateKey];
        ...
        const primaryId =
            findDefaultAccountId(
                accounts,
                newTemplate.primaryAccountCodes,
                newTemplate.primaryNameHints,
            ) || '';
        const counterId =
            findDefaultAccountId(
                accounts,
                newTemplate.counterAccountCodes,
                newTemplate.counterNameHints,
            ) || '';
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>
                                        {template?.detailNamePlaceholder ||
                                            'Nama / Keterangan'}
                                    </TableHead>
