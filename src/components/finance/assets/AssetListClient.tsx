...
export function AssetListClient({
    ...
    const summary = useMemo(() => {
        ...
        const totalBookValue = data.reduce((sum, a) => {
            ...
            const accumulated = Math.min(
                monthlyDepr * monthsElapsed,
                value - scrap,
            );
            return sum + (value - accumulated);
        }, 0);

        const depreciatedCount = data.filter(
            (a) => a.lastDepreciationDate,
        ).length;
    ...
    const columns: ColumnDef<AssetWithAccounts>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) =>
                format(new Date(row.getValue('purchaseDate')), 'PP'),
        },
        {
            accessorKey: 'purchaseValue',
            header: 'Value',
            cell: ({ row }) =>
                formatRupiah(Number(row.getValue('purchaseValue'))),
        },
        {
        ...
        },
        {
            ...
            header: 'Sumber',
            cell: ({ row }) => {
                const { purchaseOrderNumber, goodsReceiptNumber } =
                    row.original;
