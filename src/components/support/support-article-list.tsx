...
export function SupportModuleChips({
    module,
    basePath,
}: {
    module?: string;
...
export function SupportArticleGrid({
    articles,
    from,
}: {
    ...
    return (
        <div className="grid gap-3 sm:grid-cols-2">
            {articles.map((article) => (
                <Link
                    key={article.id}
                    href={`/support/${article.slug}${from ? `?from=${from}` : ''}`}
                >
        ...
        <Card>
            <CardContent className="py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Coba tanyakan ke Virtual CS—langsung jawab pakai panduan
                    yang ada.
                </p>
                <div className="mt-4 flex justify-center gap-2">
                    <Link href="/support/cs">
                        <Button size="sm" className="gap-1.5">
                            <MessageCircle className="h-4 w-4" /> Tanya Virtual
                            CS
                        </Button>
                    </Link>
                </div>
