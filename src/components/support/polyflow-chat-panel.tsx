    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        if (trimmed.startsWith('## ')) {
            elements.push(
                <h4
                    key={idx}
                    className="font-bold text-sm tracking-tight text-foreground mt-3 mb-1"
                >
            ...
            elements.push(
                <h5
                    key={idx}
                    className="font-semibold text-xs tracking-tight text-foreground mt-2 mb-1"
                >
    ...
    const sendQuestion = async (incoming?: string) => {
        ...
        try {
            ...
            if ((err as Error).name === 'AbortError') {
                ...
                pushMessage(
                    'assistant',
                    'Koneksi ke server terputus. Silakan periksa jaringan dan coba lagi.',
                );
            }
        } finally {
            setIsLoading(false);
                ...
                <form onSubmit={onSubmit} className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mt-2 px-2 text-[11px] text-muted-foreground">
                        <div className="flex items-center gap-2">
                            <span>
                                Tekan{' '}
                                <kbd className="px-1.5 py-0.5 rounded border border-border bg-muted/60 font-mono text-[10px]">
                                    Enter ↵
                                </kbd>{' '}
                                untuk kirim
                            </span>
