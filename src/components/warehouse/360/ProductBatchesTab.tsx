...
export function ProductBatchesTab({
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await listBatchesByProductVariant(productVariantId);
        setRows(res.success ? ((res.data as unknown as Batch[]) ?? []) : []);
        setLoading(false);
    }, [productVariantId]);
    useEffect(() => {
        load();
                                        ...
                                        <TableRow key={b.id}>
                                            ...
                                            <TableCell>
                                                {days != null ? (
                                                    <Badge
                                                        variant="outline"
                                                        className={`text-[10px] ${days < 0 ? 'bg-rose-500/10 text-rose-700' : days <= 30 ? 'bg-amber-500/10 text-amber-700' : 'bg-emerald-500/10 text-emerald-700'}`}
                                                    >
                                                        {days < 0
                                                            ? `Exp ${Math.abs(days)} hari lalu`
                                                            : `${days} hari`}
                                                    </Badge>
                                                ) : (
                                                    '-'
