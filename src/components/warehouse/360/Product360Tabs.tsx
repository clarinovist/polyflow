...
interface Props {
    ...
    overview?: {
        variant: {
            ...
            reorderQuantity?: { toNumber(): number } | number | null;
...
export function Product360Tabs({
    productVariantId,
    ledgerData,
    locations,
    overview,
        ...
        <div className="p-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
                        ...
                        {variant
                            ? 'name' in variant
                                ? (variant as { skuCode: string; name: string })
                                      .name
                                : (variant as { skuCode: string; name: string })
                                      .name
                    ...
                    <div className="flex gap-2 mt-1">
                        ...
                        {overview && (
                            <Badge variant="secondary" className="text-[10px]">
                                Stock {formatQuantity(overview.totalQty)} ·
                                Value {fmtIdr(overview.totalValue)}
                            </Badge>
                        )}
                    </div>
                </div>
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange}>
                <TabsList className="flex h-auto gap-1 overflow-x-auto scrollbar-none justify-start">
                    ...
                    >
                        <Bookmark className="h-3 w-3" /> Ringkas
                    </TabsTrigger>
                    <TabsTrigger
                        value="stock"
                        className="text-xs gap-1 shrink-0"
                    >
                        <Warehouse className="h-3 w-3" /> Lokasi
                    </TabsTrigger>
                    <TabsTrigger
                        value="ledger"
                        className="text-xs gap-1 shrink-0"
                    ...
                    <TabsTrigger value="bom" className="text-xs gap-1 shrink-0">
                        <Beaker className="h-3 w-3" /> BOM
                    </TabsTrigger>
                    <TabsTrigger
                        value="cost"
                        className="text-xs gap-1 shrink-0"
                    ...
                    <TabsTrigger
                        ...
                        className="text-xs gap-1 shrink-0"
                    >
                        <ListOrdered className="h-3 w-3" /> Reservasi
                    </TabsTrigger>
                    <TabsTrigger
                        value="movements"
                        className="text-xs gap-1 shrink-0"
                ...
                <div className="mt-4">
                                ...
                                {overview ? (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <p className="font-mono">
                                                {overview.variant.minStockAlert
                                                    ? formatQuantity(
                                                          toN(
                                                              overview.variant
                                                                  .minStockAlert,
                                        ...
                                        <div className="md:col-span-2 pt-2 border-t flex gap-4">
                                            <div>
                                                <p className="text-xs text-muted-foreground">
                                                    Total Qty Semua Lokasi
                                                </p>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Tidak ada overview detail (ledger only
                                        mode).
                                    </p>
                                )}
                            </CardContent>
                    ...
                    {activeTab === 'bom' && (
                        <ProductBomTab productVariantId={productVariantId} />
                    )}
                    {activeTab === 'cost' && (
                        <ProductCostHistoryTab
                            productVariantId={productVariantId}
                        />
                    )}
                    {activeTab === 'reservations' && (
                        <ProductReservationsTab
                            productVariantId={productVariantId}
                        />
                    )}
                    {activeTab === 'movements' && (
