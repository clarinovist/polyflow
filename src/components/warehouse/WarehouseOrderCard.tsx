...
export default function WarehouseOrderCard({
    ...
    return (
        <Card className={cn(
            ...
            isFullyIssued ? "border-l-emerald-500 bg-emerald-500/5" : "border-l-orange-500 bg-card"
        )}>
                ...
                <div className="p-4 sm:p-5">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <h3 className="text-xl font-black text-foreground leading-none">{order.orderNumber}</h3>
                                <Badge className={cn(
                                    "text-[10px] font-bold px-2 py-0.5",
                                    order.status === 'RELEASED' ? "bg-blue-500/10 text-blue-600 dark:text-blue-400" : "bg-amber-500/10 text-amber-600 dark:text-amber-400"
                                )}>
                                    {order.status}
                                </Badge>
                            </div>
                        ...
                        <div className="text-right">
                            <div className="flex flex-col items-end">
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">{t('plannedStart')}</span>
                                <span className="text-xs font-bold text-foreground">{format(new Date(order.plannedStartDate), 'MMM d, p')}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                        <div className="flex justify-between items-end">
                            <span className="text-xs font-bold text-muted-foreground">{t('materialFulfillment')}</span>
                            <span className={cn(
                                "text-xs font-black",
                    ...
                    {/* Detail Grid */}
                    <div className="grid grid-cols-2 gap-4 mb-5 p-3 bg-muted/50 rounded-lg border border-border">
                        <div className="space-y-1">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1">
                                <MapPin className="w-3 h-3" /> {t('machine')}
                            </span>
                            <p className="text-xs font-black text-foreground truncate">{order.machine?.name || 'Generic'}</p>
                        </div>
                        <div className="space-y-1 text-right">
                            ...
                            <p className="text-xs font-black text-foreground">{t('priorityNormal')}</p>
                        </div>
                    </div>
                        ...
                        {materialItems.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-center bg-card p-2 rounded border border-border shadow-sm">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-muted-foreground font-medium">{t('requiredAbbr')} {item.required} {item.unit}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className={cn(
                                        "text-xs font-black",
                                        item.issued >= item.required ? "text-emerald-500" : "text-muted-foreground"
                                    )}>
