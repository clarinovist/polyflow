...
export function OrderWorkflowStepper({ status }: { status: string }) {
    const steps = [
        {
            id: 'DRAFT',
            label: getStatusLabel('DRAFT', 'production'),
            description: 'Perencanaan',
        },
        {
            id: 'RELEASED',
            label: getStatusLabel('RELEASED', 'production'),
            description: 'Persiapan',
        },
        {
            id: 'IN_PROGRESS',
            label: getStatusLabel('IN_PROGRESS', 'production'),
            description: 'Eksekusi',
        },
        {
            id: 'COMPLETED',
            label: getStatusLabel('COMPLETED', 'production'),
            description: 'Selesai',
        },
    ];
        ...
        return (
            <div className="w-full p-4 bg-destructive/10 border border-destructive/20 rounded-lg flex items-center gap-3 text-destructive">
                ...
                <span className="font-medium">
                    {getStatusLabel('CANCELLED', 'production')}
                </span>
            </div>
        );
    }
        ...
        <div className="w-full py-4 space-y-4">
                ...
                <div className="w-full p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3 text-amber-900 dark:bg-amber-900/20 dark:border-amber-800/50 dark:text-amber-200">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs mt-1 leading-relaxed">
                            Cek tab Bahan; issue bahan baku (Path A) di Gudang.
                            Setelah bahan tersedia, Rilis SPK.
                        </p>
                    </div>
                    <Link href="/warehouse">
                        <Button
                            size="sm"
                            variant="outline"
                            className="text-xs border-amber-300 bg-white hover:bg-amber-50 dark:bg-zinc-900 dark:border-amber-700"
                        >
                            Buka Gudang
                        </Button>
                    </Link>
            ...
            <div className="relative flex items-center justify-between w-full">
                ...
                {steps.map((step, index) => {
                    ...
                    const isCompleted = isWaitingMaterial
                        ? index === 0
                        : index < effectiveIndex;
                    const isCurrent = isWaitingMaterial
                        ? index === 1
                        : index === effectiveIndex;
                                ...
                                className={cn(
                                    ...
                                    isWaitingNode
                                        ...
                                        : isCompleted
                                          ...
                                          : isCurrent
                                            ? 'bg-background border-foreground text-foreground ring-2 ring-muted shadow-sm'
            ...
            <div className="mt-2 bg-muted/40 border rounded-md p-3 text-sm text-foreground flex items-start gap-3">
                ...
                <div>
                    {status === 'DRAFT' &&
                        "Rencanakan SPK. Periksa BOM dan kuantitas. Jika sudah siap, klik 'Rilis SPK'."}
                    {isWaitingMaterial &&
                        'SPK menunggu konfirmasi bahan. Cek tab Bahan atau hubungi Gudang (Path A). Setelah tersedia, Rilis SPK.'}
                    {status === 'RELEASED' &&
                        'SPK siap dipersiapkan. Ambil bahan dan tetapkan Shift/Mesin sebelum memulai.'}
                    {status === 'IN_PROGRESS' &&
                        'Produksi sedang berjalan. Catat hasil, scrap, dan inspeksi kualitas dengan benar.'}
                    {status === 'COMPLETED' &&
                        'SPK selesai. Inventaris sudah diperbarui. Tidak ada tindakan lebih lanjut.'}
                </div>
            </div>
        </div>
    );
