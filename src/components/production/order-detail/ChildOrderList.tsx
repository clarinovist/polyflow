        ...
        <Card className="border-blue-100 bg-blue-50/20">
            ...
            <CardContent className="space-y-4">
                ...
                {intermediateMaterials.map(mat => {
                    ...
                    return (
                        <div key={mat.id} className="flex items-center justify-between p-3 bg-white border border-blue-200 rounded-lg shadow-sm">
                            <div className="flex flex-col">
                                ...
                                <span className="text-xs text-muted-foreground">
                                    Requires Production: <span className="font-mono font-medium text-amber-600">{shortage.toFixed(2)} {String((mat.productVariant.product as any).primaryUnit)}</span>
                                </span>
                            </div>
                            <Button
                                size="sm"
                ...
                {childOrders.length > 0 && (
                    <div className="space-y-2">
                        ...
                        {childOrders.map(child => (
                            <div key={child.id} onClick={() => router.push(`/planning/orders/${child.id}`)} className="cursor-pointer group flex items-center justify-between p-3 bg-white border hover:border-blue-300 transition-colors rounded-lg">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="font-medium text-sm group-hover:text-blue-600 transition-colors">{child.orderNumber}</span>
