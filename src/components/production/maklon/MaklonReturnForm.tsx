...
import {
    createMaklonReturnSchema,
    CreateMaklonReturnValues,
...
function getMaklonReturnLocationPriority(slug?: string) {
    const priority = maklonReturnLocationPriority.findIndex(
        (value) => value === slug,
    );
...
export function MaklonReturnForm({
    customers,
    locations,
    products,
    initialData,
    ...
    const defaultSourceLocationId =
        initialData?.sourceLocationId ||
        orderedLocations.find(
            (location) => location.slug === MAKLON_STAGE_SLUGS.PACKING,
        )?.id ||
        orderedLocations.find(
            (location) => location.slug === MAKLON_STAGE_SLUGS.FINISHED_GOOD,
        )?.id ||
        orderedLocations.find(
            (location) => location.slug === MAKLON_STAGE_SLUGS.WIP,
        )?.id ||
        orderedLocations.find(
            (location) => location.slug === MAKLON_STAGE_SLUGS.RAW_MATERIAL,
        )?.id ||
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Maklon Packing Area diprioritaskan
                                            untuk return sisa barang jadi. Jika
                                            return berasal dari stage
                                            customer-owned lain, pilih lokasi
                                            yang sesuai.
                                        </p>
                                        <FormMessage />
                                    </FormItem>
