...
export function BOMList({ boms, showPrices }: BOMListProps) {
    ...
    const activeBoms = useMemo(
        () => boms.filter((b) => b.isActive !== false),
        [boms],
    );
    const archivedBoms = useMemo(
        () => boms.filter((b) => b.isActive === false),
        [boms],
    );
    ...
    async function handleBulkArchive() {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (successCount > 0) {
                    toast.success(
                        `${successCount} resep berhasil dinonaktifkan.`,
                    );
                }
                if (failedCount > 0) {
                    toast.warning(
                        `${failedCount} resep gagal dinonaktifkan (default atau error).`,
                    );
                }
    ...
    const filteredBoms = useMemo(() => {
        const filtered = sourceBoms.filter((bom) => {
            ...
            const matchesCategory =
                categoryTab === 'ALL' ||
                (bom.category || 'STANDARD') === categoryTab;
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs
                value={statusTab}
                onValueChange={(v) => setStatusTab(v as 'ACTIVE' | 'ARCHIVED')}
                className="w-full"
            >
                <TabsList>
                                                                    ...
                                                                    <div className="flex flex-col">
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            {bom.isActive ===
                                                                                ...
                                                                                <Badge
                                                                                    variant="outline"
                                                                                    className="text-[10px] h-5 px-1.5 font-bold text-muted-foreground"
                                                                                >
                                                                                    Nonaktif
                                                                                </Badge>
                                                                            )}
                                                                ...
                                                                <TableCell className="text-center">
                                                                    ...
                                                                    0 ? (
                                                                        <Link href="/production/orders">
                                                                            <Badge
                                                                                variant="outline"
                                                                                className="text-[10px] font-normal border-blue-500/20 text-blue-600 bg-blue-50/50 dark:bg-blue-900/10 dark:text-blue-400 hover:bg-blue-100 cursor-pointer"
                                                                            >
                                                                                {
                                                                                    usageCount
                                                                                }{' '}
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>
                            Nonaktifkan {selectedIds.size} resep?
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                            Resep yang dipilih akan dinonaktifkan dan tidak
                            muncul di daftar produksi baru. Resep yang merupakan
                            default akan dilewati (pilih pengganti manual).
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={isBulkArchiving}>
                            Batal
                        </AlertDialogCancel>
                        <AlertDialogAction
                            onClick={(e) => {
