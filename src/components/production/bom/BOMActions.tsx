...
import {
    ...
    archiveBom,
    reactivateBom,
    recalculateBomCostChain,
} from '@/actions/production/boms';
...
export function BOMActions({
    ...
    async function handleArchive() {
        if (isDefault && !newDefaultBomId) {
            toast.error(
                'Pilih resep pengganti sebagai default terlebih dahulu.',
            );
            return;
        }
        setIsArchiving(true);
        try {
            ...
            if (result.success) {
                ...
                toast.error('Gagal menonaktifkan resep', {
                    description: result.error,
                });
            }
        } catch (_error) {
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <strong>{name}</strong> akan dihapus permanen dari
                            sistem. Hanya bisa dilakukan untuk resep yang belum
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Resep <strong>{name}</strong> akan hilang dari
                            daftar produksi baru. History Work Order lama tetap
                            aman.
                            {usageCount > 0 && (
                                <>
                                    {' '}
                                    Resep ini sudah dipakai di{' '}
                                    <strong>{usageCount} Work Order</strong>.
                                </>
                            )}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                        ...
                        <div className="px-6 pb-2">
                            ...
                            {activeBomOptions.length === 0 ? (
                                <p className="text-sm text-destructive">
                                    Tidak ada resep aktif lain di produk ini.
                                    Buat/duplikat resep pengganti dulu, atau
                                    jadikan resep lain sebagai default sebelum
                                    menonaktifkan.
                                </p>
                            ) : (
                                <Select
                                    value={newDefaultBomId}
                                    onValueChange={setNewDefaultBomId}
                                >
                                    <SelectTrigger>
                            ...
                            <p className="text-[11px] text-muted-foreground mt-1">
                                Resep ini adalah default untuk produk ini. Wajib
                                pilih pengganti sebelum nonaktifkan.
                            </p>
                        </div>
                    )}
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Resep <strong>{name}</strong> akan muncul kembali di
                            daftar produksi aktif.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
