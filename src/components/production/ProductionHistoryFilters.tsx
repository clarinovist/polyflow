...
export function ProductionHistoryFilters({
    ...
    function handleYesterday() {
        ...
        // Convert back to UTC for URL params
        const fromUtc = new Date(yesterday.getTime() - 7 * 60 * 60 * 1000);
        const toUtc = new Date(endOfYesterday.getTime() - 7 * 60 * 60 * 1000);
        pushParams({ from: fromUtc.toISOString(), to: toUtc.toISOString() });
    }
    ...
    // Check if current range is "Kemarin"
    const isYesterday = (() => {
        ...
        try {
            ...
            const toUtc = new Date(
                endOfYesterday.getTime() - 7 * 60 * 60 * 1000,
            );
            const fromMs = new Date(from).getTime();
            const toMs = new Date(to).getTime();
            return (
                Math.abs(fromMs - fromUtc.getTime()) < 1000 &&
                Math.abs(toMs - toUtc.getTime()) < 1000
            );
        } catch {
            return false;
        }
        ...
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        ...
                        className="h-8 text-xs"
                        onClick={() =>
                            toggleFilter('missingPhoto', missingPhoto)
                        }
                    >
                        Tanpa foto
                    ...
                    <Button
                        ...
                        className="h-8 text-xs"
                        onClick={() =>
                            toggleFilter('includeVoided', includeVoided)
                        }
                    >
                        Voided
            ...
            <div className="flex flex-wrap items-center gap-2">
                <SelectFilter
                    label="Semua Mesin"
                    value={machineId}
                    options={filterOptions.machines.map((m) => ({
                        id: m.id,
                        label: `${m.code} — ${m.name}`,
                    }))}
                ...
                <SelectFilter
                    label="Semua Shift"
                    value={shiftId}
                    options={filterOptions.shifts.map((s) => ({
                        id: s.id,
                        label: s.shiftName,
                    }))}
