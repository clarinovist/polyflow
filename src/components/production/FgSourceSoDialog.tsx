...
export function FgSourceSoDialog({
  ...
  const totalResidual = sourceSoItems.reduce((sum, item) => sum + item.residualQty, 0);
  const distinctSoCount = new Set(sourceSoItems.map((item) => item.soId)).size;
                ...
                {sourceSoItems.map((item) => (
                  <TableRow key={item.soItemId}>
                    ...
                    <TableCell className="text-right font-medium">
                      {item.residualQty.toLocaleString("id-ID")} {unit}
                    </TableCell>
                    <TableCell>
                      {item.expectedDate ? (
