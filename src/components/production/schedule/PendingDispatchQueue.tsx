...
export function PendingDispatchQueue({
    ...
    const unassigned = orders
        .filter(
            (o) =>
                !o.machineId &&
                ['RELEASED', 'DRAFT', 'WAITING_MATERIAL'].includes(o.status),
        )
        .sort((a, b) => {
        ...
        <div className="w-full">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b">
                <div className="flex items-center gap-2">
                    ...
                    <Badge
                        variant="secondary"
                        className="ml-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300"
                    >
            ...
            <div className="flex flex-wrap gap-3">
                {unassigned.map((order) => {
                    const orderDateStr = toBusinessDateString(
                    ...
                    return (
                        <div
                            key={order.id}
                            className="w-[220px] p-3 rounded-lg border bg-muted/50 flex flex-col gap-2 relative group hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors"
                        >
                            ...
                            <div className="text-[11px] font-medium line-clamp-1 text-zinc-800 dark:text-zinc-200">
                                {order.bomName}
                            </div>
