...
function getAnomalies(exec: ExecutionData, hasPhoto: boolean): AnomalyType[] {
    ...
    if (exec.quantityProduced === 0 && exec.status !== 'VOIDED')
        anomalies.push('qty_zero');
...
function AnomalyBadge({ type }: { type: AnomalyType }) {
    ...
    return (
        <Badge variant="outline" className={cn('text-[9px] px-1 py-0', cls)}>
            {label}
        </Badge>
    );
}
...
function exportCsv(groups: GroupData[]) {
    const rows: string[] = [
        'Waktu Mulai,Waktu Selesai,Durasi,No. SPK,Produk,BOM,Mesin,Operator,Shift,Hasil,Scrap,Unit,Status,Foto,Notes',
    ];
    for (const g of groups) {
        for (const e of g.executions) {
            const start = e.startTime
                ? formatWIB(e.startTime, 'yyyy-MM-dd HH:mm')
                : '';
            const end = e.endTime
                ? formatWIB(e.endTime, 'yyyy-MM-dd HH:mm')
                : '';
...
export function ProductionHistoryClient({
    ...
    const hasFilter =
        ...
        searchParams.has('q') ||
        searchParams.has('hasScrap') ||
        searchParams.has('missingPhoto') ||
        searchParams.has('includeVoided') ||
        searchParams.has('machineId') ||
        searchParams.has('operatorId') ||
        searchParams.has('shiftId') ||
        searchParams.has('productVariantId');
    ...
    const timelineByDay = useMemo(() => {
        ...
        const dayMap = new Map<
            string,
            { exec: ExecutionData; group: GroupData }[]
        >();
        ...
        return (
            <div className="text-center py-10 text-muted-foreground italic space-y-1">
                {hasFilter ? (
                    <>
                        <p>
                            Tidak ada entri cocok filter. Longgarkan
                            produk/mesin/operator.
                        </p>
                        <p className="text-xs">
                            Atau ubah rentang tanggal di atas.
                        </p>
                    </>
                ) : (
                    <>
                        ...
                        <p className="text-xs">
                            Cek kiosk / SPK aktif, atau ubah rentang tanggal.
                        </p>
                    </>
                )}
            </div>
                    ...
                    <div className="hidden md:block rounded-md border overflow-x-auto">
                        <Table>
                            ...
                            <TableBody>
                                {groups.map((group) => {
                                    const isExpanded =
                                        expandedId === group.productionOrder.id;
                                    const scrapPct =
                                        group.totalQuantity + group.totalScrap >
                                        0
                                            ? (group.totalScrap /
                                                  (group.totalQuantity +
                                                      group.totalScrap)) *
...
function GroupRow({
    ...
    const timeRange =
        group.earliestEndTime && group.latestEndTime
                ...
                <TableCell className="text-right">
                    {group.totalScrap > 0 ? (
                        <span className="font-bold text-destructive">
                            {group.totalScrap.toLocaleString()}
                        </span>
                    ) : (
                    ...
                    <TableCell colSpan={12} className="bg-muted/30 p-0">
                        <div className="p-4 space-y-1">
                            <div className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-2">
                                Detail Log Output ({group.executions.length}{' '}
                                entri)
                                {groupAnomalies.length > 0 && (
                                    <div className="flex gap-1">
                                        {groupAnomalies.map((a) => (
                                            <AnomalyBadge key={a} type={a} />
                                        ))}
                                    </div>
                                )}
                            </div>
        ...
        >
            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                {exec.shift && (
                    <Badge
                        variant="outline"
                        className="text-[9px] bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800"
                    >
                ...
                {exec.helpers.length > 0 && (
                    <div
                        ...
                        title={exec.helpers.map((h) => h.name).join(', ')}
                    >
                        <Users className="h-3 w-3" />
                        {exec.helpers.length > 1
                            ? `${exec.helpers[0].name} +${exec.helpers.length - 1}`
                            : exec.helpers[0].name}
                    </div>
                )}
                {anomalies.map((a) => (
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {exec.scrapDaunQty > 0 && (
                        <span className="text-[10px] text-destructive/80 inline-flex items-center gap-0.5">
                            ...
                            {Number(exec.scrapDaunQty).toLocaleString()}
                        </span>
                    )}
                    {exec.scrapProngkolQty > 0 && (
                        <span className="text-[10px] text-destructive/80 inline-flex items-center gap-0.5">
                            <Scissors className="h-2.5 w-2.5" />
                            {Number(exec.scrapProngkolQty).toLocaleString()}
                        </span>
                    )}
                    ...
                    {exec.enteredQuantity !== null && (
                        <span className="text-[10px] text-muted-foreground">
                            Input: {exec.enteredQuantity.toLocaleString()}{' '}
                            {exec.enteredUnit || ''}
                        </span>
                    )}
                    ...
                    {exec.status === 'VOIDED' && (
                        <Badge
                            variant="destructive"
                            className="h-4 text-[8px] uppercase px-1"
                        >
                            Voided
                        </Badge>
                    )}
...
function TimelineRow({
    ...
    return (
        ...
        >
            ...
            {exec.operator && (
                <span className="text-xs text-muted-foreground truncate max-w-[80px] shrink-0">
                    {exec.shift?.operator?.name || exec.operator.name}
                </span>
            )}
    ...
    return (
        <div className="space-y-3 md:hidden">
            {groups.map((group) => {
                ...
                const scrapPct =
                    group.totalQuantity + group.totalScrap > 0
                        ? (group.totalScrap /
                              (group.totalQuantity + group.totalScrap)) *
                          100
                        : null;
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="space-y-1">
                                ...
                                <h4 className="text-sm font-medium leading-snug">
                                    {
                                        group.productionOrder.bom.productVariant
                                            .name
                                    }
                                </h4>
                            ...
                            <Button
                                ...
                                onClick={() =>
                                    setExpandedId(
                                        isExpanded
                                            ? null
                                            : group.productionOrder.id,
                                    )
                                }
                            >
                                <span className="text-xs mr-1">
                                    {group.executions.length} log
                                </span>
                                <ChevronDown
                                    className={cn(
                                        'h-4 w-4 transition-transform',
                        ...
                        {isExpanded && (
                            <div className="pt-2 border-t space-y-1.5">
                                <div className="text-xs font-medium text-muted-foreground mb-1">
                                    Detail Entri ({group.executions.length}):
                                </div>
                                {group.executions.map((exec, idx) => (
                                    <ExecutionDetailRow
                                        key={exec.id}
