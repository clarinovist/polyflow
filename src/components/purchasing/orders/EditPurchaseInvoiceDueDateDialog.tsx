...
export function EditPurchaseInvoiceDueDateDialog({
    open,
    onOpenChange,
    invoice,
}: Props) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await updatePurchaseInvoiceDueDate(
                invoice.id,
                payload as never,
            );
            if (res.success) {
                toast.success(
                    `Jatuh tempo diperbarui → ${format(computedDueDate, 'dd MMM yyyy')}`,
                );
                onOpenChange(false);
                router.refresh();
            } else {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[480px]">
                <DialogHeader>
                    <DialogTitle>
                        Edit Jatuh Tempo — {invoice.invoiceNumber}
                    </DialogTitle>
                    <DialogDescription>
                        Perbaiki tempo & jatuh tempo tanpa mengubah data lain.
                        Invoice lama tidak otomatis diubah sebelumnya.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="text-sm py-2 font-medium">
                                {invoice.termOfPaymentDays ?? 30} hari
                            </div>
                        </div>
                    </div>
                    ...
                    {termDays === -1 && (
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setCustomTermDays(e.target.value)
                                }
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleSave} disabled={isLoading}>
                        {isLoading ? 'Menyimpan...' : 'Simpan Jatuh Tempo'}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
