...
export function SupplierAnalyticsTab({ supplierId }: { supplierId: string }) {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await getSupplierSpendingAnalytics(supplierId);
        setData(res.success ? ((res.data as unknown as Entry[]) ?? []) : []);
        setLoading(false);
    }, [supplierId]);
    useEffect(() => {
        load();
                            ...
                            <div key={entry.month} className="space-y-1">
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="text-muted-foreground">
                                        {entry.count} PO · {fmtIdr(entry.total)}
                                    </span>
                                </div>
