...
export function SupplierPaymentsTab({ supplierId }: { supplierId: string }) {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await listPurchaseInvoicesBySupplier(supplierId);
        setInvoices(
            res.success
                ? ((res.data as unknown as { invoices: Inv[] })?.invoices ?? [])
                : [],
        );
        setLoading(false);
    }, [supplierId]);
                ...
                {loading ? (
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            ...
                            <TableBody>
                                {invoices.map((inv) => {
                                    const total = toN(inv.totalAmount);
                                    const paid = toN(inv.paidAmount);
                                    return (
                                        <TableRow key={inv.id}>
