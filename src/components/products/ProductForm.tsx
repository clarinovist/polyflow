...
export function ProductForm({
    ...
    const productType = (useWatch as unknown as (opts: unknown) => unknown)({
        control: form.control,
        name: 'productType',
    }) as ProductType | undefined;
    const productName = (useWatch as unknown as (opts: unknown) => unknown)({
        control: form.control,
        name: 'name',
    }) as string;
    const assetCategory = (useWatch as unknown as (opts: unknown) => unknown)({
        control: form.control,
        name: 'assetCategory',
    }) as AssetCategory | undefined;
    ...
    useEffect(() => {
        if (productType === ProductType.FIXED_ASSET && assetCategory) {
            ...
            if (!current) {
                form.setValue(
                    'usefulLifeMonths' as unknown as never,
                    def as never,
                );
            }
        }
    ...
    async function onSubmit(data: CreateProductValues | UpdateProductValues) {
        ...
        if (result.success) {
            toast.success(
                `Produk berhasil ${mode === 'create' ? 'dibuat' : 'diperbarui'}.`,
            );
            ...
            toast.error(
                result.error ||
                    `Gagal ${mode === 'create' ? 'membuat' : 'memperbarui'} produk.`,
            );
        }
    }
            ...
            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel>
                                        {productFormLabels.productName}
                                    </FormLabel>
                                    <FormControl>
                                        <Input
                                            placeholder={
                                                productFormLabels.productNamePlaceholder
                                            }
                                            {...field}
                                        />
                                    </FormControl>
                                    ...
                                    >
                                        <FormControl>
                                            <SelectTrigger>
                                                <SelectValue
                                                    placeholder={
                                                        productFormLabels.selectProductType
                                                    }
                                                />
                                            </SelectTrigger>
                                        </FormControl>
                        ...
                        <>
                            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 flex gap-2 text-xs text-amber-800 dark:text-amber-200">
                                ...
                                <div>
                                    <p className="font-medium">
                                        Aset Tetap tidak masuk stok gudang
                                    </p>
                                    <p className="text-amber-700/80 dark:text-amber-200/70">
                                        Pengakuan akuntansi saat Goods Receipt.
                                        Lokasi fisik dicatat di kartu aset. Akun
                                        aset wajib dipilih.
                                    </p>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription className="text-[10px]">
                                                Default: Mesin/Peralatan/Lain
                                                60, Kendaraan 48, Bangunan 240.
                                                Bisa custom.
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                ...
                                <FormField
                                    control={form.control}
                                    name={
                                        'inventoryAccountId' as unknown as never
                                    }
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>
                                                Akun Aset Tetap (Wajib)
                                            </FormLabel>
                                            <Select
                                                onValueChange={field.onChange}
                                                value={
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-semibold">
                            {productFormLabels.productVariants}
                        </h3>
                        <Button
                            type="button"
                            variant="outline"
                ...
                <div className="flex justify-end gap-4">
                    ...
                    >
                        {form.formState.isSubmitting
                            ? mode === 'create'
                                ? productFormLabels.creating
                                : productFormLabels.updating
                            : mode === 'create'
                              ? productFormLabels.createProduct
                              : productFormLabels.updateProduct}
                    </Button>
                </div>
            </form>
        </Form>
