...
function isNavItemActive(
    ...
    return Array.from(targetQuery.entries()).every(
        ([key, value]) => searchParams.get(key) === value,
    );
}

function isSubtreeActive(
    items: NavItemProps[],
    pathname: string,
    searchParams: URLSearchParams,
): boolean {
    return items.some((item) => {
        if (isNavItemActive(item.href, pathname, searchParams, item.exact))
            return true;
        if (item.children)
            return isSubtreeActive(item.children, pathname, searchParams);
        return false;
    });
}
...
const accentClasses = {
    primary: {
        bg: 'bg-sidebar-accent text-sidebar-accent-foreground',
        icon: 'text-sidebar-accent-foreground',
    },
    emerald: {
        bg: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/50',
        icon: 'text-emerald-600 dark:text-emerald-400',
    },
    blue: {
        bg: 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400 border border-blue-100 dark:border-blue-800/50',
        icon: 'text-blue-600 dark:text-blue-400',
    },
    purple: {
        bg: 'bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400 border border-purple-100 dark:border-purple-800/50',
        icon: 'text-purple-600 dark:text-purple-400',
    },
    amber: {
        bg: 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400 border border-amber-100 dark:border-amber-800/50',
        icon: 'text-amber-600 dark:text-amber-400',
    },
    rose: {
        bg: 'bg-rose-50 text-rose-700 dark:bg-rose-900/20 dark:text-rose-400 border border-rose-100 dark:border-rose-800/50',
...
export function PortalNavItem({
    ...
    const childIsActive =
        hasChildren && isSubtreeActive(children!, pathname, searchParams);
    ...
    return (
        ...
        >
            <Icon
                className={cn(
                    'h-4 w-4',
                    isActive ? iconActiveClasses : 'text-muted-foreground',
                )}
            />
            <span>{label}</span>
        </Link>
...
export function PortalNavGroup({
    ...
    return (
        <div className="space-y-1">
            ...
            <div
                className={cn(
                    'space-y-1 overflow-hidden transition-all ml-2',
                    isOpen ? 'block' : 'hidden',
                )}
            >
                {items.map((item) => (
