...
export function DisciplinaryManager() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.employeeId || !form.reason || !form.effectiveDate) {
            toast.error(
                'Lengkapi field wajib (karyawan, alasan, tanggal berlaku)',
            );
            return;
        }
        try {
                ...
                <div className="rounded-lg border bg-card p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ...
                        {recap.byType.slice(0, 2).map((t) => (
                            ...
                            >
                                <div className="text-[10px] text-muted-foreground">
                                    {TYPE_LABEL[t.type as DType] ?? t.type}
                                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3">
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {(Object.keys(TYPE_LABEL) as DType[]).map(
                                        (t) => (
                                ...
                                {actions.map((a) => {
                                    ...
                                    return (
                                        <tr key={a.id} className="border-t">
                                            ...
                                            <td className="p-2">
                                                <span
                                                    className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${active ? 'bg-red-500/10 text-red-700' : 'bg-muted text-muted-foreground'}`}
                                                >
                                                    {active
                                                        ? 'Aktif'
                                                        : 'Hangus'}
                                            ...
                                            <td
                                                className="p-2 text-xs text-muted-foreground max-w-[260px] truncate"
                                                title={a.reason}
                                            >
                                                {a.reason}
                                                {a.documentUrl ? (
