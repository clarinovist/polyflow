...
export function Employee360DocumentsTab({ employeeId }: Props) {
    ...
    const loadDocs = useCallback(
        async (archived = false) => {
            ...
            setDocuments(
                res.success ? ((res.data ?? []) as unknown as Doc[]) : [],
            );
            setLoading(false);
        },
        [employeeId],
            ...
            <CardContent className="space-y-4">
                ...
                >
                    ...
                    <Button
                        type="submit"
                        size="sm"
                        disabled={uploading || !docFile || !docForm.name.trim()}
                    >
                        ...
                        <div className="flex gap-1 shrink-0">
                            ...
                            {d.status === 'ACTIVE' ? (
                                <button
                                    ...
                                    onClick={async () => {
                                        const res =
                                            await archiveEmployeeDocument(d.id);
                                        if (res.success) {
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        const res =
                                            await restoreEmployeeDocument(d.id);
                                        if (res.success) {
