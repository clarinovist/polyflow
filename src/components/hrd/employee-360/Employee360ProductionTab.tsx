...
export function Employee360ProductionTab({ employeeId }: Props) {
    ...
    const load = useCallback(
        async (base: Date) => {
            ...
            const res = await listExecutionsByOperator(
                employeeId,
                from.toISOString(),
                to.toISOString(),
            );
            setExecs(
                res.success ? ((res.data ?? []) as unknown as Exec[]) : [],
            );
            setLoading(false);
        },
        [employeeId],
    ...
    const paidExecs = execs.filter(
        (e) => e.pieceRateSnapshot != null && toN(e.pieceRateSnapshot) > 0,
    );
