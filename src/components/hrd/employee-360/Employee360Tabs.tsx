        ...
        <div className="space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange}>
                <TabsList className="flex h-auto gap-1 overflow-x-auto scrollbar-none justify-start bg-muted/30 p-1">
                    ...
                    <TabsTrigger value="loans" className="text-xs shrink-0">
                        Kasbon
                    </TabsTrigger>
                    <TabsTrigger
                        value="leave-discipline"
                        className="text-xs shrink-0"
                    >
                        Cuti & Disiplin
                    </TabsTrigger>
                    <TabsTrigger value="documents" className="text-xs shrink-0">
                        Dokumen
                    </TabsTrigger>
                </TabsList>
                ...
                <div className="mt-4">
                    {activeTab === 'overview' && (
                        <Employee360Overview employee={employee} />
                    )}
                    {activeTab === 'attendance' && (
                        <Employee360AttendanceTab employeeId={employee.id} />
                    )}
                    {activeTab === 'production' && isPiece && (
                        <Employee360ProductionTab employeeId={employee.id} />
                    )}
                    ...
                    {activeTab === 'payroll-monthly' &&
                        !isDaily &&
                        !isPiece && (
                            <Employee360PayrollMonthlyTab
                                employeeId={employee.id}
                            />
                        )}
                    {activeTab === 'loans' && (
                        <Employee360LoansTab employeeId={employee.id} />
                    )}
                    {activeTab === 'leave-discipline' && (
                        <Employee360LeaveDisciplineTab
                            employeeId={employee.id}
                        />
                    )}
                    {activeTab === 'documents' && (
                        <Employee360DocumentsTab employeeId={employee.id} />
                    )}
                </div>
            </Tabs>
        </div>
