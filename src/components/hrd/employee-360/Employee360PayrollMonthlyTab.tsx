...
import {
    ...
    listActiveAllowances,
} from '@/actions/hrd/payslips-employee';
...
export function Employee360PayrollMonthlyTab({ employeeId }: Props) {
    ...
    const load = useCallback(async () => {
        ...
        setPayslips(
            pRes.success ? ((pRes.data ?? []) as unknown as Payslip[]) : [],
        );
        setAllowances(
            aRes.success ? ((aRes.data ?? []) as unknown as Allowance[]) : [],
        );
        setLoading(false);
    }, [employeeId]);
            ...
            <Card>
                ...
                <CardContent>
                    {loading ? (
                        ...
                        <p className="text-xs text-muted-foreground text-center py-8">
                            Belum ada riwayat gaji bulanan.
                        </p>
                    ) : (
                                    ...
                                    {payslips.map((p) => (
                                        <TableRow key={p.id}>
                                            <TableCell className="text-sm font-medium">
                                                {p.payrollPeriod
                                                    ? `${MONTHS[p.payrollPeriod.month]} ${p.payrollPeriod.year}`
                                                    : '-'}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono">
                                                {fmtIdr(toN(p.baseSalary))}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono">
                                                {fmtIdr(toN(p.allowanceTotal))}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono">
                                                {fmtIdr(toN(p.thrAmount))}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono text-red-600">
                                                -{fmtIdr(toN(p.bpjsDeduction))}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono text-red-600">
                                                -{fmtIdr(toN(p.loanDeduction))}
                                            </TableCell>
                                            <TableCell className="text-right text-sm font-mono text-red-600">
                                                -
                                                {fmtIdr(
