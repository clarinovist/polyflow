...
function urgencyBadge(emp: {
    ...
    if (pDays !== null && pDays >= 0 && pDays <= 30) {
        return {
            variant:
                pDays <= 7 ? ('destructive' as const) : ('outline' as const),
            text: `Probation H-${pDays}`,
        };
    }
    if (cDays !== null && cDays >= 0 && cDays <= 30) {
        return {
            variant:
                cDays <= 7 ? ('destructive' as const) : ('outline' as const),
            text: `Kontrak H-${cDays}`,
        };
    }
    return null;
}
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1 text-sm text-muted-foreground">
                        ...
                        <Badge
                            variant="outline"
                            className={`text-[10px] ${isPiece ? 'text-amber-600 border-amber-500/30' : isMonthly ? 'text-blue-600 border-blue-500/30' : ''}`}
                        >
                            {PAY_TYPE_LABELS[emp.payType] || emp.payType}
                        </Badge>
                        {emp.employmentStatus && (
                            <Badge variant="outline" className="text-[10px]">
                                {EMPLOYMENT_LABELS[emp.employmentStatus] ||
                                    emp.employmentStatus}
                            </Badge>
                        )}
                        {badge && (
                ...
                <div className="flex gap-2 shrink-0">
                    ...
                    <Link
                        href={`/dashboard/employees/${emp.id}?tab=payroll-weekly`}
                    >
                        <Button variant="outline" size="sm">
