...
export function Employee360LeaveDisciplineTab({ employeeId }: Props) {
    ...
    const load = useCallback(async () => {
        ...
        setDisciplines(
            dRes.success ? ((dRes.data ?? []) as unknown as Discipline[]) : [],
        );
        setLeaves(
            lRes.success ? ((lRes.data ?? []) as unknown as Leave[]) : [],
        );
        setLoading(false);
    }, [employeeId]);
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-2">
                            {leaves.map((l) => (
                                <div
                                    key={l.id}
                                    className="border-b border-border/50 py-2 flex items-start justify-between gap-3"
                                >
                                    <div>
                                    ...
                                    <Badge
                                        variant="outline"
                                        className={`text-[10px] shrink-0 ${LEAVE_STATUS[l.status] || ''}`}
                                    >
                                        {l.status}
                                    </Badge>
                                </div>
