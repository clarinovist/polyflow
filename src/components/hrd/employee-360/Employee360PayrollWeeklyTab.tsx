...
export function Employee360PayrollWeeklyTab({
    employeeId,
    initialWeek,
}: Props) {
    ...
    const load = useCallback(
        async (base: Date) => {
            ...
            setPayroll(
                res.success ? (res.data as unknown as WeeklyPayroll) : null,
            );
            setLoading(false);
        },
        [employeeId],
    ...
    if (!payroll)
        return (
            <p className="text-xs text-muted-foreground text-center py-8">
                Gagal memuat data gaji mingguan.
            </p>
        );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                        ...
                        {payroll.bpjsDeduction > 0 && (
                            <div className="bg-rose-500/5 p-3 rounded-lg border border-rose-500/10">
                                ...
                                <div className="text-lg font-bold text-rose-700 dark:text-rose-400">
                                    -{formatIdr(payroll.bpjsDeduction)}
                                </div>
                            </div>
                        )}
                                        ...
                                        <TableRow key={line.executionId}>
                                            ...
                                            <TableCell>
                                                {line.paid ? (
                                                    <Badge
                                                        variant="outline"
                                                        className="text-[10px] bg-emerald-500/10 text-emerald-600 border-emerald-500/10"
                                                    >
                                                        Dibayar
                                                    </Badge>
                                                ) : (
                                                    <Badge
                                                        variant="outline"
                                                        className="text-[10px] bg-rose-500/10 text-rose-600 border-rose-500/10"
                                                    >
                                                        {line.reason ===
                                                        'NO_ATTENDANCE'
                                                            ? 'Tanpa Absen'
