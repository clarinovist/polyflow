    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (res.success) {
                const r = res.data!;
                toast.success(
                    `${r.created} payslip dibuat (${r.skipped} dilewati)`,
                );
                setGenOpen(false);
                load();
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                        Pilih periode atau generate baru untuk karyawan MONTHLY
                    </p>
                </div>
                <Button size="sm" onClick={() => setGenOpen((o) => !o)}>
                    {genOpen ? 'Tutup' : 'Generate Periode'}
                </Button>
            </div>
                ...
                <Card className="bg-background/40 border-white/5">
                    <CardContent className="p-4 flex flex-wrap items-end gap-3">
                        ...
                        >
                            {genLoading
                                ? 'Memproses…'
                                : `Generate ${MONTH_NAMES[genForm.month - 1]} ${genForm.year}`}
                        </Button>
                    </CardContent>
                </Card>
            )}
                        ...
                        {!loading && periods.length === 0 && (
                            <tr>
                                <td
                                    ...
                                    className="p-4 text-center text-muted-foreground"
                                >
                                    Belum ada periode — generate baru
                                </td>
                            </tr>
                        )}
                        {periods.map((p) => (
                            <tr
                                ...
                                onClick={() =>
                                    router.push(`/hrd/payroll-monthly/${p.id}`)
                                }
                            >
                                ...
                                <td className="p-2">
                                    <span
                                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${badgeClass(p.status)}`}
                                    >
                                        {p.status}
                                    </span>
                                </td>
