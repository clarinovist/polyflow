    ...
    const startCamera = useCallback(async () => {
        ...
        try {
            if (!navigator.mediaDevices?.getUserMedia) {
                throw new Error(
                    'Kamera tidak didukung di browser ini. Gunakan HTTPS / browser modern.',
                );
            }
            stopStream();
    ...
    const handleCapture = useCallback(async () => {
        ...
        try {
            ...
            const rawBlob = await new Promise<Blob>((resolve, reject) => {
                canvas.toBlob(
                    (b) =>
                        b ? resolve(b) : reject(new Error('Gagal ambil foto')),
                    'image/jpeg',
                    0.92,
                );
            });
            ...
            setCameraError(
                err instanceof Error ? err.message : 'Gagal ambil selfie',
            );
        } finally {
            setCapturing(false);
        }
