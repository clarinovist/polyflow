...
async function uploadLoanDoc(
    ...
    const res = await fetch('/api/upload/hrd-doc', {
        method: 'POST',
        body: fd,
    });
...
export function LoansManager() {
    ...
    const handleDefaulted = async (id: string, loanNumber: string) => {
        if (!confirm(`Tandai kasbon ${loanNumber} sebagai DEFAULTED (macet)?`))
            return;
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="rounded-xl border bg-card p-3">
                        ...
                        <div className="text-lg font-bold text-amber-700">
                            {formatIdr(portfolio.sumRemainingActive)}
                        </div>
                    </div>
                    <div className="rounded-xl border bg-card p-3">
                        <div className="text-[10px] uppercase text-muted-foreground font-medium">
                        ...
                        <div className="text-lg font-bold">
                            {formatIdr(portfolio.sumPrincipalActive)}
                        </div>
                    </div>
                    <div className="rounded-xl border bg-card p-3">
                        <div className="text-[10px] uppercase text-muted-foreground font-medium">
                        ...
                        <div className="text-lg font-bold">
                            {portfolio.paidOffCount} /{' '}
                            {portfolio.defaultedCount}
                        </div>
                    </div>
                </div>
            )}
                            ...
                            <Select
                                ...
                                onValueChange={(v) =>
                                    setForm({
                                        ...form,
                                        repaymentType:
                                            v as typeof form.repaymentType,
                                    })
                                }
                            >
                                <SelectTrigger className="h-9">
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="FULL_NEXT_MONTH">
                                        Lunas bulan depan
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                ...
                <div className="lg:col-span-2 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            <a
                                href={`/api/hrd/loans/export?status=${filterStatus}`}
                            >
                                <Button
                                    type="button"
                                    size="sm"
                                    ...
                                    <Fragment key={l.id}>
                                        <tr className="border-t">
                                            ...
                                            <td className="p-2 text-right">
                                                {formatIdr(
                                                    toN(l.principalAmount),
                                                )}
                                            </td>
                                            <td className="p-2 text-right font-semibold">
                                                {formatIdr(
                                                    toN(l.remainingBalance),
                                                )}
                                            </td>
                                            <td className="p-2 text-xs">
                                                {l.repaymentType ===
