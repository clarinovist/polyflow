        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Peserta & iuran master · potongan real dari gaji ·
                        minggu potong shopfloor: {weekLabel || '—'}
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <a
                        href={`/api/hrd/bpjs/export?year=${year}&month=${month}`}
                    >
                        <Button
                            type="button"
                            size="sm"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="rounded-xl border bg-card p-3">
                    ...
                    <div className="text-lg font-bold">
                        {formatIdr(totals.sumEmployeeDeductionMaster)}
                    </div>
                </div>
                <div className="rounded-xl border bg-card p-3">
                    <div className="text-[10px] uppercase text-muted-foreground font-medium">
                    ...
                    <div className="text-lg font-bold">
                        {formatIdr(totals.sumEmployerCostMaster)}
                    </div>
                </div>
                <div className="rounded-xl border bg-card p-3">
                    <div className="text-[10px] uppercase text-muted-foreground font-medium">
                    ...
                    <div className="text-lg font-bold text-rose-700">
                        {formatIdr(totals.sumActualDeducted)}
                    </div>
                </div>
            </div>
                            ...
                            rows.map((r) => (
                                <tr key={r.employeeId} className="border-t">
                                    ...
                                    <td className="p-3 text-right">
                                        {formatIdr(r.employeeDeductionMaster)}
                                    </td>
                                    <td className="p-3 text-right">
                                        {formatIdr(r.employerCostMaster)}
                                    </td>
                                    <td className="p-3 text-right">
                                        {r.actualDeducted > 0 ? (
                                            <span className="text-rose-700 font-medium">
                                                -{formatIdr(r.actualDeducted)}
                                            </span>
                                        ) : (
                                    ...
                                    <td className="p-3 text-xs text-muted-foreground">
                                        {r.source === 'WEEKLY_LAST'
                                            ? 'Minggu akhir'
                                            : r.source === 'MONTHLY_SLIP'
                                              ? 'Slip bulanan'
                                              : '—'}
                                    </td>
                                </tr>
            ...
            <p className="text-xs text-muted-foreground">
                Iuran master dari data karyawan. Shopfloor (harian/borongan):
                potongan real = iuran karyawan di minggu yang memuat akhir
                bulan. Kantor (bulanan): potongan real dari slip yang sudah
                di-generate. Belum potong = slip belum ada / iuran 0.
            </p>
        </div>
    );
}
