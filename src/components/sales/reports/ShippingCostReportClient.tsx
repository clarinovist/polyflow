...
export function ShippingCostReportClient() {
    ...
    const [activeTab, setActiveTab] = useState<
        'summary' | 'byVehicle' | 'byMonth' | 'byRoute' | 'detail'
    >('summary');
    ...
    const loadReport = useCallback(async () => {
        ...
        try {
            ...
            if (filterOwnership !== 'ALL')
                filters.ownershipType = filterOwnership;
            if (filterRoute.trim()) filters.routeName = filterRoute.trim();

            const res = await getShippingCostReport(
                filters as unknown as Parameters<
                    typeof getShippingCostReport
                >[0],
            );
            if (res.success && res.data) {
    ...
    const handleExportCSV = () => {
        ...
        const rows = [
            ...
            ...report.deliveries.map((d) => [
                ...
                d.vehicle?.ownershipType === 'FACTORY'
                    ? 'Pabrik'
                    : 'Perorangan',
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Rata-rata:{' '}
                                {formatRupiah(summary.avgCostPerDelivery)}/kirim
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="flex items-center gap-2 text-muted-foreground text-sm">
                                <DollarSign className="h-4 w-4" /> Total Biaya
                                Customer
                            </div>
                            <p className="text-2xl font-bold mt-1 text-green-600">
                                {formatRupiah(summary.totalCharge)}
                            </p>
                            <p className="text-xs text-muted-foreground">
                                Rata-rata:{' '}
                                {formatRupiah(summary.avgChargePerDelivery)}
                                /kirim
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            ...
                            <p
                                className={`text-2xl font-bold mt-1 ${summary.totalMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}
                            >
                                    ...
                                    <TableBody>
                                        {summary?.byVehicle.map((v) => (
                                            <TableRow key={v.vehicleId}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-medium ${v.totalMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}
                                                >
                                                    {formatRupiah(
                                                        v.totalMargin,
                                                    )}
                                    ...
                                    <TableBody>
                                        {summary?.byMonth.map((m) => (
                                            <TableRow key={m.month}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-medium ${m.totalMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}
                                                >
                                                    {formatRupiah(
                                                        m.totalMargin,
                                                    )}
                                    ...
                                    <TableBody>
                                        {summary?.byRoute.map((r) => (
                                            <TableRow key={r.route}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-medium ${r.totalMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}
                                                >
                                                    {formatRupiah(
                                                        r.totalMargin,
                                                    )}
                                    ...
                                    <TableBody>
                                        {report.deliveries.map((d) => (
                                            <TableRow key={d.id}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-medium ${d.totalCharge - d.totalCost >= 0 ? 'text-green-600' : 'text-red-600'}`}
                                                >
                                                    {formatRupiah(
                                                        d.totalCharge -
                                                            d.totalCost,
