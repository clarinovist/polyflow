...
export function VehicleTariffDialog({
    mode,
    vehicleId,
    initialData,
    trigger,
    ...
    const getDefaultValues = (): CreateVehicleTariffValues => {
        ...
        return {
            ...
            chargeRate: initialData?.chargeRate
                ? Number(initialData.chargeRate)
                : 0,
            routeName: initialData?.routeName || '',
            minKg: initialData?.minKg ? Number(initialData.minKg) : undefined,
            validFrom: initialData?.validFrom
                ? new Date(initialData.validFrom as string | Date)
                : new Date(now),
            validUntil: initialData?.validUntil
                ? new Date(initialData.validUntil as string | Date)
                : undefined,
        ...
        try {
            const result =
                mode === 'create'
                    ...
                    : await updateVehicleTariff(initialData!.id, {
                          ...values,
                          vehicleId,
                      });
            ...
            if (!result.success) {
                ...
                if (msg.includes('Failed to find Server Action')) {
                    ...
                    toast.error(
                        msg ||
                            `Gagal ${mode === 'create' ? 'menambahkan' : 'mengupdate'} tarif.`,
                    );
                }
                return;
            }
            ...
            toast.success(
                `Tarif berhasil ${mode === 'create' ? 'ditambahkan' : 'diupdate'}.`,
            );
            setOpen(false);
            form.reset();
            router.refresh();
            ...
            toast.error(
                `Gagal ${mode === 'create' ? 'menambahkan' : 'mengupdate'} tarif. Silakan coba lagi.`,
            );
        } finally {
