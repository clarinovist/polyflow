...
export function VehicleDialog({
    ...
    const form = useForm<CreateVehicleValues>({
        ...
        defaultValues: {
            ...
            ownershipType:
                (initialData?.ownershipType as OwnershipType) || 'PRIVATE',
            ownerName: initialData?.ownerName || '',
            driverName: initialData?.driverName || '',
            capacityKg: initialData?.capacityKg
                ? Number(initialData.capacityKg)
                : undefined,
            status: (initialData?.status as VehicleStatus) || 'ACTIVE',
            notes: initialData?.notes || '',
            photoUrl: initialData?.photoUrl || '',
            kirNumber: initialData?.kirNumber || '',
            kirExpireDate: initialData?.kirExpireDate
                ? new Date(initialData.kirExpireDate)
                : null,
        },
    });
        ...
        try {
            ...
            if (!result.success) {
                ...
                if (msg.includes('Failed to find Server Action')) {
                    toast.error(
                        'Halaman sudah kedaluwarsa karena deploy baru. Silakan refresh (F5) lalu coba lagi.',
                        {
                            duration: 10000,
                            action: {
                                label: 'Refresh',
                    ...
                    toast.error(
                        msg ||
                            `Gagal ${mode === 'create' ? 'membuat' : 'mengupdate'} kendaraan.`,
                    );
                }
                return;
            }
            ...
            toast.success(
                `Kendaraan berhasil ${mode === 'create' ? 'dibuat' : 'diupdate'}.`,
            );
            setOpen(false);
            form.reset();
            router.refresh();
            ...
            toast.error(
                `Gagal ${mode === 'create' ? 'membuat' : 'mengupdate'} kendaraan. Silakan coba lagi.`,
            );
        } finally {
                    ...
                    >
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel>
                                        {salesLabels.vehicleName} *
                                    </FormLabel>
                                    <FormControl>
                                        <Input
                                            {...field}
                                            placeholder="Contoh: L300 Mas Joko"
                                        />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                <SelectItem value="FACTORY">
                                                    {salesLabels.factoryOwned}
                                                </SelectItem>
                                                <SelectItem value="PRIVATE">
                                                    {salesLabels.privateOwned}
                                                </SelectItem>
                                            </SelectContent>
                                        </Select>
                                        <FormMessage />
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input
                                            {...field}
                                            placeholder="Nama pemilik (jika perorangan)"
                                        />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
