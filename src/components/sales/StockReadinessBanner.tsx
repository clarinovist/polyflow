 ...
 */
export function StockReadinessBanner({
    lines,
    className,
    compact = false,
}: StockReadinessBannerProps) {
    ...
    return (
        ...
        >
            ...
            <AlertDescription className="text-amber-700 dark:text-amber-300">
                {compact ? (
                    <span>
                        {notReadyLines.length} dari {lines.length} item
                        kekurangan stok (total kurang:{' '}
                        {totalShortfall.toLocaleString('id-ID')}). Surat Jalan
                        bisa dicetak, tapi <strong>Tandai Dikirim</strong> akan
                        gagal sampai stok cukup.
                    </span>
                ) : (
                        ...
                        {notReadyLines.map((line) => (
                            ...
                            >
                                ...
                                <span className="text-muted-foreground">
                                    perlu{' '}
                                    {line.neededQty.toLocaleString('id-ID')},
                                    tersedia{' '}
                                    {line.availableQty.toLocaleString('id-ID')}
                                    {line.reservedForThisSo > 0 &&
                                        ` (${line.reservedForThisSo.toLocaleString('id-ID')} reserved)`}{' '}
                                    — kurang{' '}
                                    <strong>
                                        {line.shortfall.toLocaleString('id-ID')}
                                    </strong>
                                </span>
                            </div>
                        ))}
                        <p className="mt-2 text-xs">
                            Surat Jalan bisa dicetak, tapi{' '}
                            <strong>Tandai Dikirim</strong> akan gagal sampai
                            stok cukup.
                        </p>
                    </div>
                )}
