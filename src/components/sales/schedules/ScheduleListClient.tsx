...
export function ScheduleListClient({ schedules }: ScheduleListClientProps) {
    ...
    const filtered =
        filterStatus === 'ALL'
            ...
            : filterStatus === 'ACTIVE'
              ? schedules.filter((s) =>
                    ['ACTIVE', 'CONFIRMED', 'IN_TRANSIT'].includes(s.status),
                )
    ...
    const getAggregates = (s: ScheduleRow) => {
        ...
        const stopCount = s.vehicles.reduce(
            (sum, sv) => sum + sv.orders.length,
            0,
        );
        const unlinked = s.vehicles.reduce(
            (sum, sv) =>
                sum + sv.orders.filter((o) => !o.deliveryOrderId).length,
            0,
        );
                ...
                <CardContent>
                    {filtered.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                            Belum ada jadwal kiriman. Klik &quot;Jadwal
                            Baru&quot; untuk membuat.
                        </div>
                    ) : (
                        <Table>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <form onSubmit={handleCreate} className="space-y-4">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Pilih tanggal untuk minggu jadwal yang ingin
                                dibuat. Sistem akan otomatis menentukan awal
                                (Senin) dan akhir (Minggu) minggu dari tanggal
                                tersebut.
                            </DialogDescription>
                        </DialogHeader>
