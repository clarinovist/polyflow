...
export function EditDeliveryPricingDialog({
    ...
    const [overrideCostRate, setOverrideCostRate] = useState(
        order.appliedCostRate != null
            ? String(Number(order.appliedCostRate))
            : '',
    );
    const [overrideChargeRate, setOverrideChargeRate] = useState(
        order.appliedChargeRate != null
            ? String(Number(order.appliedChargeRate))
            : '',
    );
    const [estimatedWeightKg, setEstimatedWeightKg] = useState(
        order.estimatedWeightKg != null
            ? String(Number(order.estimatedWeightKg))
            : '',
    );
    ...
    const { suggestedCharge, suggestedCost } = useMemo(() => {
        ...
        return {
            suggestedCharge:
                chargeRate > 0
                    ? Math.round(weight * chargeRate * 100) / 100
                    : null,
            suggestedCost:
                costRate > 0 ? Math.round(weight * costRate * 100) / 100 : null,
        };
    }, [
        tariffRateType,
        estimatedWeightKg,
    ...
    const loadTariffForRoute = useCallback(
        async (vehicleId: string, routeName: string) => {
            ...
            if (order.vehicleId) {
                fetchRouteOptions(order.vehicleId).then((res) => {
                    if (res.success && res.data)
                        setRouteOptions(res.data as string[]);
                });
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await updateDeliveryPricing({
                ...
                appliedRateType:
                    (tariffRateType as 'PER_KG' | 'FLAT_RATE') || null,
            ...
            if (sync?.reason === 'INVOICE_LOCKED') {
                toast.warning(
                    'Charge disimpan di surat jalan, tetapi invoice sudah final — ongkir SO tidak diubah.',
                );
            } else if (sync?.synced && sync.shippingCost > 0) {
                toast.success(
                    `Ongkir SO diupdate ke ${fmtIDR(sync.shippingCost)}`,
                );
            } else {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>
                        Edit Pricing — {order.orderNumber}
                    </DialogTitle>
                    <DialogDescription>
                        Ubah armada, rute, berat, atau tarif. Total akan
                ...
                <div className="grid gap-4 py-4">
                    ...
                    {selectedVehicleId && selectedVehicleId !== '__none__' && (
                        <div className="space-y-2">
                            <Label>Rute</Label>
                            <Select
                                value={selectedRouteName || '__all__'}
