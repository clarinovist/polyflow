...
export function BottomNav({ permissions, badges }: BottomNavProps) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            !isAdmin &&
            document.cookie.includes('bypass_mobile=true')
        ) {
            document.cookie =
                'bypass_mobile=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;';
        }
    }, [isAdmin]);

    const visibleTabs = tabs.filter((tab) =>
        canSeeNavHref(
            tab.resource ?? tab.href,
            permissions,
            tab.resource ? '/warehouse' : '/sales',
        ),
    );
