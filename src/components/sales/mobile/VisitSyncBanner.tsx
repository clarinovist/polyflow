...
export function VisitSyncBanner() {
    ...
    const handleSync = async (retryLogs: VisitLog[] | null = null) => {
        ...
        if (!isOnline) {
            toast.error(
                'Tidak ada koneksi internet. Aktifkan internet untuk sinkronisasi.',
            );
            return;
        }
        ...
        try {
            ...
            if (res?.success && res.data?.results) {
                ...
                const failedCount = serverResults.filter(
                    (r) => !r.success,
                ).length;
                if (failedCount > 0) {
                    toast.warning(
                ...
                toast.error(
                    (res as { error?: string })?.error ||
                        'Gagal sinkronisasi data ke server',
                );
            }
        } catch (err) {
            console.error(err);
