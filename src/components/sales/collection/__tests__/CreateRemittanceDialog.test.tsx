...
describe('CreateRemittanceDialog', () => {
    ...
    it.each([true, false])('shows session expiry for 401 (JSON body: %s) without saving proof', async (isJson) => {
        ...
        render(<CreateRemittanceDialog open={true} onOpenChange={() => {}} invoices={invoices} />);
        selectInvoice();
        ...
        expect(mockCreateRemittanceAction).not.toHaveBeenCalled();
        expect(screen.getByRole('button', { name: /upload screenshot bukti transfer/i }).hasAttribute('disabled')).toBe(false);
    });
