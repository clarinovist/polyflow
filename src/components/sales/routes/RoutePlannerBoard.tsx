...
import { createRoutePlan, publishRoutePlan, deleteRoutePlan, copyLastWeekRoute, importRouteExcel, optimizeRouteNearestNeighbor } from "@/actions/sales/route-plans";
...
export function RoutePlannerBoard({ plans, customers }: RoutePlannerBoardProps) {
  ...
  const reps: Rep[] = useMemo(() => {
    ...
    for (const plan of plans) {
      if (!map.has(plan.userId)) {
        map.set(plan.userId, { id: plan.userId, name: plan.user.name ?? "Unknown" });
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success(`Rute berhasil disimpan: ${selectedCustomerIds.length} toko`);
      } else {
        toast.error((result as { error?: string })?.error || "Gagal menyimpan rute");
      }
    } catch {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        toast.error((result as { error?: string })?.error || "Gagal mempublikasikan rute");
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        toast.error((result as { error?: string })?.error || "Gagal menghapus rute");
      }
    } catch {
  ...
  const handleCopyLastWeek = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        toast.success(`Berhasil menyalin ${data?.items?.length ?? 0} toko dari minggu lalu`);
      } else {
        toast.error((result as { error?: string })?.error || "Gagal menyalin rute");
      }
    } catch {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          const code =
            row["code"] || row["kode"] || row["customer_code"] || row["Kode Customer"] || row["kode_customer"];
          if (code) codes.push(String(code).trim());
        }

        if (codes.length === 0) {
          toast.error("Tidak ada kode customer ditemukan di file. Kolom: code/kode/customer_code");
          return;
        }
        ...
        if (result?.success) {
          toast.success(`Import berhasil: ${codes.length} kode, ditemukan di database`);
        } else {
          toast.error((result as { error?: string })?.error || "Gagal import");
        }
      } catch {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        toast.error((result as { error?: string })?.error || "Gagal optimasi rute");
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-4 h-10">
            ...
            {existingPlan && (
              <Badge
                variant={existingPlan.status === "PUBLISHED" ? "default" : "secondary"}
                className="text-[10px]"
              >
                {existingPlan.status}
              </Badge>
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          onClick={handleSave}
          disabled={isSaving || !selectedRepId || selectedCustomerIds.length === 0}
          size="sm"
        >
        ...
        {existingPlan && (
          <Button onClick={loadExistingPlan} size="sm" variant="outline">
            Muat Rute
          </Button>
        )}
        <Button onClick={handleCopyLastWeek} size="sm" variant="outline" disabled={isSaving || !selectedRepId}>
          <Copy className="h-4 w-4 mr-1" />
          Salin Minggu Lalu
        </Button>
        <Button onClick={handleImportExcel} size="sm" variant="outline" disabled={isSaving}>
          <Upload className="h-4 w-4 mr-1" />
          Import Excel
        </Button>
        {existingPlan && (
          <Button onClick={handleOptimize} size="sm" variant="outline" disabled={isSaving}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold">Daftar Customer</h3>
          ...
          <div className="border rounded-lg max-h-[500px] overflow-y-auto">
            {filteredCustomers.length === 0 ? (
              ...
              filteredCustomers.map((customer) => {
                ...
                const orderIdx = selectedCustomerIds.indexOf(customer.id);
                return (
                  <button
                    key={customer.id}
                    type="button"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        type="button"
                        onClick={() => moveCustomer(customerId, "up")}
                        disabled={idx === 0}
                      ...
                      <button
                        type="button"
                        onClick={() => moveCustomer(customerId, "down")}
                        disabled={idx === selectedCustomerIds.length - 1}
                        className="p-1 hover:bg-muted rounded disabled:opacity-30"
                      >
