...
export function TransactionDateFilter({
    ...
    const getActivePreset = React.useCallback(() => {
        ...
        if (
            date.to &&
            isSameDay(date.from, todayStart) &&
            isSameDay(date.to, todayEnd)
        )
            return 'today';
        if (
            date.to &&
            isSameDay(date.from, weekStart) &&
            isSameDay(date.to, weekEnd)
        )
            return 'this_week';
        if (
            date.to &&
            isSameDay(date.from, monthStart) &&
            isSameDay(date.to, monthEnd)
        )
            return 'this_month';
