...
export function CustomerProductPricesManager({
    ...
    async function handleImportCSV(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (!header.includes('sku') || !header.includes('harga')) {
            toast.error(
                'Format CSV tidak valid. Header harus: SKU, Nama Produk, Harga Customer, Catatan',
            );
            return;
        }
        ...
        if (successCount > 0) {
            toast.success(
                `${successCount} harga berhasil diimpor${errorCount > 0 ? `, ${errorCount} gagal` : ''}`,
            );
        } else {
            toast.error(
                `Semua ${errorCount} baris gagal diimpor. Periksa SKU dan format harga.`,
            );
        }
    }
                            ...
                            prices.map((price) => (
                                <TableRow key={price.id}>
                                    ...
                                    <TableCell className="text-right">
                                        {formatRupiah(
                                            price.productVariant.sellPrice ??
