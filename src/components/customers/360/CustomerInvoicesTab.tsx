...
export function CustomerInvoicesTab({ customerId }: { customerId: string }) {
    ...
    const load = useCallback(async () => {
        ...
        setInvoices(
            res.success ? ((res.data ?? []) as unknown as Invoice[]) : [],
        );
        setLoading(false);
    }, [customerId]);
    ...
    const totalUnpaid = invoices
        ...
        .reduce((s, i) => s + (toN(i.totalAmount) - toN(i.paidAmount)), 0);
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            {invoices.length} invoice · belum dibayar{' '}
                            {fmtIdr(totalUnpaid)}
                        </p>
                    </div>
                </div>
                                ...
                                {invoices.map((inv) => (
                                    <TableRow key={inv.id}>
                                        ...
                                        <TableCell className="text-right font-mono text-sm">
                                            {fmtIdr(toN(inv.totalAmount))}
                                        </TableCell>
                                        <TableCell className="text-right font-mono text-sm text-emerald-600">
                                            {fmtIdr(toN(inv.paidAmount))}
                                        </TableCell>
                                        <TableCell>
                                            <Badge
                                                variant="outline"
                                                className={`text-[10px] ${STATUS_BADGE[inv.status] || ''}`}
                                            >
                                                {inv.status}
                                            </Badge>
                                        </TableCell>
