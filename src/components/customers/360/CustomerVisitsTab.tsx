...
export function CustomerVisitsTab({ customerId }: { customerId: string }) {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await listCustomerVisits(customerId);
        setRows(res.success ? ((res.data as unknown as Visit[]) ?? []) : []);
        setLoading(false);
    }, [customerId]);
    useEffect(() => {
        load();
            ...
            <CardContent className="space-y-3">
                {loading ? (
                    ...
                    rows.map((v) => (
                        <div
                            key={v.id}
                            className="border-b border-border/50 py-3 flex justify-between gap-3"
                        >
                            <div>
                                ...
                                {v.latitude != null && v.longitude != null && (
                                    ...
                                    >
                                        GPS {Number(v.latitude).toFixed(5)},{' '}
