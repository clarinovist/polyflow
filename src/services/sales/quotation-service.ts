...
function normalizeQuotationLineItem(
    item: QuotationLineInput,
    variant: {
        primaryUnit: Unit;
        salesUnit: Unit | null;
    ...
    if (payloadCount > 0 && payloadCount < 4) {
        throw new ValidationError(
            'Incomplete quotation conversion payload. Send enteredQuantity, enteredUnit, enteredUnitPrice, and conversionFactorSnapshot together.',
        );
    }
    ...
    if (item.enteredUnit !== variant.primaryUnit) {
        if (!variant.salesUnit || item.enteredUnit !== variant.salesUnit) {
            throw new ValidationError(
                `Unit ${item.enteredUnit} is not valid for this product variant`,
            );
        }
    ...
    if (!Number.isFinite(factor) || factor <= 0) {
        throw new ValidationError(
            `Invalid conversion factor for quotation unit ${item.enteredUnit}`,
        );
    }
    ...
    assertNearlyEqual(
        item.quantity,
        baseQuantity,
        'Quotation quantity conversion',
    );
    assertNearlyEqual(
        item.unitPrice,
        baseUnitPrice,
        'Quotation unit price conversion',
    );
    assertNearlyEqual(
        item.conversionFactorSnapshot,
        factor,
        'Quotation conversion factor',
    );
...
export class QuotationService {
    ...
    static async createQuotation(
        data: CreateSalesQuotationValues,
        userId: string,
    ) {
        ...
        const itemsWithTotals = await Promise.all(
            data.items.map(async (item) => {
                ...
                const discountAmount =
                    rawSubtotal * (normalized.discountPercent / 100);
                const subtotalAfterDiscount = rawSubtotal - discountAmount;
                const taxAmount =
                    subtotalAfterDiscount * (normalized.taxPercent / 100);
