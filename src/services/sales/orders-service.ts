...
export async function confirmOrder(
    ...
    if (order.status !== SalesOrderStatus.DRAFT) {
        throw new BusinessRuleError('Only draft orders can be confirmed', {
            status: order.status,
        });
    }
    if (!order.customerId) {
    ...
    await prisma.$transaction(async (tx) => {
        if (order.sourceLocationId) {
            ...
            for (const item of order.items) {
                ...
                const reservedQty =
                    reservationMap.get(item.productVariantId) || 0;
            ...
            try {
                ...
                results.forEach((res, idx) => {
                    if (res.status === 'fulfilled') {
                        ...
                        const variantId =
                            creatableShortages[idx].productVariantId;
                ...
                warnings.push({
                    code: 'WO_CREATE_FAILED',
                    productVariantIds: creatableShortages.map(
