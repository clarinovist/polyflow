...
export function scopedCustomerWhere(
  ...
  return {
    OR: [
      { salesAssignments: { some: { userId: scope.actorUserId, unassignedAt: null } } },
      { routePlanItems: { some: { routePlan: { userId: scope.actorUserId, date: today, status: { in: ["DRAFT", "PUBLISHED"] } } } } },
...
export function scopedSalesOrderWhere(
  ...
  return {
    OR: [
      ...
      { customer: { salesAssignments: { some: { userId: scope.actorUserId, unassignedAt: null } } } },
    ],
  };
}
...
export function scopedInvoiceWhere(
  ...
  return {
    status: { in: ["UNPAID", "PARTIAL", "OVERDUE"] },
    salesOrder: { ...scopedSalesOrderWhere(scope), customerId: { not: null } },
  };
}
