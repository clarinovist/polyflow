  ...
  return prisma.$transaction(async (tx) => {
    ...
    const visit = await tx.salesVisit.create({
      data: {
        ...
        isExtraCall,
        extraReason: isExtraCall ? (extraReason as "TOKO_BARU" | "DEKAT_RUTE" | "PERMINTAAN_DADAKAN" | "TOKO_TUTUP_GANTI") : undefined,
      },
    });
...
export async function syncVisitLogs(
  ...
  const results: { clientVisitId: string; success: boolean; visitId?: string; error?: string }[] = [];

  for (const log of logs) {
    try {
      // Check idempotency
      const existing = await prisma.salesVisit.findUnique({
        where: { userId_clientVisitId: { userId, clientVisitId: log.clientVisitId } },
      });

      if (existing) {
        results.push({ clientVisitId: log.clientVisitId, success: true, visitId: existing.id });
        continue;
      }
      ...
      results.push({ clientVisitId: log.clientVisitId, success: true, visitId: visit.id });
    } catch (error) {
      results.push({
        clientVisitId: log.clientVisitId,
        success: false,
