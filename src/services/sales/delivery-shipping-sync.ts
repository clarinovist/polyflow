  ...
  if (!opts?.tx) {
    try {
      ...
      const soWithItems = await db.salesOrder.findUnique({
        where: { id: salesOrderId },
        select: {
          items: { select: { quantity: true, unitPrice: true, discountPercent: true, taxPercent: true, ppnMode: true, deliveredQty: true } },
        },
      });
      if (soWithItems && soWithItems.items.some((i: { deliveredQty: { toNumber(): number } | number }) => {
        const v = (i as { deliveredQty: { toNumber(): number } | number }).deliveredQty;
        const n = typeof (v as { toNumber?: () => number })?.toNumber === 'function' ? (v as { toNumber: () => number }).toNumber() : Number(v);
        return n > 0;
      })) {
        ...
        let goods = 0;
        for (const it of soWithItems.items as Array<Record<string, unknown>>) {
          const deliveredRaw = (it as { deliveredQty?: unknown }).deliveredQty as { toNumber?: () => number } | number | null | undefined;
          let delivered = 0;
          if (deliveredRaw != null) {
            delivered = typeof (deliveredRaw as { toNumber?: unknown }).toNumber === 'function'
              ? (deliveredRaw as { toNumber: () => number }).toNumber()
              : Number(deliveredRaw);
          }
          const unitRaw = (it as { unitPrice?: unknown }).unitPrice as { toNumber?: () => number } | number | null | undefined;
          let unit = 0;
          if (unitRaw != null) {
            unit = typeof (unitRaw as { toNumber?: unknown }).toNumber === 'function'
              ? (unitRaw as { toNumber: () => number }).toNumber()
              : Number(unitRaw);
          }
          const discRaw = (it as { discountPercent?: unknown }).discountPercent as { toNumber?: () => number } | number | null | undefined;
          let discPct = 0;
          if (discRaw != null) {
            discPct = typeof (discRaw as { toNumber?: unknown }).toNumber === 'function'
              ? (discRaw as { toNumber: () => number }).toNumber()
              : Number(discRaw);
          }
          const taxRaw = (it as { taxPercent?: unknown }).taxPercent as { toNumber?: () => number } | number | null | undefined;
          let taxPct = 0;
          if (taxRaw != null) {
            taxPct = typeof (taxRaw as { toNumber?: unknown }).toNumber === 'function'
              ? (taxRaw as { toNumber: () => number }).toNumber()
              : Number(taxRaw);
          }
          const raw = delivered * unit;
          const discount = raw * (discPct / 100);
          const afterDiscount = raw - discount;
          const ppnRes = calculatePpn(afterDiscount, taxPct, (it as { ppnMode: string }).ppnMode as PpnMode);
          goods += ppnRes.total;
        }
