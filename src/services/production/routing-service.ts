...
export class ProductionRoutingService {
    ...
    static async listProcesses(filter?: {
        isActive?: boolean;
        search?: string;
    }) {
        return prisma.productionProcess.findMany({
            ...
            include: {
                _count: { select: { capabilities: true, routeSteps: true } },
            },
        });
    }

    static async getProcessById(id: string) {
        const p = await prisma.productionProcess.findUnique({
            where: { id },
            include: { capabilities: { include: { machine: true } } },
        });
    ...
    static async createProcess(data: {
        ...
        const exists = await prisma.productionProcess.findUnique({
            where: { code: data.code },
        });
        if (exists)
            throw new BusinessRuleError(
                `Process code ${data.code} sudah ada`,
                undefined,
    ...
    static async updateProcess(
        ...
        if (data.code) {
            const dup = await prisma.productionProcess.findFirst({
    ...
    static async deleteProcess(id: string) {
        ...
        const used = await prisma.productionRouteStep.count({
            where: { processId: id },
        });
        if (used > 0)
            throw new BusinessRuleError(
                `Process ${p.code} masih dipakai di ${used} route step`,
                undefined,
                'ROUTE_PROCESS_IN_USE',
            );
        await prisma.machineProcessCapability.deleteMany({
            where: { processId: id },
        });
    ...
    static async setMachineCapabilities(
        ...
        return prisma.$transaction(async (tx) => {
            await tx.machineProcessCapability.deleteMany({
                where: { machineId },
            });
    ...
    static async listMachineCapabilities(
        machineId?: string,
        processId?: string,
    ) {
    ...
    static async addCapability(
        ...
        if (exists)
            throw new BusinessRuleError(
                'Capability sudah ada',
                undefined,
                'ROUTE_CAPABILITY_EXISTS',
            );
        return prisma.machineProcessCapability.create({
            data: { machineId, processId, isPrimary },
        });
    }
    ...
    static async createRoute(data: {
        ...
        if (!variant)
            throw new NotFoundError('ProductVariant', data.productVariantId);
        ...
        throw new BusinessRuleError(
            'Gagal membuat route setelah retry version collision',
            undefined,
            'ROUTE_CREATE_FAILED',
        );
    }
    ...
    static async updateRoute(
        ...
        if (route.status !== 'DRAFT') {
            throw new BusinessRuleError(
                'Hanya route DRAFT yang bisa diedit. Duplicate sebagai versi baru.',
                undefined,
        ...
        if (data.isDefault === true) {
            throw new BusinessRuleError(
                'Draft tidak bisa jadi default. Publish dulu lalu set default.',
                undefined,
                'ROUTE_DEFAULT_CONFLICT',
            );
        }
    ...
    static async duplicateRoute(id: string, actorId?: string) {
            ...
            try {
                return await prisma.$transaction(async (tx) => {
                    ...
                    const newRoute = await tx.productionRoute.create({
                        data: {
                            code: nextCode(
                                `RT-${route.productVariant.skuCode.slice(0, 6)}`,
                            ),
        ...
        throw new BusinessRuleError(
            'Gagal menduplikasi route setelah retry version collision',
            undefined,
            'ROUTE_DUPLICATE_FAILED',
        );
    }
    ...
    static async deleteRoute(id: string) {
        ...
        if (route.status !== 'DRAFT') {
            throw new BusinessRuleError(
                'Hanya DRAFT route yang bisa dihapus',
                undefined,
                'ROUTE_VERSION_IMMUTABLE',
            );
        }
        if (route._count.runs > 0) {
            throw new BusinessRuleError(
                'Route sudah punya production runs, tidak bisa dihapus. Archive saja.',
                undefined,
                'ROUTE_HAS_RUNS',
            );
        }
    ...
    static async addRouteStep(data: {
        ...
        if (route.status !== 'DRAFT')
            throw new BusinessRuleError(
                'Hanya DRAFT route yang bisa tambah step',
                undefined,
                'ROUTE_VERSION_IMMUTABLE',
            );
        ...
        if (!process)
            throw new NotFoundError('ProductionProcess', data.processId);
    ...
    static async updateRouteStep(
        ...
        if (step.route.status !== 'DRAFT')
            throw new BusinessRuleError(
                'Hanya DRAFT route yang bisa edit step',
                undefined,
                'ROUTE_VERSION_IMMUTABLE',
            );
    ...
    static async deleteRouteStep(id: string) {
        ...
        if (step.route.status !== 'DRAFT')
            throw new BusinessRuleError(
                'Hanya DRAFT route yang bisa hapus step',
                undefined,
                'ROUTE_VERSION_IMMUTABLE',
            );
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (let i = 0; i < remaining.length; i++) {
                if (remaining[i].sequence !== i) {
                    await tx.productionRouteStep.update({
    ...
    static async reorderSteps(routeId: string, orderedIds: string[]) {
        ...
        if (route.status !== 'DRAFT')
            throw new BusinessRuleError(
                'Hanya DRAFT route yang bisa reorder',
                undefined,
                'ROUTE_VERSION_IMMUTABLE',
            );

        const existing = await prisma.productionRouteStep.findMany({
            where: { routeId },
        });
        if (existing.length !== orderedIds.length) {
            throw new ValidationError(
                `Jumlah orderedIds (${orderedIds.length}) tidak sama dengan steps (${existing.length})`,
            );
        }
        ...
        for (const oid of orderedIds) {
            if (seen.has(oid))
                throw new ValidationError(
                    `orderedIds mengandung duplikat: ${oid}`,
                );
            seen.add(oid);
        }
        ...
        for (const oid of orderedIds) {
            if (!existingIds.has(oid))
                throw new ValidationError(
                    `Step ${oid} bukan bagian dari route`,
                );
        }
        ...
        return prisma.$transaction(async (tx) => {
            ...
            return tx.productionRouteStep.findMany({
                where: { routeId },
                orderBy: { sequence: 'asc' },
            });
        });
    ...
    static async validateRoute(
        routeId: string,
    ): Promise<{ valid: boolean; issues: RouteValidationIssue[] }> {
        ...
        const input: ValidateRouteInput = {
            ...
            steps: route.steps.map((s) => ({
                ...
                sourceLocationActive: s.materialSourceLocation
                    ? !isInactiveLocation(s.materialSourceLocation)
                    : undefined,
                outputLocationActive: s.outputLocation
                    ? !isInactiveLocation(s.outputLocation)
                    : undefined,
                isRiskyOutput: s.outputLocation
                    ? isRiskyOutputLocation(s.outputLocation)
                    : false,
                isRiskySource: s.materialSourceLocation
                    ? isInactiveLocation(s.materialSourceLocation) ||
                      isPackagingSuppliesWarehouse(s.materialSourceLocation)
                    : false,
                bomItems: s.bom.items.map((bi) => ({
                    productVariantId: bi.productVariantId,
                })),
                hasCapableMachine:
                    capableSet.has(s.processId) || !s.process.requiresMachine,
            })),
        };
    ...
    static async setDefaultRoute(routeId: string) {
        ...
        if (route.status !== 'ACTIVE')
            throw new BusinessRuleError(
                'Hanya ACTIVE route yang bisa jadi default',
                undefined,
                'ROUTE_DEFAULT_CONFLICT',
            );
