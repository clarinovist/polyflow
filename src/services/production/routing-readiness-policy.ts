...
export type RouteReadinessStatus = 'WAITING_WIP' | 'PARTIAL_WIP' | 'READY' | 'FIRST_STEP';
...
export function computeReadiness(input: ReadinessInput): {
  ...
  if (input.isLegacy || input.isFirstStep) {
    return { status: 'FIRST_STEP', canStart: true, availableQty: input.predecessorOutputAvailable };
  }
  ...
  if (input.predecessorOutputAvailable >= input.requiredQty) {
    return { status: 'READY', canStart: true, availableQty: input.predecessorOutputAvailable };
  }
...
export function canConsumeWip(
  ...
  if (requestedQty <= 0) return { allowed: false, reason: 'Quantity harus > 0' };
...
export function validateRouteContinuity(input: ValidateRouteInput): RouteValidationIssue[] {
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    const nextConsumes = next.bomItems.some((item) => item.productVariantId === cur.bomOutputVariantId);
    if (!nextConsumes) {
      issues.push({
