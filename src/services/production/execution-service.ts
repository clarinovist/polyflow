...
import { ProductionStatus, MovementType, ProductionExecution, Unit, Prisma } from '@prisma/client';
...
function assertClientBaseQuantityMatches(clientBaseQty: number | undefined, serverBaseQty: number) {
...
function assertClientConversionFactorMatches(clientFactor: number | undefined, serverFactor: number) {
...
async function resolveOutputQuantity(params: {
    ...
    if (enteredQuantity !== undefined && enteredUnit !== undefined) {
        ...
        assertClientBaseQuantityMatches(
            baseQuantityProduced !== undefined ? Number(baseQuantityProduced) : undefined,
            conversion.baseQuantityProduced
        );
        assertClientConversionFactorMatches(
            conversionFactorSnapshot !== undefined ? Number(conversionFactorSnapshot) : undefined,
...
function mergeExecutionEnteredQuantity(params: {
    ...
    const currentQty = currentEnteredQuantity ? Number(currentEnteredQuantity) : 0;
    ...
    static async stopExecution(data: StopExecutionValues & { userId?: string }) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const resolved = await resolveOutputQuantity({
                ...
                enteredQuantity: enteredQuantity !== undefined ? Number(enteredQuantity) : undefined,
                enteredUnit: enteredUnit as Unit | undefined,
                baseQuantityProduced: baseQuantityProduced !== undefined ? Number(baseQuantityProduced) : undefined,
                conversionFactorSnapshot: conversionFactorSnapshot !== undefined ? Number(conversionFactorSnapshot) : undefined,
                tx,
            });
            ...
            finalExecution = await tx.productionExecution.update({
                ...
                data: {
                    ...
                    conversionFactorSnapshot: resolved.conversionSnapshot ?? existingExecution.conversionFactorSnapshot,
                }
            });
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.productionExecution.update({
                ...
                data: {
                    ...
                    conversionFactorSnapshot: resolved.conversionSnapshot ?? execution.conversionFactorSnapshot,
                }
            });
