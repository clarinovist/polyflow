...
type ProductionOrderUnitClient = Pick<
...
export async function resolveProductionOutputUnit(
    ...
    if (salesUnit && enteredUnit === salesUnit) {
        conversionFactorSnapshot = toNumericConversionFactor(
            variant.conversionFactor,
        );

        if (
            !Number.isFinite(conversionFactorSnapshot) ||
            conversionFactorSnapshot <= 0
        ) {
