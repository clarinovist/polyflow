...
describe('ProductionMaterialService', () => {
    ...
    describe('recordMaterialIssue', () => {
        it('should record material issue and deduct stock', async () => {
            ...
            vi.mocked(prisma.productionOrder.findUnique).mockResolvedValue(mockOrder as any);
            vi.mocked(prisma.inventory.findUnique).mockResolvedValue(mockInventory as any);
    ...
    describe('deleteMaterialIssue', () => {
        it('should delete material issue and reverse stock', async () => {
            ...
            vi.mocked(prisma.materialIssue.findUnique).mockResolvedValue(mockIssue as any);
    ...
    describe('deleteScrap', () => {
        it('should delete scrap record', async () => {
            ...
            vi.mocked(prisma.scrapRecord.findUnique).mockResolvedValue(mockScrap as any);
    ...
    describe('batchIssueMaterials', () => {
        ...
        it('should issue materials from batches using FIFO', async () => {
            ...
            vi.mocked(prisma.batch.findMany).mockResolvedValue([
                { id: 'batch-1', quantity: { toNumber: () => 30 }, batchNumber: 'B001', manufacturingDate: new Date() },
                { id: 'batch-2', quantity: { toNumber: () => 20 }, batchNumber: 'B002', manufacturingDate: new Date() },
            ] as any);

            vi.mocked(prisma.inventory.update).mockResolvedValue({} as any);
            vi.mocked(prisma.materialIssue.create).mockResolvedValue({ id: 'issue-1' } as any);
            vi.mocked(prisma.stockMovement.create).mockResolvedValue({ id: 'movement-1' } as any);
