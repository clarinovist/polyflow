...
export class ProductionRoutingRunService {
    ...
    return prisma.productionRun.findMany({
      where: {
        ...
        ...(filter?.search
          ? {
              OR: [
                { runNumber: { contains: filter.search, mode: 'insensitive' as const } },
                { routeNameSnapshot: { contains: filter.search, mode: 'insensitive' as const } },
              ],
            }
          : {}),
      },
      ...
      include: {
        ...
        route: { select: { id: true, code: true, name: true, version: true, status: true } },
        orders: {
          select: {
            id: true,
            orderNumber: true,
  ...
  static async getRunById(id: string) {
    const run = await prisma.productionRun.findUnique({
      ...
      include: {
        ...
        route: {
          include: {
            steps: { orderBy: { sequence: 'asc' }, include: { process: true, bom: true, outputLocation: true, materialSourceLocation: true } },
          },
        },
        orders: {
          orderBy: { routeSequenceSnapshot: 'asc' },
          include: {
            bom: { select: { id: true, name: true, productVariantId: true, outputQuantity: true, items: { select: { productVariantId: true, quantity: true } } } },
            machine: { select: { id: true, name: true, code: true } },
            location: { select: { id: true, name: true, slug: true } },
            sourceLocation: { select: { id: true, name: true, slug: true } },
            routeStep: { select: { id: true, stepCode: true, label: true, processId: true, allowsPartialHandoff: true, sequence: true } },
          },
        },
   ...
   */
  static async generateRunNumber(tx: Prisma.TransactionClient): Promise<string> {
    for (let attempt = 0; attempt < 5; attempt++) {
      const candidate = genRunNumber();
      const exists = await tx.productionRun.findUnique({ where: { runNumber: candidate }, select: { id: true } });
  ...
  static computeStepQuantities(
    routeSteps: Array<{
      bom: {
        ...
        productVariantId?: string;
        items: Array<{ quantity: unknown; scrapPercentage?: unknown; productVariantId: string }>;
      };
      sequence: number;
    }>,
    finalQty: number,
    ...
    const sortedAsc = [...routeSteps].sort((a, b) => a.sequence - b.sequence);
    ...
    for (let idx = 0; idx < sortedDesc.length; idx++) {
      ...
      if (idx < sortedDesc.length - 1) {
        ...
        if (upstreamOutputVariant) {
          const consumingItem = step.bom.items.find((it) => it.productVariantId === upstreamOutputVariant);
          if (consumingItem) {
            const itemQty = Number(consumingItem.quantity ?? 1) || 1;
            const scrap = Number((consumingItem as { scrapPercentage?: unknown }).scrapPercentage ?? 0);
            requiredOfOutput = stepOutputQty * (itemQty / outputQty) * (1 + scrap / 100);
          } else {
  ...
  static async createRun(data: {
    ...
    if (data.plannedQuantity <= 0) throw new ValidationError('plannedQuantity harus > 0');

    const route = await ProductionRoutingService.getRouteById(data.routeId);
    if (route.status !== 'ACTIVE') throw new BusinessRuleError('Hanya ACTIVE route yang bisa dipakai membuat run', undefined, 'ROUTE_NOT_ACTIVE');
    if (route.steps.length === 0) throw new BusinessRuleError('Route tidak punya steps', undefined, 'ROUTE_NO_STEPS');

    const { valid, issues } = await ProductionRoutingService.validateRoute(data.routeId);
    if (!valid) {
      throw new BusinessRuleError(`Route tidak valid saat create run: ${issues.filter((i) => i.severity === 'BLOCKING').map((i) => i.message).join('; ')}`, { issues }, 'ROUTE_VALIDATION_FAILED');
    }

    // Idempotency check before tx if key provided
    if (data.idempotencyKey) {
      const existing = await prisma.productionRun.findUnique({ where: { idempotencyKey: data.idempotencyKey } });
    ...
    const attemptCreate = async (retries = 5): Promise<unknown> => {
      for (let attempt = 0; attempt < retries; attempt++) {
        try {
          return await prisma.$transaction(async (tx) => {
            const runNumber = await ProductionRoutingRunService.generateRunNumber(tx);
            ...
            for (let i = 0; i < route.steps.length; i++) {
              ...
              if (!outputLocId) throw new BusinessRuleError(`Step ${step.stepCode} tidak punya output location`, { stepCode: step.stepCode }, 'ROUTE_INVALID_LOCATION');
              const locExists = await tx.location.findUnique({ where: { id: outputLocId } });
              if (!locExists) throw new NotFoundError('Location', outputLocId);
              const scaledQty = stepQtyMap.get(step.sequence) ?? data.plannedQuantity;
              const plannedQtyDecimal = new Prisma.Decimal(scaledQty);
              const materialEntries = await tx.bom.findUnique({ where: { id: step.bom.id }, include: { items: true } });
              if (!materialEntries) throw new NotFoundError('Bom', step.bom.id);
              ...
              const order = await createProductionOrderWithGeneratedNumber(tx, orderData as unknown as Omit<Prisma.ProductionOrderCreateInput, 'orderNumber'>);
              const plannedMats = materialEntries.items.map((it) => ({
                productionOrderId: order.id,
                productVariantId: it.productVariantId,
                quantity: new Prisma.Decimal(it.quantity.toString()).mul(plannedQtyDecimal).div(new Prisma.Decimal(materialEntries.outputQuantity?.toString() || '1')),
              }));
              if (plannedMats.length > 0) await tx.productionMaterial.createMany({ data: plannedMats });
            }

            // I4: RM ready check for first step (location-aware)
            const firstStep = route.steps.find((s) => s.sequence === 0);
            if (firstStep) {
              const firstOrder = await tx.productionOrder.findFirst({
                where: { productionRunId: run.id, routeSequenceSnapshot: 0 },
                include: { plannedMaterials: true, bom: { include: { items: true } } },
              });
              if (firstOrder?.plannedMaterials && firstOrder.plannedMaterials.length > 0) {
                const srcLocId = firstStep.materialSourceLocationId;
                ...
                for (const pm of firstOrder.plannedMaterials) {
                  ...
                  if (srcLocId) {
                    const inv = await tx.inventory.findUnique({
                      where: { locationId_productVariantId: { locationId: srcLocId, productVariantId: pm.productVariantId } },
                      select: { quantity: true },
                    });
                    const invQty = inv ? Number(inv.quantity) : 0;
                    const resv = await tx.stockReservation.aggregate({
                      where: { locationId: srcLocId, productVariantId: pm.productVariantId, status: 'ACTIVE' },
                      _sum: { quantity: true },
                    });
                    available = Math.max(0, invQty - (resv._sum.quantity ? Number(resv._sum.quantity) : 0));
                  } else {
                    const invAgg = await tx.inventory.aggregate({
                      where: { productVariantId: pm.productVariantId },
                      _sum: { quantity: true },
                    });
                    const invQty = invAgg._sum.quantity ? Number(invAgg._sum.quantity) : 0;
                    const resv = await tx.stockReservation.aggregate({
                      where: { productVariantId: pm.productVariantId, status: 'ACTIVE' },
                      _sum: { quantity: true },
                    });
                    available = Math.max(0, invQty - (resv._sum.quantity ? Number(resv._sum.quantity) : 0));
                  }
      ...
      throw new BusinessRuleError('Gagal membuat run setelah retry collision', undefined, 'RUN_CREATE_FAILED');
    };
    ...
    for (const pm of (firstOrder as { plannedMaterials?: { productVariantId: string; quantity: unknown }[] }).plannedMaterials ?? []) {
      ...
      if (sourceLocId) {
        const inv = await prisma.inventory.findUnique({
          where: { locationId_productVariantId: { locationId: sourceLocId, productVariantId: pm.productVariantId } },
          select: { quantity: true },
        });
        const invQty = inv ? Number(inv.quantity) : 0;
        // Subtract active reservations at this location
        const resv = await prisma.stockReservation.aggregate({
          where: { locationId: sourceLocId, productVariantId: pm.productVariantId, status: 'ACTIVE' },
          _sum: { quantity: true },
        });
        const reserved = resv._sum.quantity ? Number(resv._sum.quantity) : 0;
   ...
   */
  static async cancelRun(id: string, actorId?: string, opts?: { force?: boolean; reason?: string }) {
    return prisma.$transaction(async (tx) => {
      ...
      const run = await tx.productionRun.findUnique({
        where: { id },
        include: { orders: { select: { id: true, orderNumber: true, status: true, actualQuantity: true } } },
      });
      if (!run) throw new NotFoundError('ProductionRun', id);
      if (run.status === 'COMPLETED' || run.status === 'CANCELLED') {
        throw new BusinessRuleError(`Run sudah ${run.status}`, undefined, 'RUN_ALREADY_FINAL');
      }

      const activeExec = await tx.productionExecution.findFirst({
        where: { productionOrder: { productionRunId: id }, endTime: null },
      ...
      const ordersWithOutput = run.orders.filter((o) => Number(o.actualQuantity ?? 0) > 0);
      if (ordersWithOutput.length > 0 && !opts?.force) {
        throw new BusinessRuleError(
          ...
          { ordersWithOutput: ordersWithOutput.map((o) => ({ id: o.id, orderNumber: o.orderNumber, actual: String(o.actualQuantity) })) },
          'RUN_HAS_OUTPUT',
        );
      }
      ...
      if (opts?.force) {
        ...
        const executions = await tx.productionExecution.findMany({
          where: { productionOrder: { productionRunId: id }, status: { not: 'VOIDED' } },
          select: { id: true },
          orderBy: { createdAt: 'asc' },
        });
        for (const execution of executions) {
          await voidProductionExecutionInTransaction(tx, execution.id, {
            sourceMarker: true,
            syncRun: false,
          });
        }
        ...
        await tx.productionOrder.updateMany({
          where: { productionRunId: id, status: { not: 'CANCELLED' } },
          data: { actualQuantity: 0, status: 'CANCELLED' },
        });
      } else {
        await tx.productionOrder.updateMany({
          where: { productionRunId: id, status: { notIn: ['COMPLETED', 'CANCELLED'] } },
      ...
      const updatedRun = await tx.productionRun.update({ where: { id }, data: { status: 'CANCELLED' } });
      if (actorId) {
        await tx.auditLog.create({
          data: {
            ...
            toStatus: 'CANCELLED',
            changes: opts?.reason ? JSON.stringify({ reason: opts.reason, force: opts?.force, ordersWithOutput: ordersWithOutput.length }) : undefined,
          },
        });
      }
      return updatedRun;
  ...
  static async computeRunProgress(runId: string) {
    ...
    if (total === 0) return { total: 0, completed: 0, inProgress: 0, percent: 0, currentStepLabel: null, blockedReason: null };

    const completed = run.orders.filter((o) => o.status === 'COMPLETED').length;
    const inProgress = run.orders.filter((o) => o.status === 'IN_PROGRESS').length;
    const percent = total > 0 ? Math.round((completed / total) * 100) : 0;

    const current = run.orders.find((o) => ['IN_PROGRESS', 'RELEASED', 'WAITING_MATERIAL', 'DRAFT'].includes(o.status as string));
    const currentStepLabel = current ? (current as { processNameSnapshot?: string }).processNameSnapshot ?? current.orderNumber : null;

    return { total, completed, inProgress, percent, currentStepLabel, blockedReason: null as string | null };
  }
  ...
  static async computeRunReadiness(runId: string) {
    const run = await this.getRunById(runId);
    const sortedOrders = [...run.orders].sort((a, b) => (a.routeSequenceSnapshot ?? 0) - (b.routeSequenceSnapshot ?? 0));

    const readiness: Array<{ orderId: string; status: string; canStart: boolean; reason?: string; available: number }> = [];
    ...
    for (let i = 0; i < sortedOrders.length; i++) {
      ...
      if (i === 0) {
        readiness.push({ orderId: order.id, status: 'FIRST_STEP', canStart: true, available: 0 });
        continue;
      }
      const prev = sortedOrders[i - 1] as (typeof sortedOrders)[0] & { bom?: { productVariantId: string } | null; actualQuantity?: unknown };

      const downstreamBom = order.bom as { productVariantId?: string; outputQuantity?: unknown; items?: Array<{ productVariantId: string; quantity: unknown }> } | null | undefined;
      const bomOutputQty = Number(downstreamBom?.outputQuantity ?? 1) || 1;
      const bomItem = downstreamBom?.items?.find((item) => item.productVariantId === prev.bom?.productVariantId);
      const requiredQty = Number(order.plannedQuantity) * (Number(bomItem?.quantity ?? 1) || 1) / bomOutputQty;

      let available = prev.actualQuantity ? Number(prev.actualQuantity) : 0;

      // If source location exists, compute inventory available-unallocated
      const sourceLocId = (order as unknown as { materialSourceLocationId?: string }).materialSourceLocationId ?? order.sourceLocation?.id;
      if (sourceLocId && prev.bom?.productVariantId) {
        try {
          const inv = await prisma.inventory.findUnique({
            where: { locationId_productVariantId: { locationId: sourceLocId, productVariantId: prev.bom.productVariantId } },
            select: { quantity: true },
          });
          const resv = await prisma.stockReservation.aggregate({
            where: { locationId: sourceLocId, productVariantId: prev.bom.productVariantId, status: 'ACTIVE', NOT: { referenceId: order.id } },
            _sum: { quantity: true },
          });
          const allocated = await prisma.productionOrder.aggregate({
            where: {
              productionRunId: runId,
              routeSequenceSnapshot: { gt: (order.routeSequenceSnapshot ?? i) - 1 },
              status: { in: ['RELEASED', 'IN_PROGRESS', 'COMPLETED'] },
              NOT: { id: order.id },
            },
            _sum: { plannedQuantity: true },
          });
          const allocatedQty = allocated._sum.plannedQuantity ? Number(allocated._sum.plannedQuantity) : 0;
          const invQty = inv ? Number(inv.quantity) : 0;
          const reserved = resv._sum.quantity ? Number(resv._sum.quantity) : 0;
          ...
          if (invQty > 0) available = Math.max(0, invAvailable - allocatedQty);
      ...
      const allowsPartial = (order.routeStep as { allowsPartialHandoff?: boolean } | undefined)?.allowsPartialHandoff ?? false;
      ...
      readiness.push({ orderId: order.id, status: r.status, canStart: r.canStart, reason: r.reason, available });
    }
