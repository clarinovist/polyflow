...
const ALLOWED_START_STATUSES = new Set([
    'RELEASED',
    'WAITING_MATERIAL',
    'IN_PROGRESS',
]);
...
export async function assertRoutedOrderCanStart(
    ...
    if (
        effectiveTarget === 'RELEASED' &&
        ALLOWED_RELEASE_STATUSES.has(order.status)
    ) {
    ...
    if (
        !ALLOWED_START_STATUSES.has(order.status) &&
        effectiveTarget !== 'RELEASED'
    ) {
    ...
    const prevActual = prevOrder.actualQuantity
        ? Number(prevOrder.actualQuantity)
        : 0;
    ...
    if (resolvedSourceLocationId && prevOrder.bom.productVariantId) {
        ...
        const allocatedQty = downstreamAllocated._sum.plannedQuantity
            ? Number(downstreamAllocated._sum.plannedQuantity)
            : 0;
    ...
    const bomOutputQty = downstreamBom
        ? Number(downstreamBom.outputQuantity ?? 1) || 1
        : 1;
    const bomItem = downstreamBom?.items.find(
        (it) => it.productVariantId === prevOrder.bom.productVariantId,
    );
    const itemQtyPerOutput = bomItem ? Number(bomItem.quantity ?? 1) || 1 : 1;
    const required =
        Number(order.plannedQuantity) * (itemQtyPerOutput / bomOutputQty);
    ...
    if (!readiness.canStart) {
        throw new BusinessRuleError(
            readiness.reason ??
                'SPK tahap downstream belum siap — menunggu WIP predecessor',
            {
                predecessorOrderId: prevOrder.id,
                available: availableUnallocated,
                required,
                allowsPartial: step.allowsPartialHandoff,
            },
            'ROUTE_WIP_NOT_READY',
        );
    }
...
export async function ensureRoutedOrderWipReservation(
    ...
    const itemQty =
        Number(
            bom?.items.find(
                (item) => item.productVariantId === predecessorVariantId,
            )?.quantity ?? 1,
        ) || 1;
...
export async function assertMachineCapableForOrder(
    ...
    const step = await tx.productionRouteStep.findUnique({
        ...
        select: {
            processId: true,
            process: { select: { code: true, requiresMachine: true } },
...
export async function syncProductionRunStatusFromOrders(
    ...
    const startDates = orders
        .map((o) => o.actualStartDate)
    ...
    if (newStatus) {
        ...
        if (current && current.status !== newStatus) {
            ...
            if (newIdx > curIdx || newStatus === 'CANCELLED') {
                ...
                if (newStatus === 'COMPLETED') {
                    updateData.actualEndDate =
                        latestEnd ?? opts?.completedAt ?? new Date();
                }
            ...
            if (
                ...
                await tx.productionRun.update({
                    where: { id: productionRunId },
                    data: { actualEndDate: latestEnd } as never,
                });
            }
        }
    }
