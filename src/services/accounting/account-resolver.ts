...
export async function resolveByPatterns(
    ...
    for (const pattern of patterns) {
        ...
        if (pattern.code) {
            ...
            if (
                tenantIsMelindo &&
                GHOST_CODES_SET.has(pattern.code) &&
                !pattern.code.startsWith('1-') &&
                !pattern.code.startsWith('7-') &&
                !pattern.code.startsWith('8-') &&
                !pattern.code.startsWith('3-200')
            ) {
            ...
            account = await target.account.findUnique({
...
export async function resolveAccount(
    ...
    const { tenantContext, getTenantIdFromContext, getMainPrisma } =
        ...
        try {
            ...
            if (mapping) {
                ...
                const account = tenantDb
                    ? await tenantDb.account.findUnique({
                ...
                if (mapping && !account) {
                    ...
                    logger.warn(
                        'TenantAccountRole orphan mapping; falling back to patterns',
                        {
            ...
            const { logger } = await import('@/lib/config/logger');
            logger.warn(
                'TenantAccountRole DB lookup failed; falling back to patterns',
                {
                    module: 'account-resolver',
                    role,
                    tenantId,
