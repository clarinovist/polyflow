...
import { findActiveReversal } from './journal-queries';
...
export async function postJournal(
    ...
    if (!isOpen) {
        throw new BusinessRuleError(
            'Tidak dapat memposting jurnal ke periode fiskal yang sudah ditutup.',
...
/**
 * Shared guards for void/reverse. Both operations only accept a POSTED,
 * non-auto-generated journal inside an open fiscal period.
 */
async function loadJournalForCancellation(id: string, actionLabel: string) {
    ...
    if (journal.status !== 'POSTED')
        throw new BusinessRuleError(
            `Hanya jurnal POSTED yang dapat ${actionLabel}`,
            { journalId: id, currentStatus: journal.status },
            'INVALID_STATUS_TRANSITION',
        );
    if (journal.isAutoGenerated)
        throw new BusinessRuleError(
            `Tidak dapat ${actionLabel} jurnal otomatis secara langsung`,
    ...
    if (!isOpen) {
        throw new BusinessRuleError(
            `Tidak dapat ${actionLabel} jurnal di periode fiskal yang sudah ditutup.`,
            { journalId: id },
            'FISCAL_PERIOD_CLOSED',
        );
    }
...
/**
 * Reverse: the original stays POSTED and a mirrored entry is posted alongside
 * it. Both remain visible in reports and net to zero.
 */
export async function reverseJournal(
    ...
    const journal = await loadJournalForCancellation(id, 'direverse');

    const existingReversal = await findActiveReversal(id);
    if (existingReversal)
        throw new BusinessRuleError(
            `Jurnal ini sudah dibalik oleh ${existingReversal.entryNumber}. Void jurnal pembalik tersebut lebih dulu jika ingin mereverse ulang.`,
            { journalId: id, reversalId: existingReversal.id },
            'ALREADY_REVERSED',
        );
...
/**
 * Void: a clean cancellation. The original drops to VOIDED and no counter-entry
 * is created — every report reads POSTED only, so the journal simply disappears.
 * Creating a reversal here too would leave the mirrored side posted on its own
...
export async function voidJournal(id: string, userId?: string) {
    ...
    await loadJournalForCancellation(id, 'di-void');

    const existingReversal = await findActiveReversal(id);
    if (existingReversal)
        throw new BusinessRuleError(
            `Jurnal ini sudah dibalik oleh ${existingReversal.entryNumber}, saldonya sudah netral. Void jurnal pembalik tersebut, bukan jurnal ini.`,
            { journalId: id, reversalId: existingReversal.id },
            'ALREADY_REVERSED',
        );
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
            throw new BusinessRuleError(
                `Unbalanced journal for reference ${entry.reference || 'UNKNOWN'}`,
                {
                    totalDebit,
                    totalCredit,
                    reference: entry.reference || 'UNKNOWN',
                },
                'UNBALANCED_JOURNAL',
            );
        }
