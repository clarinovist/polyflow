...
export async function createClosingJournalEntry(periodId: string, userId: string, tx?: Prisma.TransactionClient) {
    ...
    const lines = balances.map(b => {
        // Reverse Profit/Loss
        // Revenue (Positive netBalance) -> Debit it to reset to zero
        // Expense (Positive netBalance) -> Credit it to reset to zero
    ...
    // Find Retained Earnings account (Current Year Earnings 33000)
    ...
    // Add balancing line to Earnings Account
    // If Net Income is Positive (Profit) -> Credit Earnings
    // If Net Income is Negative (Loss) -> Debit Earnings
    lines.push({
    ...
    return await createJournalEntry({
        ...
        reference: `CLOSING-${period.name.replace(/\s+/g, '-')}`,
