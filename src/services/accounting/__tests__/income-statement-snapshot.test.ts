...
it('binds income statement to the supplied transaction, never the ambient proxy', async () => {
    ...
    expect(findMany).toHaveBeenCalledWith(expect.objectContaining({ include: { journalLines: { where: { journalEntry: { entryDate: expect.any(Object), status: 'POSTED', OR: [{ reference: null }, { NOT: [{ reference: { startsWith: 'CLOSING-' } }, { reference: { startsWith: 'CLOSE-' } }] }] } } } } }));
});
