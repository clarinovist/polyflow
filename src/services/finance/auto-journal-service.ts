...
export class AutoJournalService {
    ...
    static async handlePurchaseInvoiceCreated(invoiceId: string) {
        const invoice = await prisma.purchaseInvoice.findUnique({
            where: { id: invoiceId },
            include: { purchaseOrder: { include: { goodsReceipts: { take: 1 } } } }
        });
        ...
        const hasGoodsReceipt = invoice.purchaseOrder.goodsReceipts.length > 0;

        const lines = [];

        if (hasGoodsReceipt) {
            // Normal flow: GR already credited 21120, so debit to clear
            const unbilledAcc = await this.getAccountByCode('21120');
            lines.push({ accountId: unbilledAcc.id, debit: subtotal, credit: 0, description: 'Clear Unbilled Accrual' });
        } else {
            // No GR yet: debit inventory directly to avoid 21120 going negative
            const inventoryAcc = await this.getAccountByCode('11310');
            lines.push({ accountId: inventoryAcc.id, debit: subtotal, credit: 0, description: 'Direct Purchase (No GR)' });
        }
