...
describe("JournalsService", () => {
      ...
      vi.mocked(prisma.journalEntry.findUnique).mockImplementation(
        (async (args: Record<string, unknown>) => {
          ...
          if (where && typeof where === "object" && "entryNumber" in where) {
            // First call: candidate sudah ada; after resync: free
  ...
  describe("createClosingJournalEntry", () => {
    it("throws when period is tidak ditemukan", async () => {
      // Arrange
    ...
    it("throws when earnings account tidak ditemukan in COA", async () => {
      // Arrange
      vi.mocked(prisma.fiscalPeriod.findUnique).mockResolvedValue({
        id: "period-1",
        name: "January 2026",
  ...
  describe("createYearEndClosingEntry", () => {
    ...
    it("throws when earnings account tidak ditemukan", async () => {
      // Arrange
      vi.mocked(resolveAccount).mockResolvedValue({
        id: earningsAccountId,
      } as never);
    ...
    it("throws when retained earnings account tidak ditemukan", async () => {
      // Arrange
      vi.mocked(resolveAccount)
  ...
  describe("postJournal", () => {
    ...
    it("throws when journal tidak ditemukan", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue(null);
    ...
    it("throws when journal is not DRAFT", async () => {
      ...
      await expect(postJournal("je-1")).rejects.toThrow(
        "Hanya jurnal berstatus DRAFT yang dapat diposting",
      );
    });
    ...
    it("throws when fiscal period is closed", async () => {
      ...
      // Act & Assert
      await expect(postJournal("je-1")).rejects.toThrow(/periode fiskal/i);
    });
  ...
  describe("voidJournal", () => {
    it("voids a posted journal without creating a counter-entry", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue({
        id: "je-1",
        status: "POSTED",
        ...
        lines: [
          { id: "line-1", accountId: "acc-1", debit: 100, credit: 0, description: "Debit" },
          { id: "line-2", accountId: "acc-2", debit: 0, credit: 100, description: "Credit" },
        ],
      } as never);
      vi.mocked(prisma.journalEntry.findFirst).mockResolvedValue(null);
      vi.mocked(prisma.journalEntry.update).mockResolvedValue({
      ...
      // Assert — a reversal here would leave the mirrored side posted alone
      // and shift the account balance by the full amount.
      expect(result.status).toBe("VOIDED");
      expect(prisma.journalEntry.create).not.toHaveBeenCalled();
    });

    it("throws when the journal already has an active reversal", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue({
        id: "je-1",
        status: "POSTED",
      ...
      // Act & Assert
      await expect(voidJournal("je-1", "user-1")).rejects.toThrow(
        /sudah dibalik oleh JE - 2026 -00002/i,
      );
    ...
    it("throws when journal is not POSTED", async () => {
      ...
      await expect(voidJournal("je-1")).rejects.toThrow(
        "Hanya jurnal POSTED yang dapat di-void",
      );
    });
    ...
    it("throws when journal is auto-generated", async () => {
      ...
      // Act & Assert
      await expect(voidJournal("je-1")).rejects.toThrow(/otomatis/i);
    });
    ...
    it("throws when fiscal period is closed", async () => {
      ...
      // Act & Assert
      await expect(voidJournal("je-1")).rejects.toThrow(/periode fiskal/i);
    });
  });
  ...
  describe("reverseJournal", () => {
    it("creates a reversal entry with swapped debit/credit", async () => {
      ...
      expect(data.referenceId).toBe("je-1");
    });

    it("leaves the original journal POSTED so both sides stay in reports", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique)
        .mockResolvedValueOnce({
          id: "je-1",
          ...
          isAutoGenerated: false,
          lines: [
            { accountId: "acc-1", debit: BigInt(100), credit: BigInt(0), description: "Debit line" },
          ],
        } as never)
      ...
      // Act
      await reverseJournal("je-1", "user-1");

      // Assert — voiding the original would drop it from the POSTED-only
      // reports and leave the reversal standing on its own.
      expect(prisma.journalEntry.update).not.toHaveBeenCalled();
    });

    it("throws when the journal was already reversed", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue({
        id: "je-1",
        entryNumber: "JE - 2026 -00001",
      ...
      // Act & Assert
      await expect(reverseJournal("je-1", "user-1")).rejects.toThrow(
        /sudah dibalik oleh JE - 2026 -00002/i,
      );
      expect(prisma.journalEntry.create).not.toHaveBeenCalled();
    });
    ...
    it("throws when journal is auto-generated", async () => {
      ...
      // Act & Assert
      await expect(reverseJournal("je-1")).rejects.toThrow(/otomatis/i);
    });
  ...
  describe("getJournalById", () => {
    it("returns journal with relations", async () => {
      ...
      // Assert
      expect(result).toEqual({ ...mockJournal, reversal: null });
    ...
    it("attaches the active reversal so the UI can block a second offset", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue({
        id: "je-1",
        entryNumber: "JE - 2026 -00001",
    ...
    it("returns null without querying reversals when the journal is missing", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue(null);
    ...
    it("returns null when journal tidak ditemukan", async () => {
      // Arrange
      vi.mocked(prisma.journalEntry.findUnique).mockResolvedValue(null);
