        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                const count = await prisma.pettyCashTransaction.count({
                    where: {
                        voucherNumber: { startsWith: `PCV-${yearMonth}` },
                    },
                });
    ...
    static async approveExpense(id: string, userId: string) {
        ...
        if (transaction.status !== 'DRAFT')
            throw new BusinessRuleError(
                'Hanya transaksi berstatus DRAFT yang dapat disetujui',
                { transactionId: id, currentStatus: transaction.status },
            );
        if (!transaction.expenseAccountId)
            throw new BusinessRuleError('Akun biaya harus dipilih', {
                transactionId: id,
            });
    ...
    static async replenish(
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                const count = await prisma.pettyCashTransaction.count({
                    where: {
                        voucherNumber: { startsWith: `PCR-${yearMonth}` },
                    },
                });
