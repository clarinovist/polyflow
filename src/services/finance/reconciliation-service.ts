...
export class ReconciliationService {
    ...
    static async autoMatchAndSave(reconciliationId: string) {
        ...
        for (const item of unmatchedItems) {
            ...
            const candidates = journalLines.filter((line) => {
                ...
                const daysDiff =
                    Math.abs(
                        line.journalEntry.entryDate.getTime() -
                            bankDate.getTime(),
                    ) /
                    (1000 * 3600 * 24);
                return daysDiff <= 3;
     ...
     */
    static async manualMatch(itemId: string, journalLineId: string) {
    ...
    static async createAdjustmentJournals(
        ...
        for (const adj of bookAdjustments) {
            ...
            const offsetRole =
                ADJUSTMENT_OFFSET_ROLE[adj.type] ?? 'suspense-clearing';
            ...
            const bankIncreases =
                ADJUSTMENT_BANK_INCREASES[adj.type] ?? amount > 0;

            // Build journal lines: bank ↔ offset
            const drAccountId = bankIncreases
                ? reconciliation.accountId
                : offset.id;
            const crAccountId = bankIncreases
                ? offset.id
                : reconciliation.accountId;
