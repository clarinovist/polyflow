...
export class PurchaseReturnService {
    ...
    static async createReturn(
        data: CreatePurchaseReturnValues,
        userId: string,
    ) {
        let totalAmount = 0;
        ...
        const maxAttempts = 3;
        let attempt = 0;
        let purchaseReturn: Awaited<
            ReturnType<typeof prisma.purchaseReturn.create>
        > | null = null;
        let returnNumber = '';
    ...
    static async updateReturn(
        ...
        if (data.items) {
            totalAmount = data.items.reduce(
                (sum, item) => sum + item.returnedQty * item.unitCost,
                0,
            );
    ...
    static async shipReturn(id: string, userId: string) {
        ...
        try {
            ...
            logger.error(
                ...
                { error, returnId: id, module: 'PurchaseReturnService' },
            );
        }
