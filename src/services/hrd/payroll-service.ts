...
export const PayrollService = {
    async getWeeklyPayroll(
        ...
        const records: PayrollRecord[] = attendance.map((r) => {
            const actualHours =
                r.actualHours == null ? null : toNum(r.actualHours);
        ...
        if (employee.payType === 'PIECE') {
            ...
            for (const ex of executions) {
                ...
                const rate =
                    ex.pieceRateSnapshot == null
                        ? null
                        : toNum(ex.pieceRateSnapshot);
                const earnings =
                    ex.pieceEarnings == null ? null : toNum(ex.pieceEarnings);
        ...
        const isShopfloor =
            employee.payType === EmployeePayType.DAILY ||
            employee.payType === EmployeePayType.PIECE;
        const bpjsDeduction =
        ...
        return {
            ...
            totalOvertimeEarnings:
                Math.round(totalOvertimeEarnings * 100) / 100,
    ...
    async getWeeklyPayrollForAll(
        ...
        return Promise.all(
            employees.map((e) =>
                this.getWeeklyPayroll(db, e.id, weekStart, weekEnd),
            ),
        );
    },
};
