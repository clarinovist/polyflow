...
async function main() {
    ...
    const locations = [
        ...
        },
        {
            ...
            slug: 'wip_storage',
            description:
                'Work-in-Progress storage for Roll Film before Converting',
        },
        {
            name: 'Maklon Raw Material Storage',
            slug: 'maklon_raw_material',
            description:
                'Customer-owned raw materials received for maklon production',
        ...
        // Extrusion output now goes to FG, so no separate Extrusion Warehouse.
        {
            ...
            slug: 'fg_warehouse',
            description:
                'Hold Extrusion Output (Jumbo Rolls) and Packing Output (Small Packs)',
        },
        {
            name: 'Packing Area',
            slug: 'packing_area',
            description:
                'Production floor for bag-making and finishing processes',
        },
        {
        ...
        {
            name: 'Customer Owned Storage',
            slug: 'CUSTOMER_OWNED',
            description: 'Storage for customer owned materials (Maklon)',
            locationType: LocationType.CUSTOMER_OWNED,
        },
    ];
    ...
    await prisma.customer.create({
        data: {
            name: 'Plastic Shop Jaya',
            phone: '08987654321',
            billingAddress: 'Downtown Market',
        },
    });
    ...
    const locRM = (await prisma.location.findUnique({
        where: { slug: 'rm_warehouse' },
    }))!;
    const locMix = (await prisma.location.findUnique({
        where: { slug: 'mixing_area' },
    }))!;
    const locFG = (await prisma.location.findUnique({
        where: { slug: 'fg_warehouse' },
    }))!;
    ...
    const vMixed = await prisma.productVariant.findFirst({
        where: { skuCode: 'INT-MIX-BLACK' },
    });
    const vPPKarung = await prisma.productVariant.findFirst({
        where: { skuCode: 'RM-PP-KARUNG' },
    });
    const vPPSablon = await prisma.productVariant.findFirst({
        where: { skuCode: 'RM-PP-SABLON' },
    });
    const vPPGreen = await prisma.productVariant.findFirst({
        where: { skuCode: 'RM-PP-GREEN-REC' },
    });
    const vMB = await prisma.productVariant.findFirst({
        where: { skuCode: 'RM-MB-BLACK' },
    });
            ...
            data: {
                ...
                items: {
                    create: [
                        ...
                        {
                            productVariantId: vPPSablon.id,
                            quantity: 30,
                            scrapPercentage: 0,
                        },
                        {
                            productVariantId: vPPGreen.id,
                            quantity: 30,
                            scrapPercentage: 0,
                        },
                        {
                            productVariantId: vMB.id,
                            quantity: 2,
                            scrapPercentage: 0,
                        },
                    ],
    ...
    const vJumbo = await prisma.productVariant.findFirst({
        where: { skuCode: 'FG-JUMBO-BLACK' },
    });
    ...
    if (vJumbo && vMixed) {
        await prisma.bom.create({
            data: {
                ...
                isDefault: true,
                items: {
                    create: [{ productVariantId: vMixed.id, quantity: 50 }],
                },
            },
        });
    }
    ...
    const vPack = await prisma.productVariant.findFirst({
        where: { skuCode: 'FG-PACK-BLACK-1KG' },
    });
    ...
    if (vPack && vJumbo) {
        await prisma.bom.create({
            data: {
                ...
                isDefault: true,
                items: {
                    create: [{ productVariantId: vJumbo.id, quantity: 15 }],
                },
            },
        });
    }
    ...
    const locPack = (await prisma.location.findUnique({
        where: { slug: 'packing_area' },
    }))!;
    ...
    const bomMixing = await prisma.bom.findFirst({
        where: { name: 'Formula Adonan Hitam Standard' },
    });
    const mixerMachine = await prisma.machine.findFirst({
        where: { code: 'MIX-01' },
    });
