...
async function ensureDefaultAdmin() {
  const email =
    ...
    "admin@polyflow.uk";
  const name =
    process.env.TENANT_ADMIN_NAME ??
    process.env.DEFAULT_ADMIN_NAME ??
    "Nugroho Pramono";

  // Checking if we already got a hash from the action, else hash from env var
  ...
  if (!passwordHash && !passwordPlain) {
    throw new Error(
      "DEFAULT_ADMIN_PASSWORD or TENANT_ADMIN_PASSWORD_HASH must be set",
    );
  }
  ...
  const password = passwordHash
    ? passwordHash
    : await bcrypt.hash(passwordPlain!, 10);
...
async function ensureExampleProducts() {
  ...
  for (const ex of examples) {
    const existing = await prisma.productVariant.findUnique({
      where: { skuCode: ex.variant.skuCode },
    });
...
async function main() {
  ...
  const [locations, products, variants, suppliers, customers] =
    await Promise.all([
