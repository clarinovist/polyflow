...
- **Issue:** `src/lib/zod-schemas.ts` contains validation logic for _every_ domain (Inventory, Products, Production, CRM).
...
- **Status:** ✅ RESOLVED — Schemas split into `src/lib/schemas/` with domain-specific files (`finance.ts`, `inventory.ts`, `invoice.ts`, etc.)
...
- **Status:** ✅ RESOLVED — Service layer implemented in `src/services/` with 12 domain modules (accounting, analytics, auth, core, dashboard, finance, inventory, maklon, production, purchasing, sales, plus test suites).
...
1.  **Split Schemas:** ✅ COMPLETED
    - Schemas split into `src/lib/schemas/` with domain-specific files.
2.  **Fix Serialization:**
...
3.  **Implement Service Layer:** ✅ COMPLETED
    - Service layer implemented in `src/services/` with 12 domain modules.
    - Server Actions now follow: `Auth Check` -> `Input Parsing` -> `Call Service` -> `Return Result`.
